...
Slaw-derived text ingestion stays limited to issue titles/descriptions, issue comments, and issue documents.
...
The accepted policy lives in [doc/plans/2026-05-06-llm-wiki-slaw-asset-security-gate.md](../../../doc/plans/2026-05-06-llm-wiki-slaw-asset-security-gate.md).
...
Run these commands from the Slaw repo root before handing off alpha plugin
changes:
...
- disabled and enabled Slaw event ingestion paths
...
- Host-level plugin install and live agent invocation still need Slaw
...
curl -X POST http://127.0.0.1:3100/api/plugins/install \
  ...
  -d '{"packageName":"/Users/dotta/slaw/.slaw/worktrees/PAP-3179-design-a-llm-wiki-plugin/packages/plugins/plugin-llm-wiki","isLocalPath":true}'
```
...
restart the Slaw dev server so the host imports the fresh bundle.
...
write the starter files. The plugin uses Slaw's local folder API for path
...
`CLAUDE.md` renamed to `AGENTS.md` and Slaw project overviews, standups,
