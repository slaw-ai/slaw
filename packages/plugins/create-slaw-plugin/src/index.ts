...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".slaw-sdk");
...
/**
 * Generate a complete Slaw plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Slaw plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "slaw-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@slaw-ai/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@slaw-ai/plugin-sdk": sdkDependency,
...
`,
    ...
    writeFile(
      ...
      `import { usePluginData, type PluginWidgetProps } from "@slaw-ai/plugin-sdk/ui";
...
`,
    ...
    writeFile(
      path.join(outputDir, "src", "ui", "index.tsx"),
      `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@slaw-ai/plugin-sdk/ui";
...
When this package is installed from a local path, Slaw watches that rebuilt
output and reloads the plugin worker. Local installs run trusted code from this
folder on your machine.

${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@slaw-ai/plugin-sdk\` and \`@slaw-ai/shared\` from a local Slaw checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.slaw-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}

## Install Into Slaw

\`\`\`bash
slaw plugin install ${shellQuote(toPosixPath(outputDir))}
\`\`\`
