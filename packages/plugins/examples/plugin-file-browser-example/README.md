...
Example Slaw plugin that demonstrates:
...
```bash
pnpm slaw plugin uninstall slaw-file-browser-example --force
```
...
- Optional: use `slaw-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
