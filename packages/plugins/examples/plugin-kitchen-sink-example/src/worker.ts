...
async function runCuratedCommand(
  ...
  const workspace = await resolveWorkspace(ctx, squadId, projectId, workspaceId);
...
function getCurrentSquadId(params: Record<string, unknown>): string {
...
async function listIssuesForSquad(ctx: PluginContext, squadId: string, limit = 50): Promise<Issue[]> {
  return await ctx.issues.list({ squadId, limit, offset: 0 });
}

async function listGoalsForSquad(ctx: PluginContext, squadId: string, limit = 50): Promise<Goal[]> {
  return await ctx.goals.list({ squadId, limit, offset: 0 });
}
...
async function registerDataHandlers(ctx: PluginContext): Promise<void> {
  ...
  ctx.data.register("overview", async (params) => {
    ...
    const squads = await ctx.squads.list({ limit: 200, offset: 0 });
    const projects = squadId ? await ctx.projects.list({ squadId, limit: 200, offset: 0 }) : [];
    const issues = squadId ? await listIssuesForSquad(ctx, squadId, 200) : [];
    const goals = squadId ? await listGoalsForSquad(ctx, squadId, 200) : [];
    const agents = squadId ? await ctx.agents.list({ squadId, limit: 200, offset: 0 }) : [];
  ...
  ctx.data.register("squads", async (params) => {
    return await ctx.squads.list({ limit: getListLimit(params), offset: 0 });
  });

  ctx.data.register("projects", async (params) => {
    const squadId = getCurrentSquadId(params);
    return await ctx.projects.list({ squadId, limit: getListLimit(params), offset: 0 });
  });

  ctx.data.register("issues", async (params) => {
    const squadId = getCurrentSquadId(params);
    return await listIssuesForSquad(ctx, squadId, getListLimit(params));
  });

  ctx.data.register("goals", async (params) => {
    const squadId = getCurrentSquadId(params);
    return await listGoalsForSquad(ctx, squadId, getListLimit(params));
  });

  ctx.data.register("agents", async (params) => {
    const squadId = getCurrentSquadId(params);
    return await ctx.agents.list({ squadId, limit: getListLimit(params), offset: 0 });
  });
  ...
  ctx.data.register("workspaces", async (params) => {
    ...
    if (!projectId) return [];
    return await ctx.projects.listWorkspaces(projectId, squadId);
  });
  ...
  ctx.data.register("entity-context", async (params) => {
    ...
    if (!squadId || !entityId || !entityType) return null;

    if (entityType === "project") {
      return await ctx.projects.get(entityId, squadId);
    }
    if (entityType === "issue") {
      return await ctx.issues.get(entityId, squadId);
    }
    if (entityType === "goal") {
      return await ctx.goals.get(entityId, squadId);
    }
    if (entityType === "agent") {
      return await ctx.agents.get(entityId, squadId);
    }
    return { entityId, entityType, squadId };
  });
}
...
async function registerActionHandlers(ctx: PluginContext): Promise<void> {
  ctx.actions.register("emit-demo-event", async (params) => {
    ...
    await ctx.events.emit("demo-event", squadId, {
      message,
      source: "kitchen-sink",
  ...
  ctx.actions.register("create-issue", async (params) => {
    ...
    const issue = await ctx.issues.create({ squadId, projectId, title, description });
    pushRecord({
      level: "info",
  ...
  ctx.actions.register("advance-issue-status", async (params) => {
    ...
    const issue = await ctx.issues.update(issueId, { status: status as Issue["status"] }, squadId);
    pushRecord({
      level: "info",
  ...
  ctx.actions.register("create-goal", async (params) => {
    ...
    const description = typeof params.description === "string" ? params.description : undefined;
    const goal = await ctx.goals.create({ squadId, title, description, level: "team", status: "planned" });
    pushRecord({
      level: "info",
      source: "goals.create",
  ...
  ctx.actions.register("advance-goal-status", async (params) => {
    ...
    const goal = await ctx.goals.update(goalId, { status: status as Goal["status"] }, squadId);
    pushRecord({
      level: "info",
      source: "goals.update",
  ...
  ctx.actions.register("run-process", async (params) => {
    ...
    return await runCuratedCommand(ctx, config, squadId, projectId, workspaceId, commandKey);
  });
  ...
  ctx.actions.register("start-progress-stream", async (params) => {
    ...
    void (async () => {
      ctx.streams.open(STREAM_CHANNELS.progress, squadId);
      try {
  ...
  ctx.actions.register("invoke-agent", async (params) => {
    ...
    const result = await ctx.agents.invoke(agentId, squadId, { prompt, reason: "Kitchen Sink plugin demo" });
    pushRecord({
      level: "info",
  ...
  ctx.actions.register("pause-agent", async (params) => {
    ...
    const agentId = typeof params.agentId === "string" ? params.agentId : "";
    if (!agentId) throw new Error("agentId is required");
    return await ctx.agents.pause(agentId, squadId);
  });
  ...
  ctx.actions.register("resume-agent", async (params) => {
    ...
    const agentId = typeof params.agentId === "string" ? params.agentId : "";
    if (!agentId) throw new Error("agentId is required");
    return await ctx.agents.resume(agentId, squadId);
  });
  ...
  ctx.actions.register("ask-agent", async (params) => {
    ...
    ctx.streams.open(STREAM_CHANNELS.agentChat, squadId);
...
async function registerToolHandlers(ctx: PluginContext): Promise<void> {
  ...
  ctx.tools.register(
    TOOL_NAMES.squadSummary,
    {
      displayName: "Kitchen Sink Squad Summary",
      description: "Summarizes current squad counts from the Slaw APIs.",
      parametersSchema: { type: "object", properties: {} },
    },
    async (_params, runCtx): Promise<ToolResult> => {
      const projects = await ctx.projects.list({ squadId: runCtx.squadId, limit: 50, offset: 0 });
      const issues = await ctx.issues.list({ squadId: runCtx.squadId, limit: 50, offset: 0 });
      const goals = await ctx.goals.list({ squadId: runCtx.squadId, limit: 50, offset: 0 });
      const agents = await ctx.agents.list({ squadId: runCtx.squadId, limit: 50, offset: 0 });
      return {
        content: `Squad has ${projects.length} projects, ${issues.length} issues, ${goals.length} goals, and ${agents.length} agents.`,
        data: {
          squadId: runCtx.squadId,
          projects: projects.length,
          issues: issues.length,
          goals: goals.length,
          agents: agents.length,
...
const plugin: SlawPlugin = definePlugin({
  async setup(ctx) {
    currentContext = ctx;
