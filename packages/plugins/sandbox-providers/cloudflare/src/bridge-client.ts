...
function buildHeaders(config: CloudflareDriverConfig, extra: BridgeClientHeaders = {}): Headers {
  ...
  if (extra.environmentId) headers.set("X-Slaw-Environment-Id", extra.environmentId);
  if (extra.runId) headers.set("X-Slaw-Run-Id", extra.runId);
  if (extra.issueId) headers.set("X-Slaw-Issue-Id", extra.issueId);
  return headers;
}
...
export function createCloudflareBridgeClient(options: BridgeClientOptions) {
  ...
  const apiPrefix = "/api/slaw-sandbox/v1";
