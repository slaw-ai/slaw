...
const PLUGIN_ID = "slaw.exe-dev-sandbox-provider";
...
const manifest: SlawPluginManifestV1 = {
  ...
  description:
    "Sandbox provider plugin that provisions exe.dev VMs as Slaw execution environments.",
  author: "Slaw",
  ...
  environmentDrivers: [
    {
      ...
      description:
        "Provisions exe.dev VMs through the HTTPS API, then runs commands over direct SSH for long-lived Slaw workloads.",
      configSchema: {
        type: "object",
        properties: {
          ...
          apiKey: {
            ...
            format: "secret-ref",
            description:
              "Paste your exe.dev API token, or pick a saved Slaw secret. Create one at exe.dev → Settings → API tokens with `/exec` scope (`new`, `ls`, `rm`).",
          },
          sshPrivateKey: {
            type: "string",
            format: "secret-ref",
          ...
          sshUser: {
            ...
            "x-slaw-advanced": true,
            "x-slaw-group": "SSH access",
          },
          sshIdentityFile: {
            type: "string",
            description:
              "Absolute path to a private key on the Slaw host. Used only when SSH Private Key is empty.",
            "x-slaw-advanced": true,
            "x-slaw-group": "SSH access",
          },
          sshPort: {
