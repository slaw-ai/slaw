...
export interface ScopeKey {
  /** What kind of Slaw object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Slaw UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Slaw domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Slaw domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Slaw secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Slaw secret provider (e.g. `"MY_API_KEY"`).
   *
...
/**
 * `ctx.routines` — resolve and reconcile plugin-managed Slaw routines.
 *
