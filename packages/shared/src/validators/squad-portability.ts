import { z } from "zod";
import { MAX_SQUAD_ATTACHMENT_MAX_BYTES } from "../constants.js";
import {
...
export const portabilitySquadManifestEntrySchema = z.object({
  path: z.string().min(1),
  name: z.string().min(1),
  description: z.string().nullable(),
...
export const portabilityManifestSchema = z.object({
  ...
  squad: portabilitySquadManifestEntrySchema.nullable(),
...
export const portabilityTargetSchema = z.discriminatedUnion("mode", [
  z.object({
    mode: z.literal("new_squad"),
    newSquadName: z.string().min(1).optional().nullable(),
  }),
  z.object({
    mode: z.literal("existing_squad"),
    squadId: z.string().uuid(),
  }),
]);
...
export const squadPortabilityExportSchema = z.object({
...
export type SquadPortabilityExport = z.infer<typeof squadPortabilityExportSchema>;

export const squadPortabilityPreviewSchema = z.object({
...
export type SquadPortabilityPreview = z.infer<typeof squadPortabilityPreviewSchema>;
...
export const squadPortabilityImportSchema = squadPortabilityPreviewSchema.extend({
  adapterOverrides: z.record(z.string().min(1), portabilityAdapterOverrideSchema).optional(),
});

export type SquadPortabilityImport = z.infer<typeof squadPortabilityImportSchema>;
