...
async function isLikelySlawRepoRoot(candidate: string): Promise<boolean> {
...
async function isLikelySlawRuntimeSkillPath(
  candidate: string,
  skillName: string,
  ...
  for (let depth = 0; depth < 6; depth += 1) {
    if (await isLikelySlawRepoRoot(cursor)) return true;
...
async function pruneBrokenUnavailableSlawSkillSymlinks(
  skillsHome: string,
  ...
  for (const entry of entries) {
    ...
    if (
      !(await isLikelySlawRuntimeSkillPath(resolvedLinkedPath, entry.name, {
    ...
    await onLog(
      "stdout",
      `[slaw] Removed stale Codex skill "${entry.name}" from ${skillsHome}\n`,
    );
  }
}
...
export async function ensureCodexSkillsInjected(
  ...
  const allSkillsEntries = options.skillsEntries ?? await readSlawRuntimeSkillEntries({}, __moduleDir);
    ...
    try {
      ...
      if (existing?.isSymbolicLink()) {
        ...
        if (
          ...
          (await isLikelySlawRuntimeSkillPath(resolvedLinkedPath, entry.runtimeName))
        ) {
          ...
          await onLog(
            "stdout",
            `[slaw] Repaired Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
          );
          continue;
        }
      }
      ...
      await onLog(
        "stdout",
        `[slaw] ${result === "repaired" ? "Repaired" : "Injected"} Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[slaw] Failed to inject Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }

  await pruneBrokenUnavailableSlawSkillSymlinks(
    skillsHome,
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const configuredCodexHome =
    typeof envConfig.CODEX_HOME === "string" && envConfig.CODEX_HOME.trim().length > 0
      ...
      : null;
  const codexSkillEntries = await readSlawRuntimeSkillEntries(config, __moduleDir);
  ...
  const preparedExecutionTargetRuntime = executionTargetIsRemote
    ? await (async () => {
        await onLog(
          "stdout",
          `[slaw] Syncing workspace and CODEX_HOME to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
        );
  ...
  const remoteCodexHome = executionTargetIsRemote
    ? preparedExecutionTargetRuntime?.assetDirs.home ??
      path.posix.join(effectiveExecutionCwd, ".slaw-runtime", "codex", "home")
    : null;
  ...
  if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[slaw] Codex session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
    );
  } else if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[slaw] Codex session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  ...
  const continuationSummary = parseObject(context.slawContinuationSummary);
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[slaw] Codex resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
