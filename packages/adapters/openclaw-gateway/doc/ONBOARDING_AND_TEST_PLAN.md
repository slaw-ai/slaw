...
## Scope
This plan is now **gateway-only**. Slaw supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Slaw skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Slaw task.
...
```bash
pnpm --filter @slaw/server test:run -- openclaw-gateway
pnpm --filter @slaw/server typecheck
pnpm --filter @slaw/ui typecheck
pnpm --filter slaw typecheck
```
