...
const DEFAULT_CLIENT_VERSION = "slaw";
...
export function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "slaw";
  if (input.strategy === "run") {
    return prefixSessionKeyForAgent(`slaw:run:${input.runId}`, input.agentId);
  }
  if (input.strategy === "issue" && input.issueId) {
    return prefixSessionKeyForAgent(`slaw:issue:${input.issueId}`, input.agentId);
  }
...
function resolveSlawApiUrlOverride(value: unknown): string | null {
...
const DEFAULT_CLAIMED_API_KEY_PATH = "~/.openclaw/workspace/slaw-claimed-api-key.json";

function resolveClaimedApiKeyPath(value: unknown): string {
  return nonEmpty(value) ?? DEFAULT_CLAIMED_API_KEY_PATH;
}

function buildSlawEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const slawApiUrlOverride = resolveSlawApiUrlOverride(ctx.config.slawApiUrl);
  const slawEnv: Record<string, string> = {
    ...buildSlawEnv(ctx.agent),
    SLAW_RUN_ID: ctx.runId,
  };

  if (slawApiUrlOverride) {
    slawEnv.SLAW_API_URL = slawApiUrlOverride;
  }
  if (wakePayload.taskId) slawEnv.SLAW_TASK_ID = wakePayload.taskId;
  const issueWorkMode = readSlawIssueWorkModeFromContext(ctx.context);
  if (issueWorkMode) slawEnv.SLAW_ISSUE_WORK_MODE = issueWorkMode;
  if (wakePayload.wakeReason) slawEnv.SLAW_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) slawEnv.SLAW_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) slawEnv.SLAW_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) slawEnv.SLAW_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    slawEnv.SLAW_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(
  payload: WakePayload,
  slawEnv: Record<string, string>,
  structuredWakePrompt: string,
): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/slaw-claimed-api-key.json";
  const orderedKeys = [
    "SLAW_RUN_ID",
    "SLAW_AGENT_ID",
    "SLAW_COMPANY_ID",
    ...
    "SLAW_TASK_ID",
    "SLAW_WAKE_REASON",
    "SLAW_WAKE_COMMENT_ID",
    "SLAW_APPROVAL_ID",
    "SLAW_APPROVAL_STATUS",
    "SLAW_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = slawEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = slawEnv.SLAW_API_URL ?? "<set SLAW_API_URL>";

  const lines = [
    "Slaw wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `SLAW_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load SLAW_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $SLAW_API_KEY on every API call.",
    "- Use X-Slaw-Run-Id: $SLAW_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: SLAW_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$SLAW_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\",\"in_review\"]}",
    ...
    "   - GET /api/companies/$SLAW_COMPANY_ID/issues?assigneeAgentId=$SLAW_AGENT_ID&status=todo,in_progress,in_review,blocked",
...
function buildStandardSlawPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  slawEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateSlaw = parseObject(payloadTemplate.slaw);
  const workspace = asRecord(ctx.context.slawWorkspace);
  const workspaces = Array.isArray(ctx.context.slawWorkspaces)
    ? ctx.context.slawWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.slawRuntimeServiceIntents)
    ? ctx.context.slawRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardSlaw: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: slawEnv.SLAW_API_URL ?? null,
  };
  const structuredWake = parseObject(ctx.context.slawWake);
  if (Object.keys(structuredWake).length > 0) {
    standardSlaw.wake = structuredWake;
  }

  if (workspace) {
    standardSlaw.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardSlaw.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardSlaw.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "slaw-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const slawEnv = buildSlawEnvForWake(ctx, wakePayload);
  const structuredWakePrompt = renderSlawWakePrompt(ctx.context.slawWake);
  const structuredWakeJson = stringifySlawWakePayload(ctx.context.slawWake);
  ...
  const slawPayload = buildStandardSlawPayload(ctx, wakePayload, slawEnv, payloadTemplate);
