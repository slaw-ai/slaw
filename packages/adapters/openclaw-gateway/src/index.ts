...
Use when:
- You want Slaw to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Slaw server.
...
- slawApiUrl (string, optional): absolute Slaw base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/slaw-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default slaw)

Standard outbound payload additions:
- slaw (object): standardized Slaw context added to every gateway agent request
- slaw.workspace (object, optional): resolved execution workspace for this run
- slaw.workspaces (array, optional): additional workspace hints Slaw exposed to the run
- slaw.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
