...
function renderSlawEnvNote(env: Record<string, string>): string {
  const slawKeys = Object.keys(env)
    .filter((key) => key.startsWith("SLAW_"))
    .sort();
  if (slawKeys.length === 0) return "";
  return [
    "Slaw runtime note:",
    `The following SLAW_* environment variables are available in this run: ${slawKeys.join(", ")}`,
...
function renderApiAccessNote(env: Record<string, string>): string {
  if (!hasNonEmptyEnvValue(env, "SLAW_API_URL") || !hasNonEmptyEnvValue(env, "SLAW_API_KEY")) return "";
  return [
    "Slaw API access note:",
    "Use run_shell_command with curl to make Slaw API requests.",
    "GET example:",
    `  run_shell_command({ command: "curl -s -H \\"Authorization: Bearer $SLAW_API_KEY\\" \\"$SLAW_API_URL/api/agents/me\\"" })`,
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer $SLAW_API_KEY\\" -H 'Content-Type: application/json' -H \\"X-Slaw-Run-Id: $SLAW_RUN_ID\\" -d '{...}' \\"$SLAW_API_URL/api/issues/{id}/checkout\\"" })`,
    "",
    "",
  ].join("\n");
}
...
/**
 * Inject Slaw skills directly into `~/.gemini/skills/` via symlinks.
...
async function ensureGeminiSkillsInjected(
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[slaw] Failed to prepare Gemini skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[slaw] Removed maintainer-only Gemini skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      const result = await ensureSlawSkillSymlink(entry.source, target);
      if (result === "skipped") continue;
      await onLog(
        "stderr",
        `[slaw] ${result === "repaired" ? "Repaired" : "Linked"} Gemini skill: ${entry.key}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[slaw] Failed to link Gemini skill "${entry.key}": ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function buildGeminiSkillsDir(
  ...
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "slaw-gemini-skills-"));
  const target = path.join(tmp, "skills");
  await fs.mkdir(target, { recursive: true });
  const availableEntries = await readSlawRuntimeSkillEntries(config, __moduleDir);
  const desiredNames = new Set(resolveSlawDesiredSkillNames(config, availableEntries));
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const geminiSkillEntries = await readSlawRuntimeSkillEntries(config, __moduleDir);
  const desiredGeminiSkillNames = resolveSlawDesiredSkillNames(config, geminiSkillEntries);
  ...
  if (executionTargetIsRemote) {
    try {
      ...
      await onLog(
        "stdout",
        `[slaw] Syncing workspace and Gemini runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
  ...
  if (executionTargetIsRemote && adapterExecutionTargetUsesSlawBridge(executionTarget)) {
  ...
  if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[slaw] Gemini session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
    );
  } else if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[slaw] Gemini session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  ...
  const slawEnvNote = renderSlawEnvNote(env);
  ...
  const promptMetrics = {
    ...
    runtimeNoteChars: slawEnvNote.length + apiAccessNote.length,
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[slaw] Gemini resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
