...
Use when:
- You want Slaw to run the native Grok Build CLI locally on the host machine
...
- instructionsFilePath (string, optional): absolute path to a markdown instructions file. Slaw stages it into the execution workspace as \`Agents.md\` when safe, otherwise falls back to \`--rules @file\`
...
- Slaw stages desired runtime skills into \`.claude/skills\` inside the execution workspace so Grok discovers them as project skills.
