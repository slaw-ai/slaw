...
const SLAW_MANAGED_CODEX_SKILLS_MANIFEST = ".slaw-managed-skills.json";
...
interface AcpxPreparedRuntime {
  ...
  slawClaudeSettings: SlawClaudeSettingsResult | null;
}
...
function defaultSlawInstanceDir(): string {
  const home = process.env.SLAW_HOME?.trim() || path.join(os.homedir(), ".slaw");
  const instanceId = process.env.SLAW_INSTANCE_ID?.trim() || "default";
  return resolveSlawInstanceRootForAdapter({
    homeDir: home,
    instanceId,
  });
}
...
async function prepareManagedCodexHome(input: {
  ...
  await onLog(
    "stdout",
    `[slaw] Using Slaw-managed ACPX Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
  );
  return targetHome;
}
...
async function buildSkillSetKey(input: {
  skills: SlawSkillEntry[];
  label: string;
}): Promise<string> {
  const hash = createHash("sha256");
  hash.update(`slaw-acpx-${input.label}-skills:v1\n`);
...
): Promise<{ allSkills: SlawSkillEntry[]; selectedSkills: SlawSkillEntry[]; desiredSkillNames: string[] }> {
  const allSkills = await readSlawRuntimeSkillEntries(config, __moduleDir);
  const desiredSkillNames = resolveSlawDesiredSkillNames(config, allSkills);
...
async function prepareClaudeSkillRuntime(input: {
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      const result = await materializeSlawSkillCopy(entry.source, target);
      if (result.skippedSymlinks.length > 0) {
        await input.onLog(
          "stdout",
          `[slaw] Materialized ACPX Claude skill "${entry.runtimeName}" into ${skillsHome} and skipped ${result.skippedSymlinks.length} symlink(s).\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[slaw] Failed to materialize ACPX Claude skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
  ...
  const promptInstructions = selectedSkills.length > 0
    ? [
        "Slaw has materialized selected runtime skills for this ACPX Claude session.",
  ...
  return {
    ...
    commandNotes: selectedSkills.length > 0
      ? [`Materialized ${selectedSkills.length} Slaw skill(s) for ACPX Claude at ${skillsHome}.`]
      : [],
  };
}

async function readManagedCodexSkillsManifest(skillsHome: string): Promise<Set<string>> {
  const manifestPath = path.join(skillsHome, SLAW_MANAGED_CODEX_SKILLS_MANIFEST);
  try {
...
async function writeManagedCodexSkillsManifest(skillsHome: string, skillNames: Iterable<string>): Promise<void> {
  ...
  await fs.writeFile(
    path.join(skillsHome, SLAW_MANAGED_CODEX_SKILLS_MANIFEST),
...
async function reconcileManagedCodexSkills(input: {
  skillsHome: string;
  allSkills: SlawSkillEntry[];
  selectedSkills: SlawSkillEntry[];
  onLog: AdapterExecutionContext["onLog"];
}): Promise<void> {
  ...
  for (const name of managed) {
    ...
    if (await removeSkillTarget(path.join(input.skillsHome, name))) {
      await input.onLog("stdout", `[slaw] Revoked ACPX Codex skill "${name}" from ${input.skillsHome}\n`);
    }
  }
  ...
  for (const entry of input.allSkills) {
    ...
    if (await removeSkillTarget(target)) {
      await input.onLog("stdout", `[slaw] Revoked legacy ACPX Codex skill "${entry.runtimeName}" from ${input.skillsHome}\n`);
    }
  }
  ...
  for (const name of managed) {
    ...
    if (await removeSkillTarget(path.join(input.skillsHome, name))) {
      await input.onLog("stdout", `[slaw] Revoked unavailable ACPX Codex skill "${name}" from ${input.skillsHome}\n`);
    }
  }
}
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      ...
      if (result.skippedSymlinks.length > 0) {
        await input.onLog(
          "stdout",
          `[slaw] Materialized ACPX Codex skill "${entry.runtimeName}" into ${skillsHome} and skipped ${result.skippedSymlinks.length} symlink(s).\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[slaw] Failed to inject ACPX Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
...
interface SlawClaudeSettingsResult {
  filePath: string;
  allow: string[];
...
// widen the SDK's Read sandbox to include the Slaw state dirs the agent
// needs to talk to its own control plane.
async function writeSlawClaudeSettings(input: {
  cwd: string;
  stateDir: string;
  agentHome: string;
  squadId: string;
  ...
  const slawAllow = uniqueSorted([
    ...
    "Bash(env:*)",
    "Bash(env)",
    `Bash(${input.cwd}/scripts/slaw-issue-update.sh:*)`,
    `Bash(${input.cwd}/scripts/slaw:*)`,
  ]);
  ...
  const existingAdditionalDirectories = Array.isArray(existingPerms.additionalDirectories)
    ...
    : [];
  const mergedAllow = uniqueSorted([...existingAllow, ...slawAllow]);
  const mergedAdditionalDirectories = uniqueSorted([
    ...existingAdditionalDirectories,
    ...slawAdditionalDirectories,
  ]);
...
async function writeAgentWrapper(input: {
  ...
  const script = [
    ...
    "if [[ -n \"${SLAW_RUN_ID:-}\" ]]; then",
    "  mkdir -p \"$stderr_dir\"",
    "  exec 2> >(tee -a \"$stderr_dir/$SLAW_RUN_ID.log\" >&2)",
    "fi",
...
async function buildRuntime(input: {
  ...
  const env: Record<string, string> = { ...buildSlawEnv(agent), SLAW_RUN_ID: runId };
  const wakeTaskId =
  ...
  if (wakeTaskId) env.SLAW_TASK_ID = wakeTaskId;
  if (issueWorkMode) env.SLAW_ISSUE_WORK_MODE = issueWorkMode;
  if (wakeReason) env.SLAW_WAKE_REASON = wakeReason;
  if (wakeCommentId) env.SLAW_WAKE_COMMENT_ID = wakeCommentId;
  if (approvalId) env.SLAW_APPROVAL_ID = approvalId;
  if (approvalStatus) env.SLAW_APPROVAL_STATUS = approvalStatus;
  if (linkedIssueIds.length > 0) env.SLAW_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
  if (wakePayloadJson) env.SLAW_WAKE_PAYLOAD_JSON = wakePayloadJson;
  ...
  if (!hasExplicitApiKey && authToken) env.SLAW_API_KEY = authToken;
  ...
  let slawClaudeSettings: SlawClaudeSettingsResult | null = null;
  if (acpxAgent === "claude") {
    ...
    slawClaudeSettings = await writeSlawClaudeSettings({
      cwd,
      stateDir,
      agentHome,
    ...
    skillCommandNotes.push(
      `Wrote Slaw-managed Claude settings to ${slawClaudeSettings.filePath} (defaultMode=${slawClaudeSettings.defaultMode}${
        slawClaudeSettings.overrodeDontAsk ? "; overrode user dontAsk" : ""
      }, +${slawClaudeSettings.additionalDirectories.length} read root(s), +${slawClaudeSettings.allow.length} allow rule(s)).`,
    );
    ...
    const desired = resolveSlawDesiredSkillNames(config, await readSlawRuntimeSkillEntries(config, __moduleDir));
    skillsIdentity = { mode: "custom_unsupported", desiredSkillNames: desired };
    if (desired.length > 0) {
      skillCommandNotes.push("Selected Slaw skills are tracked only; ACPX custom commands do not expose a runtime skill contract yet.");
    }
  }
  ...
  const fingerprint = shortHash({
    ...
    slawClaudeSettings: slawClaudeSettings
      ? {
          allow: slawClaudeSettings.allow,
          additionalDirectories: slawClaudeSettings.additionalDirectories,
          defaultMode: slawClaudeSettings.defaultMode,
        }
      : null,
  });
...
async function applySessionConfigOptions(input: {
  ...
  if (!input.runtime.setConfigOption) {
    ...
    await input.onLog("stderr", `[slaw] ${message}\n`);
  ...
  for (const option of options) {
    ...
    await input.onLog(
      "stdout",
      `[slaw] Applied ACPX ${input.prepared.acpxAgent} config ${option.key}=${option.value}\n`,
    );
  }
}
...
async function buildPrompt(ctx: AdapterExecutionContext, resumedSession: boolean): Promise<{
  ...
  const promptTemplate = asString(config.promptTemplate, DEFAULT_SLAW_AGENT_PROMPT_TEMPLATE);
  ...
  const renderedBootstrapPrompt =
    !resumedSession && bootstrapPromptTemplate.trim().length > 0
      ...
      : "";
  const wakePrompt = renderSlawWakePrompt(context.slawWake, { resumedSession });
...
async function emitAcpxFailure(input: {
  ...
  if (childStderrTail) {
    await ctx.onLog(
      "stderr",
      `[slaw] ACPX child stderr tail (${phase}):\n${childStderrTail}\n`,
    );
  }
  await emitAcpxLog(ctx, {
    type: "acpx.error",
...
export function createAcpxLocalExecutor(deps: ExecuteDeps = {}) {
  ...
  return async function executeAcpxLocal(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
    ...
    if (!canResume && asString(previousParams.runtimeSessionName, "")) {
      await ctx.onLog(
        "stdout",
        `[slaw] ACPX session "${asString(previousParams.runtimeSessionName, "")}" does not match the current agent/cwd/mode/runtime identity; starting fresh in "${prepared.cwd}".\n`,
      );
    }
    ...
    try {
      if (!handle) {
        try {
          ...
          await ctx.onLog(
            "stdout",
            `[slaw] ACPX resume session "${resumeSessionId}" is unavailable; retrying with a fresh session.\n`,
          );
          handle = await runtime.ensureSession({
            sessionKey: prepared.sessionKey,
            agent: prepared.acpxAgent,
    ...
    if (ctx.onMeta) {
      await ctx.onMeta({
        ...
        cwd: prepared.cwd,
        commandNotes: [
          `ACPX runtime embedded in Slaw with ${prepared.mode} session mode.`,
    ...
    try {
      ...
      if (terminal.status === "failed" || terminal.status === "cancelled" || timedOut) {
        ...
        if (warmHandleMatches(existing, runtime, sessionHandle) && existing) {
          await closeWarmHandle({
            ...
            entry: existing,
            reason: timedOut ? "slaw timeout cleanup" : `slaw turn ${terminal.status}`,
        ...
        if (existing && !warmHandleMatches(existing, runtime, sessionHandle)) {
          await runtime.close({
            handle: sessionHandle,
            reason: "slaw duplicate warm handle cleanup",
            discardPersistentState: false,
          }).catch(() => {});
        } else {
      ...
      await runtime.close({
        handle: sessionHandle,
        reason: timedOut ? "slaw timeout cleanup" : "slaw error cleanup",
