...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Slaw data dir to use
  SLAW_HOST=127.0.0.1    Host passed to the onboarded server
  SLAW_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/slaw-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/slaw-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export SLAW_HOST="${SLAW_HOST:-127.0.0.1}"
export SLAW_PORT="${SLAW_PORT:-3100}"
export SLAW_OPEN_ON_LISTEN="${SLAW_OPEN_ON_LISTEN:-false}"
...
echo "SLAW_HOST: $SLAW_HOST"
echo "SLAW_PORT: $SLAW_PORT"
...
env \
  ...
  HOST="$SLAW_HOST" \
  PORT="$SLAW_PORT" \
  SLAW_OPEN_ON_LISTEN="$SLAW_OPEN_ON_LISTEN" \
  pnpm slaw onboard --yes --data-dir "$PC_DATA"
