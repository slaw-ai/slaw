#!/usr/bin/env node
/**
 * Screenshot utility for Slaw UI.
 *
 * Reads the board token from ~/.slaw/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/slaw-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".slaw/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.slaw/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the Slaw origin only
