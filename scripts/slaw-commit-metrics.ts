...
function printHelp() {
  console.log(`Usage: tsx scripts/slaw-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === SLAW_EMAIL) || (displayName && displayName.toLowerCase() === SLAW_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Slaw commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "slaw-commit-metrics",
