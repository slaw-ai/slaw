...
COMPANY_ID="${COMPANY_ID:-${SLAW_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-slaw-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-slaw-openclaw-smoke}"
...
Provide one of:
  SLAW_AUTH_HEADER=\"Bearer <board-token>\"
  SLAW_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Slaw OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.slaw.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
