...
OPENCLAW_USER="${OPENCLAW_USER:-slaw-smoke}"

SLAW_RUN_ID="${SLAW_RUN_ID:-smoke-run-$(date +%s)}"
SLAW_AGENT_ID="${SLAW_AGENT_ID:-openclaw-smoke-agent}"
SLAW_COMPANY_ID="${SLAW_COMPANY_ID:-openclaw-smoke-company}"
SLAW_API_URL="${SLAW_API_URL:-http://localhost:3100}"
SLAW_TASK_ID="${SLAW_TASK_ID:-openclaw-smoke-task}"
SLAW_WAKE_REASON="${SLAW_WAKE_REASON:-openclaw_smoke_test}"
SLAW_WAKE_COMMENT_ID="${SLAW_WAKE_COMMENT_ID:-}"
SLAW_APPROVAL_ID="${SLAW_APPROVAL_ID:-}"
SLAW_APPROVAL_STATUS="${SLAW_APPROVAL_STATUS:-}"
SLAW_LINKED_ISSUE_IDS="${SLAW_LINKED_ISSUE_IDS:-}"
...
SLAW_RUN_ID=${SLAW_RUN_ID}
SLAW_AGENT_ID=${SLAW_AGENT_ID}
SLAW_COMPANY_ID=${SLAW_COMPANY_ID}
SLAW_API_URL=${SLAW_API_URL}
SLAW_TASK_ID=${SLAW_TASK_ID}
SLAW_WAKE_REASON=${SLAW_WAKE_REASON}
SLAW_WAKE_COMMENT_ID=${SLAW_WAKE_COMMENT_ID}
SLAW_APPROVAL_ID=${SLAW_APPROVAL_ID}
SLAW_APPROVAL_STATUS=${SLAW_APPROVAL_STATUS}
SLAW_LINKED_ISSUE_IDS=${SLAW_LINKED_ISSUE_IDS}

Run your Slaw heartbeat procedure now.
EOF
...
PAYLOAD="$(jq -nc \
  ...
  --arg runId "$SLAW_RUN_ID" \
  --arg agentId "$SLAW_AGENT_ID" \
  --arg companyId "$SLAW_COMPANY_ID" \
  --arg apiUrl "$SLAW_API_URL" \
  --arg taskId "$SLAW_TASK_ID" \
  --arg wakeReason "$SLAW_WAKE_REASON" \
  --arg wakeCommentId "$SLAW_WAKE_COMMENT_ID" \
  --arg approvalId "$SLAW_APPROVAL_ID" \
  --arg approvalStatus "$SLAW_APPROVAL_STATUS" \
  ...
  '{
    ...
    metadata: {
      SLAW_RUN_ID: $runId,
      SLAW_AGENT_ID: $agentId,
      SLAW_COMPANY_ID: $companyId,
      SLAW_API_URL: $apiUrl,
      SLAW_TASK_ID: $taskId,
      SLAW_WAKE_REASON: $wakeReason,
      SLAW_WAKE_COMMENT_ID: $wakeCommentId,
      SLAW_APPROVAL_ID: $approvalId,
      SLAW_APPROVAL_STATUS: $approvalStatus,
      SLAW_LINKED_ISSUE_IDS: $linkedIssueIds,
      slaw_session_key: ("slaw:run:" + $runId)
    }
  }')"
...
args=(
  ...
  -H "x-openclaw-session-key: slaw:run:${SLAW_RUN_ID}"
  -D "$headers_file"
  -o "$body_file"
  --data "$PAYLOAD"
  "$OPENCLAW_URL"
