...
SLAW_HOST_PORT="${SLAW_HOST_PORT:-3100}"
SLAW_HOST_FROM_CONTAINER="${SLAW_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.slaw-openclaw.override.yml"
...
detect_slaw_base_url() {
  ...
  for candidate in "$SLAW_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${SLAW_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${SLAW_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
slaw_base_url="$(detect_slaw_base_url || true)"
...
Slaw URL for OpenClaw container:
EOF
  if [[ -n "$slaw_base_url" ]]; then
    cat <<EOF
  $slaw_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${SLAW_HOST_PORT}
  (Do not use http://127.0.0.1:${SLAW_HOST_PORT} inside the container.)
  If Slaw rejects the host, run on host machine:
    pnpm slaw allowed-hostname host.docker.internal
  Then restart Slaw and re-run this script.
EOF
  fi
  cat <<EOF
