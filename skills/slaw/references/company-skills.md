...
- App-shipped catalog: a curated set of company skills in `@slaw/skills-catalog`, browseable and installable without leaving Slaw.
...
curl -sS -X POST "$SLAW_API_URL/api/companies/$SLAW_COMPANY_ID/skills/install-catalog" \
  -H "Authorization: Bearer $SLAW_API_KEY" \
...
```sh
curl -sS -X POST "$SLAW_API_URL/api/companies/$SLAW_COMPANY_ID/skills/import" \
  -H "Authorization: Bearer $SLAW_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```sh
curl -sS -X POST "$SLAW_API_URL/api/companies/$SLAW_COMPANY_ID/skills/scan-projects" \
  -H "Authorization: Bearer $SLAW_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```sh
curl -sS "$SLAW_API_URL/api/companies/$SLAW_COMPANY_ID/skills" \
  -H "Authorization: Bearer $SLAW_API_KEY"
```
...
```sh
curl -sS "$SLAW_API_URL/api/companies/$SLAW_COMPANY_ID/skills/<skill-id>" \
  -H "Authorization: Bearer $SLAW_API_KEY"

curl -sS "$SLAW_API_URL/api/companies/$SLAW_COMPANY_ID/skills/<skill-id>/files?path=SKILL.md" \
  -H "Authorization: Bearer $SLAW_API_KEY"
```
...
```sh
curl -sS -X POST "$SLAW_API_URL/api/companies/$SLAW_COMPANY_ID/agents" \
  -H "Authorization: Bearer $SLAW_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "QA Browser Agent",
