...
usage() {
  cat <<'EOF'
Usage:
  slaw-upload-artifact.sh FILE [options]

Uploads a generated file from the current workspace to the current Slaw
...
Examples:
  scripts/slaw-upload-artifact.sh dist/demo.mp4 \
    --title "Demo video render" \
    --summary "MP4 render for board review"

  scripts/slaw-upload-artifact.sh out/walkthrough.webm \
