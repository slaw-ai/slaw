...
required: false
description: >
  Develop and operate a local Slaw instance — start and stop servers,
  ...
  Slaw codebase itself or keep a running instance healthy.
---

# Slaw Dev

This skill covers the day-to-day workflows for developing and operating a local Slaw instance. It assumes you are working inside the Slaw repo checkout with `origin` pointing to `git@github.com:slaw/slaw.git`.
...
> **MANDATORY:** Before running any CLI command, building, testing, or managing worktrees, you MUST read `doc/DEVELOPING.md` in the Slaw repo. It is the canonical reference for all `slaw` CLI commands, their options, build/test workflows, database operations, worktree management, and diagnostics. Do NOT guess at flags or options — read the doc first.
...
| Task | Command |
|------|---------|
| Start server (first time or normal) | `npx slaw run` |
...
| Back up database | `npx slaw db:backup` |
...
Slaw worktrees combine git worktrees with isolated Slaw instances — each gets its own database, server port, and environment seeded from the primary instance.
...
- Starting a feature branch that needs its own Slaw environment
- Running parallel agent work without cross-contaminating the primary instance
- Testing Slaw changes in isolation before merging
...
| `worktree:list` | List worktrees and their Slaw status |
...
npx slaw worktree:make my-feature --start-point origin/main

# 2. Move into the worktree (path printed by worktree:make) and source the environment
cd <worktree-path>
eval "$(npx slaw worktree env)"

# 3. Start the isolated Slaw server
npx slaw run

# 4. Do your work

# 5. When done, merge history back if needed
npx slaw worktree:merge-history --from slaw-my-feature --to current --apply

# 6. Clean up
npx slaw worktree:cleanup my-feature
```

## Forks — Prefer Pushing to a User Fork

If the user has a personal fork of `slaw/slaw` configured as a git remote, push your feature branches to **that fork** instead of creating branches on the main repo. This keeps the upstream branch list clean and matches the standard open-source contribution flow.

### Detect a fork remote

Before pushing or creating a PR, list remotes and check for one that points at a non-`slaw` GitHub fork:

```bash
git remote -v
```

Treat any remote whose URL points to `github.com:<user>/slaw` (or `github.com/<user>/slaw.git`) as the user's fork. Common names are `fork`, `<username>`, or `myfork`. The remote named `origin` or `upstream` that points at `slaw/slaw` is the canonical upstream — do not push feature branches there if a fork exists.
...
```bash
gh pr create --repo slaw/slaw --head <fork-owner>:<branch-name> ...
```
...
If `git remote -v` shows only `slaw/slaw` remotes (no user fork), fall back to pushing branches to `origin` as before. Do NOT create a fork on the user's behalf — ask first.

### Keeping the fork up to date

The canonical remote that points at `slaw/slaw` may be named `origin` **or** `upstream` depending on how the user set up the repo. Detect it the same way as in the "Detect a fork remote" step, then fetch and push from/with that remote so the sync works under either convention:

```bash
UPSTREAM_REMOTE=$(git remote -v | awk '/slaw\/slaw.*\(fetch\)/{print $1; exit}')
...
1. **CLI is the only interface to worktrees and databases.** All worktree and database operations MUST go through `npx slaw` / `pnpm slaw` commands. You MUST NOT:
   ...
   - Run `rm -rf` on any `.slaw/`, `.slaw-worktrees/`, or `db/` directory
   - Directly manipulate embedded postgres data directories
   - Kill postgres processes by PID

2. **If a CLI command fails, stop and report.** Do NOT attempt workarounds. If `worktree:make`, `worktree reseed`, `worktree init`, `worktree:cleanup`, or any other `slaw` command fails:
   - Report the exact error message in your task comment
   - Set the task to `blocked`
   - Suggest running `npx slaw doctor --repair` or recreating the worktree from scratch
...
4. **Starting a dev server in a worktree requires setup first.** The correct sequence is:
   ...
   eval "$(npx slaw worktree env)"
   pnpm install && pnpm build
   npx slaw run          # or pnpm dev

   # If the worktree needs a fresh database:
   npx slaw worktree reseed --seed-mode full

   # If the worktree is broken beyond repair:
   npx slaw worktree:cleanup <name>
   npx slaw worktree:make <name> --seed-mode full
   ```
...
cd <worktree-path>
eval "$(npx slaw worktree env)"   # skip if using the primary instance
...
| Mistake | Fix |
|---------|-----|
| Server won't start | Run `npx slaw doctor --repair` to diagnose and auto-fix |
| Forgetting to source worktree env | Run `eval "$(npx slaw worktree env)"` after cd-ing into the worktree |
...
| Running agents against wrong instance | Verify `SLAW_API_URL` points to the correct port |
...
| Pushed feature branch to `slaw/slaw` when a fork exists | Push to the user's fork remote instead — see "Forks" above |
