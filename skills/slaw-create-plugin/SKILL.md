---
name: slaw-create-plugin
description: >
  Create and develop external Slaw plugins with the CLI-first workflow.
  Use when scaffolding a new plugin, working on a local plugin against a running
  Slaw instance, or updating plugin authoring docs. Covers `slaw
  plugin init`, the local install loop via `slaw plugin install <path>`,
  worker/UI rebuild and reload semantics, and the required success checklist.
---

# Create and develop a Slaw plugin

Use this skill when the task is to create, scaffold, or iterate on a Slaw plugin against a local Slaw instance.

## 1. Default: build the plugin OUTSIDE Slaw core
...
- Scaffold the plugin into a directory outside the Slaw checkout (e.g. `~/dev/slaw-plugins/<name>`).
- Install it into the running Slaw instance by local absolute path.
- Edit code in the external package; let Slaw pick up rebuilt output.

Only edit Slaw core itself when the user asks to surface a plugin as a bundled example (`server/src/routes/plugins.ts`, in-repo example lists, docs).
...
Use `slaw plugin init`. Do not invoke the scaffold package node entrypoint by hand unless the CLI command is unavailable in the environment.

```bash
slaw plugin init @acme/my-plugin --output ~/dev/slaw-plugins
```
...
- `--sdk-path <path>` — snapshot the local SDK from a Slaw checkout into `.slaw-sdk/` (useful when developing against an unreleased SDK).

On success the command prints the exact next commands (`cd`, `pnpm install`, `pnpm dev`, `slaw plugin install <abs-path>`). Run them in order.
...
slaw plugin install /absolute/path/to/my-plugin
```

Notes:

- `slaw plugin install` auto-detects local paths (absolute, `./`, `../`, `~`, or an existing relative folder) and forwards `isLocalPath: true` to the server. Pass `--local` to force local mode if the heuristic is ambiguous.
...
```bash
slaw plugin list
slaw plugin inspect <plugin-key>
```
...
- `package.json` — `slawPlugin` block points at `dist/manifest.js`, `dist/worker.js`, `dist/ui/`
...
If you changed Slaw SDK/host/plugin runtime code in addition to the plugin, also run the relevant Slaw workspace checks.
...
- **Commands run** — the exact `slaw plugin init`, `pnpm install`, `pnpm dev`, `slaw plugin install <path>` invocations (and any verification commands).
- **Install status** — output of `slaw plugin list` / `plugin inspect` (plugin key, version, status). Note if `status` is anything other than `ready` and include `lastError`.
