...
description: >
  ...
  with the `slaw` skill, which covers the mechanics of writing the plan
  document and reassigning the issue.
---

# Slaw — Converting Plans to Tasks

A companion skill for turning a plan into executable Slaw work. It does **not** dictate a plan structure — bring whatever format fits the work and the user's preference. It tells you _how_ to translate that plan into issues so that the rest of Slaw works for you.

For the **mechanics** of recording a plan (issue document with key `plan`, comment links, approval gating, who to reassign back to), follow the _Planning_ section of the `slaw` skill. This skill covers planning method, not the API surface.
...
- Not a replacement for the `slaw` skill's planning mechanics. Use both.
