...
function ensureSquadPrefix(squadId: string, objectKey: string): void {
  const expectedPrefix = `${squadId}/`;
  if (!objectKey.startsWith(expectedPrefix)) {
    throw forbidden("Object does not belong to squad");
  }
...
function buildObjectKey(squadId: string, namespace: string, originalFilename: string | null): string {
  ...
  return `${squadId}/${ns}/${year}/${month}/${day}/${filename}`;
}

function assertPutFileInput(input: PutFileInput): void {
  if (!input.squadId || input.squadId.trim().length === 0) {
    throw unprocessable("squadId is required");
  }
...
export function createStorageService(provider: StorageProvider): StorageService {
  return {
    ...
    async putFile(input: PutFileInput): Promise<PutFileResult> {
      ...
      const objectKey = buildObjectKey(input.squadId, input.namespace, input.originalFilename);
    ...
    async getObject(squadId: string, objectKey: string, options) {
      ensureSquadPrefix(squadId, objectKey);
      return provider.getObject({ objectKey, range: options?.range });
    },

    async headObject(squadId: string, objectKey: string) {
      ensureSquadPrefix(squadId, objectKey);
      return provider.headObject({ objectKey });
    },

    async deleteObject(squadId: string, objectKey: string) {
      ensureSquadPrefix(squadId, objectKey);
