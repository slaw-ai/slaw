...
import { getInstanceClaimWarningUrl, initializeInstanceClaimChallenge } from "./instance-claim.js";
...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.SLAW_SECRETS_PROVIDER === undefined) {
    process.env.SLAW_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.SLAW_SECRETS_STRICT_MODE === undefined) {
    process.env.SLAW_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.SLAW_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.SLAW_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.SLAW_MIGRATION_AUTO_APPLY === "true") return true;
    if (process.env.SLAW_MIGRATION_PROMPT === "never") return false;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set SLAW_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_OPERATOR_USER_EMAIL = "local@slaw.local";
      ...
      const membership = await db
        ...
        .where(
          and(
            ...
            eq(squadMemberships.principalId, LOCAL_OPERATOR_USER_ID),
          ),
        )
        .then((rows: Array<{ id: string }>) => rows[0] ?? null);
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.SLAW_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://slaw:slaw@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "slaw");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://slaw:slaw@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "slaw");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: slaw");
    }
  
    const embeddedConnectionString = `postgres://slaw:slaw@127.0.0.1:${port}/slaw`;
  ...
  if (config.deploymentMode === "authenticated") {
    ...
    await initializeInstanceClaimChallenge(db as any, { deploymentMode: config.deploymentMode });
    authReady = true;
  }
  ...
  const configuredApiUrl = process.env.SLAW_API_URL?.trim() || runtimeApiUrl;
  ...
  process.env.SLAW_LISTEN_HOST = runtimeListenHost;
  process.env.SLAW_LISTEN_PORT = String(listenPort);
  process.env.SLAW_RUNTIME_API_URL = runtimeApiUrl;
  process.env.SLAW_RUNTIME_API_CANDIDATES_JSON = JSON.stringify(runtimeApiCandidates);
  process.env.SLAW_API_URL = configuredApiUrl;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      const instanceClaimUrl = getInstanceClaimWarningUrl(config.host, listenPort);
      if (instanceClaimUrl) {
  ...
  
  {
    const shutdown = async (signal: "SIGINT" | "SIGTERM") => {
      const appShutdown = (app as { locals?: { slawShutdown?: () => void } }).locals?.slawShutdown;
      appShutdown?.();
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "Slaw server failed to start");
    process.exit(1);
  });
}
