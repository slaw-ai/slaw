...
function jwtConfig() {
  const secret = process.env.SLAW_AGENT_JWT_SECRET?.trim() || process.env.BETTER_AUTH_SECRET?.trim();
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.SLAW_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.SLAW_AGENT_JWT_ISSUER ?? "slaw",
    audience: process.env.SLAW_AGENT_JWT_AUDIENCE ?? "slaw-api",
  };
}
...
export function createLocalAgentJwt(agentId: string, squadId: string, adapterType: string, runId: string) {
...
export function verifyLocalAgentJwt(token: string): LocalAgentJwtClaims | null {
  ...
  const squadId = typeof claims.squad_id === "string" ? claims.squad_id : null;
  ...
  if (!sub || !squadId || !adapterType || !runId || !iat || !exp) return null;
