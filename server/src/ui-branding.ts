const FAVICON_BLOCK_START = "<!-- SLAW_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- SLAW_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- SLAW_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- SLAW_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.SLAW_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.SLAW_WORKTREE_NAME) ?? nonEmpty(env.SLAW_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.SLAW_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="slaw-worktree-enabled" content="true" />',
    `<meta name="slaw-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="slaw-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="slaw-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
