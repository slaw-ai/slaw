...
import { agentApiKeys, squadMemberships, instanceUserRoles } from "@slaw-ai/db";
...
interface IncomingMessageWithContext extends IncomingMessage {
  slawUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).slawUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, squadId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.slawUpgradeContext = context;
