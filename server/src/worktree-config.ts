...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Slaw worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.SLAW_IN_WORKTREE !== "true") return null;

  const configPath = resolveSlawConfigPath(overrideConfigPath);
  const envPath = resolveSlawEnvPath(configPath);
  const persistedEnv = readEnvEntries(envPath);
  const persistedConfigPath = nonEmpty(persistedEnv.SLAW_CONFIG);
  ...
  const worktreeName =
    nonEmpty(stablePersistedEnv.SLAW_WORKTREE_NAME) ??
    nonEmpty(env.SLAW_WORKTREE_NAME) ??
    path.basename(worktreeRoot);
  const instanceId =
    nonEmpty(stablePersistedEnv.SLAW_INSTANCE_ID) ??
    nonEmpty(env.SLAW_INSTANCE_ID) ??
    sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(stablePersistedEnv.SLAW_HOME) ??
      nonEmpty(env.SLAW_HOME) ??
      nonEmpty(env.SLAW_WORKTREES_DIR) ??
      "~/.slaw-worktrees",
  );
...
function writeConfigFile(configPath: string, config: SlawConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".slaw", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as SlawConfig;
...
): { config: SlawConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.SLAW_HOME = context.homeDir;
  process.env.SLAW_INSTANCE_ID = context.instanceId;
  process.env.SLAW_CONFIG = context.configPath;
  process.env.SLAW_CONTEXT = context.contextPath;
  process.env.SLAW_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as SlawConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    SLAW_HOME: context.homeDir,
    SLAW_INSTANCE_ID: context.instanceId,
    SLAW_CONFIG: context.configPath,
    SLAW_CONTEXT: context.contextPath,
    SLAW_IN_WORKTREE: "true",
    SLAW_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: SlawConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as SlawConfig;
  } catch {
    return;
  }
