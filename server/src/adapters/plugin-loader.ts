...
function extractUiParserSource(
  ...
  const contractVersion = pkg.slaw?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no slaw.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
