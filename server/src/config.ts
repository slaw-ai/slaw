...
import { resolveSlawEnvPath } from "./paths.js";
...
const SLAW_ENV_FILE_PATH = resolveSlawEnvPath();
if (existsSync(SLAW_ENV_FILE_PATH)) {
  loadDotenv({ path: SLAW_ENV_FILE_PATH, override: false, quiet: true });
}

const CWD_ENV_PATH = resolve(process.cwd(), ".env");
const isSameFile = existsSync(CWD_ENV_PATH) && existsSync(SLAW_ENV_FILE_PATH)
  ? realpathSync(CWD_ENV_PATH) === realpathSync(SLAW_ENV_FILE_PATH)
  : CWD_ENV_PATH === SLAW_ENV_FILE_PATH;
...
function detectTailnetBindHost(): string | undefined {
  const explicit = process.env.SLAW_TAILNET_BIND_HOST?.trim();
...
export function loadConfig(): Config {
  ...
  const providerFromEnvRaw = process.env.SLAW_SECRETS_PROVIDER;
  ...
  const storageProviderFromEnvRaw = process.env.SLAW_STORAGE_PROVIDER;
  ...
  const storageS3Bucket = process.env.SLAW_STORAGE_S3_BUCKET ?? fileStorage?.s3?.bucket ?? "slaw";
  const storageS3Region = process.env.SLAW_STORAGE_S3_REGION ?? fileStorage?.s3?.region ?? "us-east-1";
  const storageS3Endpoint = process.env.SLAW_STORAGE_S3_ENDPOINT ?? fileStorage?.s3?.endpoint ?? undefined;
  const storageS3Prefix = process.env.SLAW_STORAGE_S3_PREFIX ?? fileStorage?.s3?.prefix ?? "";
  ...
  const strictModeFromEnv = process.env.SLAW_SECRETS_STRICT_MODE;
  const secretsStrictMode =
    strictModeFromEnv !== undefined
      ? strictModeFromEnv === "true"
      : (fileSecrets?.strictMode ?? deploymentMode === "authenticated");
  const deploymentExposureFromEnvRaw = process.env.SLAW_DEPLOYMENT_EXPOSURE;
  ...
  const bindFromEnvRaw = process.env.SLAW_BIND;
  const bindFromEnv =
  ...
  const customBindHost = process.env.SLAW_BIND_HOST ?? fileConfig?.server.customBindHost;
  const authBaseUrlModeFromEnvRaw = process.env.SLAW_AUTH_BASE_URL_MODE;
  const authBaseUrlModeFromEnv =
    authBaseUrlModeFromEnvRaw &&
    AUTH_BASE_URL_MODES.includes(authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      ? (authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      : null;
  const publicUrlFromEnv = process.env.SLAW_PUBLIC_URL;
  ...
  const disableSignUpFromEnv = process.env.SLAW_AUTH_DISABLE_SIGN_UP;
  const authDisableSignUp: boolean =
    disableSignUpFromEnv !== undefined
      ? disableSignUpFromEnv === "true"
      : (fileConfig?.auth?.disableSignUp ?? false);
  const allowedHostnamesFromEnvRaw = process.env.SLAW_ALLOWED_HOSTNAMES;
  ...
  const squadDeletionEnvRaw = process.env.SLAW_ENABLE_SQUAD_DELETION;
  const squadDeletionEnabled =
    squadDeletionEnvRaw !== undefined
      ? squadDeletionEnvRaw === "true"
      : deploymentMode === "local_trusted";
  const databaseBackupEnabled =
    process.env.SLAW_DB_BACKUP_ENABLED !== undefined
      ? process.env.SLAW_DB_BACKUP_ENABLED === "true"
      : (fileDatabaseBackup?.enabled ?? true);
  const databaseBackupIntervalMinutes = Math.max(
    1,
    Number(process.env.SLAW_DB_BACKUP_INTERVAL_MINUTES) ||
      fileDatabaseBackup?.intervalMinutes ||
      60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    Number(process.env.SLAW_DB_BACKUP_RETENTION_DAYS) ||
      fileDatabaseBackup?.retentionDays ||
      7,
  );
  const databaseBackupDir = resolveHomeAwarePath(
    process.env.SLAW_DB_BACKUP_DIR ??
  ...
  return {
    ...
    uiDevMiddleware: process.env.SLAW_UI_DEV_MIDDLEWARE === "true",
    secretsProvider,
    secretsStrictMode,
