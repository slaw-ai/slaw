 ...
 *    (`~/.slaw/plugins/`) and `node_modules` for packages matching
 *    the `slaw-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Slaw plugins.
 * Packages matching this pattern are considered Slaw plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "slaw-plugin-";
...
export function buildPluginWorkerEnv(input: {
  manifest: Pick<SlawPluginManifestV1, "capabilities">;
  instanceInfo: { deploymentMode?: string | null; deploymentExposure?: string | null };
  processEnv?: NodeJS.ProcessEnv;
}): Record<string, string> {
  const processEnv = input.processEnv ?? process.env;
  const env: Record<string, string> = {
    SLAW_DEPLOYMENT_MODE: input.instanceInfo.deploymentMode ?? "",
    SLAW_DEPLOYMENT_EXPOSURE: input.instanceInfo.deploymentExposure ?? "",
  };
...
export type PluginSource =
  | "local-filesystem"  // ~/.slaw/plugins/ local directory
  | "npm"               // npm packages matching slaw-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: SlawPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the slaw-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "slaw-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: SlawPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Slaw plugins installed as npm packages in the current
   * Node.js environment matching the "slaw-plugin-*" naming convention.
   *
   ...
   * the "slawPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Slaw plugin.
   * Throws if the package is a Slaw plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<SlawPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: SlawPluginManifestV1;
    newManifest: SlawPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Slaw plugin naming convention.
 * Accepts both the "slaw-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @slaw-ai/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "slawPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  if (
    ...
    const manifestRelPath = (slawPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: SlawPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumSlawVersion;
}
 ...
 *   packageName: "slaw-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: SlawPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as SlawPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Slaw plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPackageRoot(
    packageRoot: string,
  ): Promise<SlawPluginManifestV1 | null> {
  ...
  async function refreshPluginManifestFromPackage(
    ...
    if (!manifest) {
      throw new Error(`Plugin package ${plugin.packageName} no longer exposes a Slaw manifest`);
    }
   ...
   * if the package is not a Slaw plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasSlawPlugin = "slawPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasSlawPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<SlawPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: SlawPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @slaw-ai/shared exports). Run those workers through
