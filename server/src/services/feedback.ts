...
import { claudeConfigDir, parseClaudeStreamJson } from "@slaw/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@slaw/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@slaw/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveSlawInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "slaw-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "slaw-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "slaw-feedback-v1";
const FEEDBACK_DESTINATION = "slaw_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "slaw_run" &&
    file.source !== "slaw_run_events" &&
    file.source !== "slaw_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.SLAW_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readSlawSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let slawRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.squadId !== row.squadId) {
      ...
      files.push(makeBundleFile({
        ...
        contentType: "application/json",
        source: "slaw_run",
        contents: `${JSON.stringify(slawRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.slawRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "slaw/run-events.json",
        contentType: "application/json",
        source: "slaw_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "slaw/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "slaw_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.slawRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
