...
export const SQUAD_SEARCH_BRANCH_FETCH_LIMIT = SQUAD_SEARCH_MAX_OFFSET + SQUAD_SEARCH_MAX_LIMIT + 1;
...
function tokenizeQuery(normalizedQuery: string) {
  ...
  for (const match of matches) {
    ...
    if (tokens.length >= SQUAD_SEARCH_MAX_TOKENS) break;
  }
  return tokens;
}
...
function createSnippet(field: string, label: string, source: string | null | undefined, terms: string[]): SquadSearchSnippet | null {
...
function makeCounts(results: SquadSearchResult[]) {
  const counts: Record<SquadSearchResultType, number> = { issue: 0, agent: 0, project: 0 };
  for (const result of results) counts[result.type] += 1;
  return counts;
}

function scopeIncludesIssues(scope: SquadSearchScope) {
  return scope === "all" || scope === "issues" || scope === "comments" || scope === "documents";
}

function scopeIncludesAgents(scope: SquadSearchScope) {
  return scope === "all" || scope === "agents";
}

function scopeIncludesProjects(scope: SquadSearchScope) {
  return scope === "all" || scope === "projects";
}

function issueSearchCondition(scope: SquadSearchScope, input: {
...
function selectPrimarySnippets(row: IssueSearchRow, normalizedQuery: string, tokens: string[]) {
  ...
  const candidates: Array<SquadSearchSnippet | null> = [];
  ...
  return candidates.filter((snippet): snippet is SquadSearchSnippet => Boolean(snippet)).slice(0, 2);
}

function issueResult(row: IssueSearchRow, prefix: string, normalizedQuery: string, tokens: string[]): SquadSearchResult {
  ...
  const issue: SquadSearchIssueSummary = {
    id: row.id,
    identifier: row.identifier,
    title: row.title,
    status: row.status as SquadSearchIssueSummary["status"],
    priority: row.priority as SquadSearchIssueSummary["priority"],
...
export function squadSearchBranchFetchLimit(limit: number, offset = 0) {
  const normalizedLimit = Number.isFinite(limit) ? Math.max(1, Math.floor(limit)) : SQUAD_SEARCH_MAX_LIMIT;
  const normalizedOffset = Number.isFinite(offset) ? Math.max(0, Math.floor(offset)) : 0;
  return Math.min(SQUAD_SEARCH_BRANCH_FETCH_LIMIT, normalizedOffset + normalizedLimit + 1);
}

export function squadSearchService(db: Db) {
  return {
    search: async (squadId: string, query: SquadSearchQuery): Promise<SquadSearchResponse> => {
      const normalizedQuery = normalizeQuery(query.q);
      const tokens = tokenizeQuery(normalizedQuery);
      const scope = query.scope;
      const limit = query.limit;
      const offset = query.offset;
      const emptyCounts: Record<SquadSearchResultType, number> = { issue: 0, agent: 0, project: 0 };
      ...
      const prefix = routePrefix(squad?.issuePrefix);
      const fetchLimit = squadSearchBranchFetchLimit(limit, offset);
      ...
      const commentMatch = sql<boolean>`
        EXISTS (
          ...
          WHERE search_comments.squad_id = ${squadId}
      ...
      const documentMatch = sql<boolean>`
        EXISTS (
          ...
          WHERE search_issue_documents.squad_id = ${squadId}
            AND search_documents.squad_id = ${squadId}
        ...
        (
          ...
          WHERE lower(${issues.title}) LIKE '%' || search_token.value || '%' ESCAPE '\\'
            ...
            OR EXISTS (
              ...
              WHERE coverage_comments.squad_id = ${squadId}
            ...
            OR EXISTS (
              ...
              WHERE coverage_issue_documents.squad_id = ${squadId}
                AND coverage_documents.squad_id = ${squadId}
      ...
      const agentRows = scopeIncludesAgents(scope)
        ? await db
          ...
          .from(agents)
          .where(and(eq(agents.squadId, squadId), simpleCondition))
      ...
      const projectRows = scopeIncludesProjects(scope)
        ? await db
          ...
          .from(projects)
          .where(and(eq(projects.squadId, squadId), isNull(projects.archivedAt), projectCondition))
          .orderBy(desc(projects.updatedAt), desc(projects.id))
          .limit(fetchLimit)
        : [];

      const results: SquadSearchResult[] = [
