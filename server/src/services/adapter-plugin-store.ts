 ...
 * ~/.slaw/adapter-plugins.json. This is the source of truth for which
...
import { resolveSlawHomeDir } from "../home-paths.js";
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-slaw-adapter") */
  packageName: string;
...
function adapterPluginPaths() {
  const slawDir = resolveSlawHomeDir();
  return {
    adapterPluginsDir: path.join(slawDir, "adapter-plugins"),
    adapterPluginsStorePath: path.join(slawDir, "adapter-plugins.json"),
    adapterSettingsPath: path.join(slawDir, "adapter-settings.json"),
  };
}
...
function ensureDirs(): string {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "slaw-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for Slaw external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
