...
import { companies, companySkills } from "@slaw/db";
...
function buildMissingRuntimeSourceDetail(skill: Pick<CompanySkill, "name" | "sourceLocator" | "metadata">) {
  ...
  if (sourcePath) {
    return `Company skill "${skill.name}" is in the library, but Slaw cannot find its local source at ${sourcePath}.`;
  }
  return `Company skill "${skill.name}" is in the library, but Slaw cannot find a valid local runtime source for it.`;
}
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveSlawInstanceRoot(), "skills", companyId);
}
