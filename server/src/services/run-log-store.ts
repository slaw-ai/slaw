...
function createLocalFileRunLogStore(basePath: string): RunLogStore {
  ...
  return {
    async begin(input) {
      const [squadId, agentId] = safeSegments(input.squadId, input.agentId);
      const runId = safeSegments(input.runId)[0]!;
      const relDir = path.join(squadId, agentId);
...
export function getRunLogStore() {
  ...
  const basePath = process.env.RUN_LOG_BASE_PATH ?? path.resolve(resolveSlawInstanceRoot(), "data", "run-logs");
