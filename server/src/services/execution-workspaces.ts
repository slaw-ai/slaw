...
async function inspectGitCloseReadiness(workspace: ExecutionWorkspace): Promise<{
  ...
  if (!workspacePath) {
    warnings.push("Workspace has no local path, so Slaw cannot inspect git status before close.");
    return { git: null, warnings };
  }

  if (!(await pathExists(workspacePath))) {
    warnings.push(`Workspace path "${workspacePath}" does not exist, so Slaw cannot inspect git status before close.`);
    return {
      git: {
        repoRoot: null,
        workspacePath,
...
export function executionWorkspaceService(db: Db) {
  function buildListConditions(
    ...
    const conditions = [eq(executionWorkspaces.squadId, squadId)];
  ...
  return {
    list: async (squadId: string, filters?: {
      projectId?: string;
      projectWorkspaceId?: string;
      issueId?: string;
      status?: string;
      ...
      const conditions = buildListConditions(squadId, filters);
      const rows = await db
        .select()
        .from(executionWorkspaces)
        .where(and(...conditions))
        .orderBy(desc(executionWorkspaces.lastUsedAt), desc(executionWorkspaces.createdAt));
      const runtimeServicesByWorkspaceId = await loadEffectiveRuntimeServicesByExecutionWorkspace(db, squadId, rows);
      return rows.map((row) =>
    ...
    listSummaries: async (squadId: string, filters?: {
      projectId?: string;
      projectWorkspaceId?: string;
      issueId?: string;
      status?: string;
    ...
    getById: async (id: string) => {
      ...
      if (!row) return null;
      const runtimeServicesByWorkspaceId = await loadEffectiveRuntimeServicesByExecutionWorkspace(db, row.squadId, [row]);
    ...
    getCloseReadiness: async (id: string): Promise<ExecutionWorkspaceCloseReadiness | null> => {
      ...
      const runtimeServicesByWorkspaceId = await loadEffectiveRuntimeServicesByExecutionWorkspace(db, workspace.squadId, [workspace]);
      ...
      const linkedIssues = await db
        ...
        .from(issues)
        .where(and(eq(issues.squadId, workspace.squadId), eq(issues.executionWorkspaceId, workspace.id)));
      ...
      const projectWorkspace = workspace.projectWorkspaceId
        ? await db
            ...
            .where(
              and(
                eq(projectWorkspaces.squadId, workspace.squadId),
      ...
      const projectPolicy = workspace.projectId
        ? await db
            ...
            .from(projects)
            .where(and(eq(projects.id, workspace.projectId), eq(projects.squadId, workspace.squadId)))
      ...
      if (executionWorkspace.providerType === "git_worktree" && workspacePath) {
        plannedActions.push({
          ...
          description: `Slaw will run git worktree cleanup for ${workspacePath}.`,
      ...
      if (git?.createdByRuntime && executionWorkspace.branchName) {
        plannedActions.push({
          ...
          description: "Slaw will try to delete the runtime-created branch after removing the worktree.",
      ...
      if (executionWorkspace.providerType === "local_fs" && git?.createdByRuntime && workspacePath) {
        ...
        if (containsProjectWorkspace) {
          warnings.push(`Slaw will archive this workspace but keep "${workspacePath}" because it contains the project workspace.`);
        } else {
          plannedActions.push({
            kind: "remove_local_directory",
            label: "Remove runtime-created directory",
            description: `Slaw will remove the runtime-created directory at ${workspacePath}.`,
    ...
    clearEnvironmentSelection: async (squadId: string, environmentId: string) => {
      return db.transaction(async (tx) => {
        const rows = await tx
          ...
          .from(executionWorkspaces)
          .where(eq(executionWorkspaces.squadId, squadId));
