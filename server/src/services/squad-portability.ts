...
import { generateReadme } from "./squad-export-readme.js";
...
const DEFAULT_INCLUDE: SquadPortabilityInclude = {
  squad: true,
  agents: true,
  projects: false,
  issues: false,
...
const DEFAULT_COLLISION_STRATEGY: SquadPortabilityCollisionStrategy = "rename";
...
function classifyPortableFileKind(pathValue: string): SquadPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  const normalized = normalizePortablePath(pathValue);
  if (normalized === "SQUAD.md") return "squad";
  if (normalized === ".slaw.yaml" || normalized === ".slaw.yml") return "extension";
...
function readSkillSourceKind(skill: SquadSkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  return asString(metadata?.sourceKind);
}

function buildPortableCatalogProvenance(skill: SquadSkill) {
...
function deriveLocalExportNamespace(skill: SquadSkill, slug: string) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  const candidates = [
...
function derivePrimarySkillExportDir(
  skill: SquadSkill,
  slug: string,
  squadIssuePrefix: string | null | undefined,
) {
  ...
  if (primaryNamespace === "squad") {
    const squadSegment = normalizeExportPathSegment(squadIssuePrefix, true)
      ?? normalizeExportPathSegment(keySegments[1], true)
      ?? "squad";
    return `skills/squad/${squadSegment}/${slug}`;
  }
...
function deriveSkillExportDirCandidates(
  ...
  const primaryDir = derivePrimarySkillExportDir(skill, slug, squadIssuePrefix);
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  ...
  if (skill.sourceType === "github" || skill.sourceType === "skills_sh") {
    ...
    if (sourceKind === "managed_local") pushSuffix("squad");
...
function buildSkillExportDirMap(skills: SquadSkill[], squadIssuePrefix: string | null | undefined) {
  ...
  for (const skill of orderedSkills) {
    ...
    const candidates = deriveSkillExportDirCandidates(skill, slug, squadIssuePrefix);
...
function extractPortableScopedEnvInputs(
  ...
  const inputs: SquadPortabilityEnvInput[] = [];
...
type SquadPackageIncludeEntry = {
  path: string;
};

type SlawExtensionDoc = {
  schema?: string;
...
const SQUAD_LOGO_CONTENT_TYPE_EXTENSIONS: Record<string, string> = {
  "image/gif": ".gif",
  "image/jpeg": ".jpg",
  "image/png": ".png",
  "image/svg+xml": ".svg",
...
const SQUAD_LOGO_FILE_NAME = "squad-logo";
...
function readPortableIssueComments(
  ...
  const comments: SquadPortabilityIssueCommentManifestEntry[] = [];
...
function normalizeRoutineTriggerExtension(value: unknown): SquadPortabilityIssueRoutineTriggerManifestEntry | null {
...
function normalizeRoutineExtension(value: unknown): SquadPortabilityIssueRoutineManifestEntry | null {
  if (!isPlainRecord(value)) return null;
  const triggers = Array.isArray(value.triggers)
    ? value.triggers
      .map((entry) => normalizeRoutineTriggerExtension(entry))
      .filter((entry): entry is SquadPortabilityIssueRoutineTriggerManifestEntry => entry !== null)
    : [];
...
function buildRoutineManifestFromLiveRoutine(routine: RoutineLike): SquadPortabilityIssueRoutineManifestEntry {
  return {
...
function normalizePortableProjectWorkspaceExtension(
  workspaceKey: string,
  value: unknown,
): SquadPortabilityProjectWorkspaceManifestEntry | null {
  if (!isPlainRecord(value)) return null;
...
async function buildPortableProjectWorkspaces(
  ...
  const manifestWorkspaces: SquadPortabilityProjectWorkspaceManifestEntry[] = [];
  const workspaceKeyById = new Map<string, string>();
  const workspaceKeyBySignature = new Map<string, string>();
  const manifestWorkspaceByKey = new Map<string, SquadPortabilityProjectWorkspaceManifestEntry>();
...
function buildLegacyRoutineTriggerFromRecurrence(
  issue: Pick<SquadPortabilityIssueManifestEntry, "slug" | "legacyRecurrence">,
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .slaw.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .slaw.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .slaw.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .slaw.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Slaw will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .slaw.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .slaw.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .slaw.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .slaw.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .slaw.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .slaw.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .slaw.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .slaw.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .slaw.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  return {
    ...
    } satisfies SquadPortabilityIssueRoutineTriggerManifestEntry,
    warnings,
    errors,
  };
}
...
function resolvePortableRoutineDefinition(
  issue: Pick<SquadPortabilityIssueManifestEntry, "slug" | "recurring" | "routine" | "legacyRecurrence">,
  scheduleValue: unknown,
) {
  const warnings: string[] = [];
  const errors: string[] = [];
  ...
  const routine = issue.routine
    ...
    : {
      ...
      catchUpPolicy: null,
      variables: null,
      triggers: [] as SquadPortabilityIssueRoutineTriggerManifestEntry[],
    };
...
function normalizeInclude(input?: Partial<SquadPortabilityInclude>): SquadPortabilityInclude {
  return {
    squad: input?.squad ?? DEFAULT_INCLUDE.squad,
...
function isPortableBinaryFile(
  value: SquadPortabilityFileEntry,
): value is Extract<SquadPortabilityFileEntry, { encoding: "base64" }> {
...
function resolveSquadLogoExtension(contentType: string | null | undefined, originalFilename: string | null | undefined) {
  const fromContentType = contentType ? SQUAD_LOGO_CONTENT_TYPE_EXTENSIONS[contentType.toLowerCase()] : null;
...
function portableBinaryFileToBuffer(entry: Extract<SquadPortabilityFileEntry, { encoding: "base64" }>) {
  return Buffer.from(entry.data, "base64");
}

function portableFileToBuffer(entry: SquadPortabilityFileEntry, filePath: string) {
...
function bufferToPortableBinaryFile(buffer: Buffer, contentType: string | null): SquadPortabilityFileEntry {
  return {
    encoding: "base64",
...
): Record<string, SquadPortabilityFileEntry> {
  const normalizedRoot = rootPath ? normalizePortablePath(rootPath) : null;
  const out: Record<string, SquadPortabilityFileEntry> = {};
...
function pickTextFiles(files: Record<string, SquadPortabilityFileEntry>) {
...
function normalizePortableSidebarOrder(value: unknown): SquadPortabilitySidebarOrder | null {
  if (!isPlainRecord(value)) return null;
  const sidebar = {
...
function filterPortableExtensionYaml(yaml: string, selectedFiles: Set<string>) {
  ...
  const squadSection = parsed.squad;
  if (isPlainRecord(squadSection)) {
    const logoPath = asString(squadSection.logoPath) ?? asString(squadSection.logo);
    if (logoPath && !selectedFiles.has(logoPath)) {
      delete squadSection.logoPath;
      delete squadSection.logo;
    }
  }
...
function filterExportFiles(
  ...
  const filtered: Record<string, SquadPortabilityFileEntry> = {};
  for (const [filePath, content] of Object.entries(files)) {
    if (!selectedFiles.has(filePath)) continue;
    filtered[filePath] = content;
  }

  const extensionEntry = filtered[slawExtensionPath];
  if (selectedFiles.has(slawExtensionPath) && typeof extensionEntry === "string") {
    filtered[slawExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findSlawExtensionPath(files: Record<string, SquadPortabilityFileEntry>) {
  if (typeof files[".slaw.yaml"] === "string") return ".slaw.yaml";
  if (typeof files[".slaw.yml"] === "string") return ".slaw.yml";
...
function filterSquadMarkdownIncludes(
  squadPath: string,
  markdown: string,
  ...
  const filteredIncludes = includeEntries.filter((entry) =>
    selectedFiles.has(resolvePortablePath(squadPath, entry.path)),
  );
...
function applySelectedFilesToSource(source: ResolvedSource, selectedFiles?: string[]): ResolvedSource {
  ...
  const squadPath = source.manifest.squad
    ? ensureMarkdownPath(source.manifest.squad.path)
    : Object.keys(source.files).find((entry) => entry.endsWith("/SQUAD.md") || entry === "SQUAD.md") ?? null;
  if (!squadPath) {
    throw unprocessable("Squad package is missing SQUAD.md");
  }

  const squadMarkdown = source.files[squadPath];
  if (typeof squadMarkdown !== "string") {
    throw unprocessable("Squad package is missing SQUAD.md");
  }

  const effectiveFiles: Record<string, SquadPortabilityFileEntry> = {};
  ...
  effectiveFiles[squadPath] = filterSquadMarkdownIncludes(
    squadPath,
    squadMarkdown,
    normalizedSelection,
  );
  ...
  if (!normalizedSelection.has(squadPath)) {
    filtered.manifest.squad = null;
  }

  filtered.manifest.includes = {
    squad: filtered.manifest.squad !== null,
...
async function buildSkillSourceEntry(skill: SquadSkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "slaw_bundled") {
    ...
    return {
      ...
      commit,
      trackingRef: "master",
      url: `https://github.com/slaw/slaw/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: SquadSkill, expandReferencedSkills: boolean) {
  if (expandReferencedSkills) return false;
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "slaw_bundled") return true;
  return skill.sourceType === "github" || skill.sourceType === "skills_sh" || skill.sourceType === "url";
}

async function buildReferencedSkillMarkdown(skill: SquadSkill) {
...
async function withSkillSourceMetadata(skill: SquadSkill, markdown: string) {
  const sourceEntry = await buildSkillSourceEntry(skill);
  const parsed = parseFrontmatterMarkdown(markdown);
  ...
  metadata.slawSkillKey = skill.key;
  metadata.slaw = {
    ...(isPlainRecord(metadata.slaw) ? metadata.slaw : {}),
    skillKey: skill.key,
    slug: skill.slug,
...
function dedupeEnvInputs(values: SquadPortabilityManifest["envInputs"]) {
  const seen = new Set<string>();
  const out: SquadPortabilityManifest["envInputs"] = [];
...
function buildEnvInputMap(inputs: SquadPortabilityEnvInput[]) {
...
function readSquadApprovalDefault(_frontmatter: Record<string, unknown>) {
  return false;
}

function readIncludeEntries(frontmatter: Record<string, unknown>): SquadPackageIncludeEntry[] {
...
function readAgentEnvInputs(
  ...
  agentSlug: string,
): SquadPortabilityManifest["envInputs"] {
...
function buildManifestFromPackageFiles(
  files: Record<string, SquadPortabilityFileEntry>,
  opts?: { sourceLabel?: { squadId: string; squadName: string } | null },
): ResolvedSource {
  const normalizedFiles = normalizeFileMap(files);
  const squadPath = typeof normalizedFiles["SQUAD.md"] === "string"
    ? normalizedFiles["SQUAD.md"]
    : undefined;
  const resolvedSquadPath = squadPath !== undefined
    ? "SQUAD.md"
    : Object.keys(normalizedFiles).find((entry) => entry.endsWith("/SQUAD.md") || entry === "SQUAD.md");
  if (!resolvedSquadPath) {
    throw unprocessable("Squad package is missing SQUAD.md");
  }

  const squadMarkdown = readPortableTextFile(normalizedFiles, resolvedSquadPath);
  if (typeof squadMarkdown !== "string") {
    throw unprocessable(`Squad package file is not readable as text: ${resolvedSquadPath}`);
  }
  const squadDoc = parseFrontmatterMarkdown(squadMarkdown);
  const squadFrontmatter = squadDoc.frontmatter;
  const slawExtensionPath = findSlawExtensionPath(normalizedFiles);
  const slawExtension = slawExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, slawExtensionPath) ?? "")
    : {};
  const slawSquad = isPlainRecord(slawExtension.squad) ? slawExtension.squad : {};
  const slawSidebar = normalizePortableSidebarOrder(slawExtension.sidebar);
  const slawAgents = isPlainRecord(slawExtension.agents) ? slawExtension.agents : {};
  const slawProjects = isPlainRecord(slawExtension.projects) ? slawExtension.projects : {};
  const slawTasks = isPlainRecord(slawExtension.tasks) ? slawExtension.tasks : {};
  const slawRoutines = isPlainRecord(slawExtension.routines) ? slawExtension.routines : {};
  const squadName =
    asString(squadFrontmatter.name)
    ?? opts?.sourceLabel?.squadName
    ?? "Imported Squad";
  const squadSlug =
    asString(squadFrontmatter.slug)
    ?? normalizeAgentUrlKey(squadName)
    ?? "squad";

  const includeEntries = readIncludeEntries(squadFrontmatter);
  const referencedAgentPaths = includeEntries
    .map((entry) => resolvePortablePath(resolvedSquadPath, entry.path))
  ...
  const manifest: SquadPortabilityManifest = {
    ...
    squad: {
      path: resolvedSquadPath,
      name: squadName,
      description: asString(squadFrontmatter.description),
      brandColor: asString(slawSquad.brandColor),
      logoPath: asString(slawSquad.logoPath) ?? asString(slawSquad.logo),
      attachmentMaxBytes:
      ...
      feedbackDataSharingEnabled:
        typeof slawSquad.feedbackDataSharingEnabled === "boolean"
          ? slawSquad.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof slawSquad.feedbackDataSharingConsentAt === "string"
          ? slawSquad.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(slawSquad.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(slawSquad.feedbackDataSharingTermsVersion),
    },
    sidebar: slawSidebar,
    agents: [],
    skills: [],
  ...
  const warnings: string[] = [];
  if (manifest.squad?.logoPath && !normalizedFiles[manifest.squad.logoPath]) {
    warnings.push(`Referenced squad logo file is missing from package: ${manifest.squad.logoPath}`);
  }
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(slawAgents[slug]) ? slawAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(slawProjects[slug]) ? slawProjects[slug] : {};
    ...
    const workspaces = Object.entries(workspaceExtensions)
      ...
      .filter((entry): entry is SquadPortabilityProjectWorkspaceManifestEntry => entry !== null);
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(slawTasks[slug]) ? slawTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(slawRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(slawRoutines[slug]) ? slawRoutines[slug] : {};
...
export function parseGitHubSourceUrl(rawUrl: string) {
  ...
  const querySquadPath = normalizeGitHubSourcePath(url.searchParams.get("squadPath"));
  if (queryRef || queryPath || querySquadPath) {
    const squadPath = querySquadPath || [queryPath, "SQUAD.md"].filter(Boolean).join("/") || "SQUAD.md";
    let basePath = queryPath;
    if (!basePath && squadPath !== "SQUAD.md") {
      basePath = path.posix.dirname(squadPath);
  ...
  return { hostname, owner, repo, ref, basePath, squadPath };
}


export function squadPortabilityService(db: Db, storage?: StorageService) {
  ...
  const strictSecretsMode = process.env.SLAW_SECRETS_STRICT_MODE === "true";
  ...
  async function prepareImportedAgentAdapter(
    ...
    const nextAdapterConfig = writeSlawSkillSyncPreference(
  ...
  async function resolveSource(source: SquadPortabilityPreview["source"]): Promise<ResolvedSource> {
    ...
    let ref = parsed.ref;
    const warnings: string[] = [];
    const squadRelativePath = parsed.squadPath === "SQUAD.md"
      ? [parsed.basePath, "SQUAD.md"].filter(Boolean).join("/")
      : parsed.squadPath;
    let squadMarkdown: string | null = null;
    try {
      squadMarkdown = await fetchOptionalText(
        resolveRawGitHubUrl(parsed.hostname, parsed.owner, parsed.repo, ref, squadRelativePath),
      );
    } catch (err) {
      if (ref === "main") {
        ref = "master";
    ...
    if (!squadMarkdown) {
      throw unprocessable("GitHub squad package is missing SQUAD.md");
    }

    const squadPath = parsed.squadPath === "SQUAD.md"
      ? "SQUAD.md"
      : normalizePortablePath(path.posix.relative(parsed.basePath || ".", parsed.squadPath));
    const files: Record<string, SquadPortabilityFileEntry> = {
      [squadPath]: squadMarkdown,
    };
    ...
    const squadDoc = parseFrontmatterMarkdown(squadMarkdown);
    const includeEntries = readIncludeEntries(squadDoc.frontmatter);
    ...
    const squadLogoPath = resolved.manifest.squad?.logoPath;
    if (squadLogoPath && !resolved.files[squadLogoPath]) {
      const repoPath = [parsed.basePath, squadLogoPath].filter(Boolean).join("/");
      try {
        const binary = await fetchBinary(
          resolveRawGitHubUrl(parsed.hostname, parsed.owner, parsed.repo, ref, repoPath),
        );
        resolved.files[squadLogoPath] = bufferToPortableBinaryFile(binary, inferContentTypeFromPath(squadLogoPath));
      } catch (err) {
        warnings.push(`Failed to fetch squad logo ${squadLogoPath} from GitHub: ${err instanceof Error ? err.message : String(err)}`);
      }
    }
  ...
  async function exportBundle(
    squadId: string,
    input: SquadPortabilityExport,
  ): Promise<SquadPortabilityExportResult> {
    ...
    const files: Record<string, SquadPortabilityFileEntry> = {};
    const warnings: string[] = [];
    const envInputs: SquadPortabilityManifest["envInputs"] = [];
    const requestedSidebarOrder = normalizePortableSidebarOrder(input.sidebarOrder);
    const rootPath = normalizeAgentUrlKey(squad.name) ?? "squad-package";
    let squadLogoPath: string | null = null;

    const allAgentRows = include.agents ? await agents.list(squadId, { includeTerminated: true }) : [];
    const liveAgentRows = allAgentRows.filter((agent) => agent.status !== "terminated");
    const squadSkillRows = include.skills || include.agents ? await squadSkills.listFull(squadId) : [];
    if (include.agents) {
    ...
    const allProjectsRaw = include.projects || include.issues ? await projectsSvc.list(squadId) : [];
    const allProjects = allProjectsRaw.filter((project) => !project.archivedAt);
    const allRoutines = include.issues ? await routinesSvc.list(squadId) : [];
    ...
    for (const selector of input.projectIssues ?? []) {
      ...
      selectedProjects.set(match.id, match);
      const projectIssues = await issuesSvc.list(squadId, { projectId: match.id });
    ...
    if (include.issues && selectedIssues.size === 0) {
      const allIssues = await issuesSvc.list(squadId);
    ...
    const squadPath = "SQUAD.md";
    files[squadPath] = buildMarkdown(
      {
        name: squad.name,
    ...
    if (include.squad && squad.logoAssetId) {
      if (!storage) {
        warnings.push("Skipped squad logo from export because storage is unavailable.");
      } else {
        const logoAsset = await assetRecords.getById(squad.logoAssetId);
        if (!logoAsset) {
          warnings.push(`Skipped squad logo ${squad.logoAssetId} because the asset record was not found.`);
        } else {
          try {
            const object = await storage.getObject(squad.id, logoAsset.objectKey);
            const body = await streamToBuffer(object.stream);
            squadLogoPath = `images/${SQUAD_LOGO_FILE_NAME}${resolveSquadLogoExtension(logoAsset.contentType, logoAsset.originalFilename)}`;
            files[squadLogoPath] = bufferToPortableBinaryFile(body, logoAsset.contentType);
          } catch (err) {
            warnings.push(`Failed to export squad logo ${squad.logoAssetId}: ${err instanceof Error ? err.message : String(err)}`);
          }
        }
      }
    }

    const slawAgentsOut: Record<string, Record<string, unknown>> = {};
    const slawProjectsOut: Record<string, Record<string, unknown>> = {};
    const slawTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const slawRoutinesOut: Record<string, Record<string, unknown>> = {};

    const skillByReference = new Map<string, typeof squadSkillRows[number]>();
    for (const skill of squadSkillRows) {
    ...
    const selectedSkills = new Map<string, typeof squadSkillRows[number]>();
    ...
    const skillExportDirs = buildSkillExportDirMap(selectedSkillRows, squad.issuePrefix);
    for (const skill of selectedSkillRows) {
      ...
      for (const inventoryEntry of skill.fileInventory) {
        const fileDetail = await squadSkills.readFile(squadId, skill.id, inventoryEntry.path).catch(() => null);
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readSlawSkillSyncPreference(
        ...
        slawAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      slawProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      slawTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      slawRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const slawExtensionPath = ".slaw.yaml";
    const slawAgents = Object.fromEntries(
      Object.entries(slawAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const slawProjects = Object.fromEntries(
      Object.entries(slawProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const slawTasks = Object.fromEntries(
      Object.entries(slawTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const slawRoutines = Object.fromEntries(
      Object.entries(slawRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[slawExtensionPath] = buildYamlFile(
      {
        schema: "slaw/v1",
        squad: stripEmptyValues({
          ...
          feedbackDataSharingTermsVersion: squad.feedbackDataSharingTermsVersion ?? null,
        }),
        sidebar: stripEmptyValues(sidebarOrder),
        agents: Object.keys(slawAgents).length > 0 ? slawAgents : undefined,
        projects: Object.keys(slawProjects).length > 0 ? slawProjects : undefined,
        tasks: Object.keys(slawTasks).length > 0 ? slawTasks : undefined,
        routines: Object.keys(slawRoutines).length > 0 ? slawRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, slawExtensionPath);
    ...
    resolved.manifest.includes = {
      squad: resolved.manifest.squad !== null,
    ...
    if (!input.selectedFiles || input.selectedFiles.some((entry) => normalizePortablePath(entry) === "README.md")) {
      finalFiles["README.md"] = generateReadme(resolved.manifest, {
        squadName: squad.name,
        squadDescription: squad.description ?? null,
      });
    }
  ...
  async function previewExport(
    squadId: string,
    input: SquadPortabilityExport,
  ): Promise<SquadPortabilityExportPreviewResult> {
    const previewInput: SquadPortabilityExport = {
      ...input,
      include: {
        ...input.include,
        issues:
    ...
    const exported = await exportBundle(squadId, previewInput);
    return {
      ...exported,
  ...
  async function buildPreview(
    ...
    const include: SquadPortabilityInclude = {
      squad: requestedInclude.squad && manifest.squad !== null,
    ...
    if (include.squad && !manifest.squad) {
      errors.push("Manifest does not include squad metadata.");
    }
    ...
    const availableSkillSlugs = new Map<string, SquadPortabilitySkillManifestEntry[]>();
    ...
    if (input.target.mode === "existing_squad") {
      const targetSquad = await squads.getById(input.target.squadId);
      if (!targetSquad) throw notFound("Target squad not found");
      targetSquadId = targetSquad.id;
      targetSquadName = targetSquad.name;
    }

    const agentPlans: SquadPortabilityPreviewAgentPlan[] = [];
    const existingSlugToAgent = new Map<string, { id: string; name: string }>();
    const existingSlugs = new Set<string>();
    const projectPlans: SquadPortabilityPreviewResult["plan"]["projectPlans"] = [];
    const issuePlans: SquadPortabilityPreviewResult["plan"]["issuePlans"] = [];
    const existingProjectSlugToProject = new Map<string, { id: string; name: string }>();
    const existingProjectSlugs = new Set<string>();

    if (input.target.mode === "existing_squad") {
      const existingAgents = await agents.list(input.target.squadId);
      ...
      const existingProjects = await projects.list(input.target.squadId);
      ...
      const existingSkills = await squadSkills.listFull(input.target.squadId);
  ...
  async function previewImport(
    input: SquadPortabilityPreview,
    options?: ImportBehaviorOptions,
  ): Promise<SquadPortabilityPreviewResult> {
    const plan = await buildPreview(input, options);
    return plan.preview;
  }

  async function importBundle(
    input: SquadPortabilityImport,
    actorUserId: string | null | undefined,
    options?: ImportBehaviorOptions,
  ): Promise<SquadPortabilityImportResult> {
    const mode = resolveImportMode(options);
    const plan = await buildPreview(input, options);
    ...
    if (
      mode === "agent_safe"
      && (
        plan.preview.plan.squadAction === "update"
    ...
    if (input.target.mode === "new_squad") {
      if (mode === "agent_safe" && !options?.sourceSquadId) {
        throw unprocessable("Safe new-squad imports require a source squad context.");
      }
      if (mode === "agent_safe" && options?.sourceSquadId) {
        const sourceMemberships = await access.listActiveUserMemberships(options.sourceSquadId);
        if (sourceMemberships.length === 0) {
          throw unprocessable("Safe new-squad import requires at least one active user membership on the source squad.");
        }
      }
      const squadName =
        asString(input.target.newSquadName) ??
        sourceManifest.squad?.name ??
        sourceManifest.source?.squadName ??
        "Imported Squad";
      const created = await squads.create({
        name: squadName,
        description: include.squad ? (sourceManifest.squad?.description ?? null) : null,
        brandColor: include.squad ? (sourceManifest.squad?.brandColor ?? null) : null,
        ...
        feedbackDataSharingEnabled: include.squad
          ...
          : false,
        feedbackDataSharingConsentAt: include.squad && sourceManifest.squad?.feedbackDataSharingConsentAt
          ? new Date(sourceManifest.squad.feedbackDataSharingConsentAt)
          : null,
        feedbackDataSharingConsentByUserId: include.squad
          ? (sourceManifest.squad?.feedbackDataSharingConsentByUserId ?? null)
          : null,
        feedbackDataSharingTermsVersion: include.squad
          ? (sourceManifest.squad?.feedbackDataSharingTermsVersion ?? null)
          : null,
      });
      if (mode === "agent_safe" && options?.sourceSquadId) {
      ...
      if (include.squad && sourceManifest.squad && mode === "operator_full") {
        const updated = await squads.update(targetSquad.id, {
          ...
          feedbackDataSharingConsentAt: sourceManifest.squad.feedbackDataSharingConsentAt
            ? new Date(sourceManifest.squad.feedbackDataSharingConsentAt)
            : null,
          feedbackDataSharingConsentByUserId: sourceManifest.squad.feedbackDataSharingConsentByUserId,
          feedbackDataSharingTermsVersion: sourceManifest.squad.feedbackDataSharingTermsVersion,
        });
        targetSquad = updated ?? targetSquad;
        squadAction = "updated";
      }
    }
    ...
    if (include.squad) {
      const logoPath = sourceManifest.squad?.logoPath ?? null;
      if (!logoPath) {
        const cleared = await squads.update(targetSquad.id, { logoAssetId: null });
        targetSquad = cleared ?? targetSquad;
      } else {
        const logoFile = plan.source.files[logoPath];
        if (!logoFile) {
          warnings.push(`Skipped squad logo import because ${logoPath} is missing from the package.`);
        } else if (!storage) {
          warnings.push("Skipped squad logo import because storage is unavailable.");
        } else {
          ...
          if (!contentType || !SQUAD_LOGO_CONTENT_TYPE_EXTENSIONS[contentType]) {
            warnings.push(`Skipped squad logo import for ${logoPath} because the file type is unsupported.`);
          } else {
            try {
              ...
              const createdAsset = await assetRecords.create(targetSquad.id, {
              ...
              warnings.push(`Failed to import squad logo ${logoPath}: ${err instanceof Error ? err.message : String(err)}`);
            }
          }
        }
      }
    ...
    const resultAgents: SquadPortabilityImportResult["agents"] = [];
    const resultProjects: SquadPortabilityImportResult["projects"] = [];
    const importedSlugToAgentId = new Map<string, string>();
    const existingSlugToAgentId = new Map<string, string>();
    const agentStatusById = new Map<string, string | null | undefined>();
    const existingAgents = await agents.list(targetSquad.id);
    for (const existing of existingAgents) {
    ...
    const existingProjects = await projects.list(targetSquad.id);
    for (const existing of existingProjects) {
      existingProjectSlugToId.set(existing.urlKey, existing.id);
    }

    const importedSkills = include.skills || include.agents
      ? await squadSkills.importPackageFiles(targetSquad.id, pickTextFiles(plan.source.files), {
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        let created = await agents.create(targetSquad.id, {
          ...patch,
          status: createdStatus,
        });
        await access.ensureMembership(targetSquad.id, "agent", created.id, "member", "active");
    ...
    if (include.projects) {
      for (const planProject of plan.preview.plan.projectPlans) {
        ...
        if (planProject.action === "update" && planProject.existingProjectId) {
          ...
          const created = await projects.create(targetSquad.id, projectPatch);
    ...
    if (include.issues) {
      ...
      for (const manifestIssue of sourceManifest.issues) {
        ...
        if (manifestIssue.recurring) {
          ...
          const createdRoutine = await routines.create(targetSquad.id, {
            projectId,
            goalId: null,
            parentIssueId: null,
        ...
        const createdIssue = await issues.create(targetSquad.id, {
          projectId,
          projectWorkspaceId,
          title: manifestIssue.title,
          description,
