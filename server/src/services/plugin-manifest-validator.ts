 ...
 */
import { pluginManifestV1Schema } from "@slaw-ai/shared";
import type { SlawPluginManifestV1 } from "@slaw-ai/shared";
import { PLUGIN_API_VERSION } from "@slaw-ai/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): SlawPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as SlawPluginManifestV1,
        };
      }
