/**
 * PluginEventBus — typed in-process event bus for the Slaw plugin system.
 *
 * Responsibilities:
 ...
 *
 * - `squadId` — always resolved from `payload.squadId`. Core domain events that
 *   belong to a squad embed the squad ID in their payload.
 *
...
function passesFilter(event: PluginEvent, filter: EventFilter | null): boolean {
  ...
  if (filter.squadId !== undefined) {
    if (event.squadId !== filter.squadId) return false;
  }
...
export function createPluginEventBus(): PluginEventBus {
  ...
  function forPlugin(pluginId: string): ScopedPluginEventBus {
    return {
       ...
       */
      async emit(name: string, squadId: string, payload: unknown): Promise<PluginEventBusEmitResult> {
        if (!name || name.trim() === "") {
          throw new Error(`Plugin "${pluginId}" must provide a non-empty event name.`);
        }

        if (!squadId || squadId.trim() === "") {
          throw new Error(`Plugin "${pluginId}" must provide a squadId when emitting events.`);
        }
...
export interface ScopedPluginEventBus {
   ...
   */
  emit(name: string, squadId: string, payload: unknown): Promise<PluginEventBusEmitResult>;
