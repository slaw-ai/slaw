...
function createDbStub() {
  ...
  const isSquadMembershipsTable = (table: unknown) =>
    !!table &&
    typeof table === "object" &&
  ...
  return {
    select() {
      return {
        from(table: unknown) {
          if (isSquadMembershipsTable(table)) {
            const query = {
              where() {
                return query;
              },
...
describe("GET /squads/:squadId/user-directory", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("returns active human users for operators without manage-permissions access", async () => {
    ...
    const res = await request(app).get("/api/squads/squad-1/user-directory");
