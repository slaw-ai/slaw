...
// 4. Restoring an older revision re-syncs `squad_secret_bindings` to the snapshot env.
...
// 6. Disabled / missing / cross-squad secret bindings fail clearly without
//    echoing the value.
...
describeEmbedded("PAP-9522 QA: routine secrets end-to-end", () => {
  ...
  const secretsTmpDir = path.join(os.tmpdir(), `slaw-qa-routine-secrets-${randomUUID()}`);
  ...
  beforeAll(async () => {
    ...
    tempDb = await startEmbeddedPostgresTestDatabase("slaw-qa-routine-secrets-");
  ...
  afterAll(async () => {
    ...
    if (previousKeyFile === undefined) delete process.env.SLAW_SECRETS_MASTER_KEY_FILE;
    else process.env.SLAW_SECRETS_MASTER_KEY_FILE = previousKeyFile;
  ...
  async function seed() {
    ...
    return { squadId, executorAgentId };
  }
  ...
  it("resolves routine env for an executing agent that has no direct binding, with routine winning precedence and zero value in access events", async () => {
    const { squadId, executorAgentId } = await seed();
  ...
  it("rejects routine env that references a secret from a different squad", async () => {
    const { squadId } = await seed();
    const { squadId: otherSquadId } = await seed();
    const secrets = secretService(db);
    const routines = routineService(db, { heartbeat: { wakeup: async () => null } });

    const foreignSecret = await secrets.create(otherSquadId, {
      name: `foreign-${randomUUID()}`,
      provider: "local_encrypted",
      value: "cross-squad-leak-bait",
    });
  ...
  it("restoring an older revision re-syncs squad_secret_bindings to the snapshot env", async () => {
    const { squadId, executorAgentId } = await seed();
    const secrets = secretService(db);
    const routines = routineService(db, { heartbeat: { wakeup: async () => null } });

    const secretA = await secrets.create(squadId, {
      name: `a-${randomUUID()}`,
      provider: "local_encrypted",
      value: "val-a",
    });
    const secretB = await secrets.create(squadId, {
      name: `b-${randomUUID()}`,
      provider: "local_encrypted",
      value: "val-b",
    });
