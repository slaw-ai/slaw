...
<head>
    <!-- SLAW_RUNTIME_BRANDING_START -->
    <!-- SLAW_RUNTIME_BRANDING_END -->
    <!-- SLAW_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from SLAW_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ SLAW_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ SLAW_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ SLAW_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      SLAW_IN_WORKTREE: "true",
      SLAW_WORKTREE_NAME: "slaw-pr-432",
      SLAW_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("slaw-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="slaw-worktree-name"');
    expect(meta).toContain('content="slaw-pr-432"');
    expect(meta).toContain('name="slaw-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="slaw-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="slaw-worktree-name"');
  });
});
