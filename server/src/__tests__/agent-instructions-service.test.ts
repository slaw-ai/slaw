...
describe("agent instructions service", () => {
  const originalSlawHome = process.env.SLAW_HOME;
  const originalSlawInstanceId = process.env.SLAW_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalSlawHome === undefined) delete process.env.SLAW_HOME;
    else process.env.SLAW_HOME = originalSlawHome;
    if (originalSlawInstanceId === undefined) delete process.env.SLAW_INSTANCE_ID;
    else process.env.SLAW_INSTANCE_ID = originalSlawInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const slawHome = await makeTempDir("slaw-agent-instructions-home-");
    const externalRoot = await makeTempDir("slaw-agent-instructions-external-");
    cleanupDirs.add(slawHome);
    cleanupDirs.add(externalRoot);
    process.env.SLAW_HOME = slawHome;
    process.env.SLAW_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("slaw-agent-instructions-new-external-");
    cleanupDirs.add(slawHome);
    cleanupDirs.add(externalRoot);
    process.env.SLAW_HOME = slawHome;
    process.env.SLAW_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("slaw-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const slawHome = await makeTempDir("slaw-agent-instructions-recover-");
    cleanupDirs.add(slawHome);
    process.env.SLAW_HOME = slawHome;
    process.env.SLAW_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const slawHome = await makeTempDir("slaw-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("slaw-agent-instructions-stale-root-");
    cleanupDirs.add(slawHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const slawHome = await makeTempDir("slaw-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("slaw-agent-instructions-heal-write-stale-");
    cleanupDirs.add(slawHome);
    cleanupDirs.add(staleRoot);
    process.env.SLAW_HOME = slawHome;
    process.env.SLAW_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const slawHome = await makeTempDir("slaw-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("slaw-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(slawHome);
    cleanupDirs.add(staleRoot);
    process.env.SLAW_HOME = slawHome;
    process.env.SLAW_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const slawHome = await makeTempDir("slaw-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("slaw-agent-instructions-partial-root-");
    cleanupDirs.add(slawHome);
    cleanupDirs.add(staleRoot);
    process.env.SLAW_HOME = slawHome;
    process.env.SLAW_INSTANCE_ID = "test-instance";
