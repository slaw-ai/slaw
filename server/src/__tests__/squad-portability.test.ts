...
const secretSvc = {
  ...
  resolveAdapterConfigForRuntime: vi.fn(async (_squadId: string, config: Record<string, unknown>) => ({ config, secretKeys: new Set<string>() })),
};
...
vi.mock("../services/squad-skills.js", () => ({
  squadSkillService: () => squadSkillSvc,
}));
...
const { squadPortabilityService, parseGitHubSourceUrl } = await import("../services/squad-portability.js");

function asTextFile(entry: SquadPortabilityFileEntry | undefined) {
  expect(typeof entry).toBe("string");
  return typeof entry === "string" ? entry : "";
}

describe("squad portability", () => {
  const slawKey = "slaw/slaw/slaw";
  const squadPlaybookKey = "squad/squad-1/squad-playbook";

  beforeEach(() => {
    vi.clearAllMocks();
    secretSvc.normalizeAdapterConfigForPersistence.mockImplementation(async (_squadId, config) => config);
    secretSvc.resolveAdapterConfigForRuntime.mockImplementation(async (_squadId, config) => ({
      config,
    ...
    squadSvc.getById.mockResolvedValue({
      id: "squad-1",
      name: "Slaw",
      description: null,
      issuePrefix: "PAP",
    ...
    routineSvc.create.mockImplementation(async (_squadId: string, input: Record<string, unknown>) => ({
      id: "routine-created",
      squadId: "squad-1",
    ...
    const squadSkills = [
      {
        ...
        slug: "slaw",
        name: "slaw",
        description: "Slaw coordination skill",
        markdown: "---\nname: slaw\ndescription: Slaw coordination skill\n---\n\n# Slaw\n",
        sourceType: "github",
        sourceLocator: "https://github.com/slaw/slaw/tree/master/skills/slaw",
        ...
        metadata: {
          ...
          trackingRef: "master",
          repoSkillDir: "skills/slaw",
        },
      },
      {
        id: "skill-2",
        squadId: "squad-1",
        key: squadPlaybookKey,
        slug: "squad-playbook",
        name: "squad-playbook",
        description: "Internal squad skill",
        markdown: "---\nname: squad-playbook\ndescription: Internal squad skill\n---\n\n# Squad Playbook\n",
        sourceType: "local_path",
        sourceLocator: "/tmp/squad-playbook",
        sourceRef: null,
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [
    ...
    squadSkillSvc.list.mockResolvedValue(squadSkills);
    squadSkillSvc.listFull.mockResolvedValue(squadSkills);
    squadSkillSvc.readFile.mockImplementation(async (_squadId: string, skillId: string, relativePath: string) => {
      if (skillId === "skill-2") {
        return {
          skillId,
          path: relativePath,
    ...
    squadSkillSvc.importPackageFiles.mockResolvedValue([]);
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/slaw/squads?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      hostname: "github.com",
      owner: "slaw",
      repo: "squads",
      ref: "feature/demo",
      basePath: "gstack",
      squadPath: "gstack/SQUAD.md",
    });
  });

  it("parses canonical GitHub import URLs with explicit squadPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/slaw/squads?ref=abc123&squadPath=gstack%2FSQUAD.md",
      ),
    ).toEqual({
      hostname: "github.com",
      owner: "slaw",
  ...
  it("exports referenced skills as stubs by default with sanitized Slaw extension data", async () => {
    const portability = squadPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files["SQUAD.md"])).toContain('name: "Slaw"');
    expect(asTextFile(exported.files["SQUAD.md"])).toContain('schema: "agentsquads/v1"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${slawKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/slaw/slaw/slaw/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/slaw/slaw/slaw/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/slaw/slaw/slaw/references/api.md"]).toBeUndefined();
    expect(asTextFile(exported.files["skills/squad/PAP/squad-playbook/SKILL.md"])).toContain("# Squad Playbook");
    expect(asTextFile(exported.files["skills/squad/PAP/squad-playbook/references/checklist.md"])).toContain("# Checklist");

    const extension = asTextFile(exported.files[".slaw.yaml"]);
    expect(extension).toContain('schema: "slaw/v1"');
  ...
  it("exports hire approval policy only when approval is required", async () => {
    ...
    squadSvc.getById.mockResolvedValueOnce({
      id: "squad-1",
      name: "Slaw",
      description: null,
      issuePrefix: "PAP",
  ...
  it("exports default sidebar order into the Slaw extension and manifest", async () => {
    const portability = squadPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".slaw.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/slaw/slaw/slaw/SKILL.md"])).toContain("# Slaw");
    expect(asTextFile(exported.files["skills/slaw/slaw/slaw/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/slaw/slaw/slaw/references/api.md"])).toContain("# API");
  });

  it("exports catalog skill provenance in portable Slaw frontmatter", async () => {
    const portability = squadPortabilityService({} as any);
    const catalogKey = "slaw/bundled/software-development/review";
    ...
    const catalogSkill = {
      ...
      markdown: "---\nname: review\ndescription: Catalog review skill\n---\n\n# Review\n",
      sourceType: "catalog",
      sourceLocator: "/tmp/slaw/catalog/review",
    ...
    squadSkillSvc.listFull.mockResolvedValue([catalogSkill]);
    squadSkillSvc.readFile.mockImplementation(async (_squadId: string, skillId: string, relativePath: string) => ({
      skillId,
      path: relativePath,
      kind: relativePath === "SKILL.md" ? "skill" : "reference",
    ...
    const skillMarkdown = asTextFile(exported.files["skills/slaw/bundled/software-development/review/SKILL.md"]);
    expect(skillMarkdown).toContain("slaw:");
    expect(skillMarkdown).toContain("catalog:");
    expect(skillMarkdown).toContain(`sourceRef: "${originHash}"`);
    expect(skillMarkdown).toContain('catalogId: "slaw:bundled:software-development:review"');
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    const exported = await portability.exportBundle("squad-1", {
      ...
      skills: ["squad-playbook"],
    });

    expect(exported.files["skills/squad/PAP/squad-playbook/SKILL.md"]).toBeDefined();
    expect(asTextFile(exported.files["skills/squad/PAP/squad-playbook/SKILL.md"])).toContain("# Squad Playbook");
    expect(exported.files["skills/slaw/slaw/slaw/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/squad/PAP/squad-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/slaw/slaw/slaw/SKILL.md"]).toBeDefined();
  });

  it("exports the squad logo into images/ and references it from .slaw.yaml", async () => {
    const storage = {
    ...
    const portability = squadPortabilityService({} as any, storage as any);
    ...
    expect(storage.getObject).toHaveBeenCalledWith("squad-1", "assets/squads/logo-1");
    expect(exported.files["images/squad-logo.png"]).toEqual({
      encoding: "base64",
      data: Buffer.from("png-bytes").toString("base64"),
      contentType: "image/png",
    });
    expect(exported.files[".slaw.yaml"]).toContain('logoPath: "images/squad-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    squadSkillSvc.listFull.mockResolvedValue([
      {
        id: "skill-local",
        squadId: "squad-1",
      ...
      },
      {
        id: "skill-slaw",
        squadId: "squad-1",
        key: "slaw/slaw/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/slaw/slaw/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
    ...
    expect(asTextFile(exported.files["skills/slaw/slaw/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/slaw/slaw/release-changelog/SKILL.md"])).toContain("slaw/slaw/release-changelog");
  });
  ...
  it("builds export previews without tasks by default", async () => {
    ...
    const preview = await portability.previewExport("squad-1", {
      include: {
        squad: true,
        agents: true,
        projects: true,
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/slaw",
            repoUrl: "https://github.com/slaw/slaw.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .slaw-tmp",
            remoteProvider: null,
    ...
    expect(issueSvc.create).toHaveBeenCalledWith("squad-imported", expect.objectContaining({
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = squadPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "slaw-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/slaw/slaw.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".slaw.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/slaw/slaw.git"');
    expect(extension).toContain('projectWorkspaceKey: "slaw"');
  ...
  it("reads env inputs back from .slaw.yaml during preview import", async () => {
    const portability = squadPortabilityService({} as any);
  ...
  it("reads project env inputs back from .slaw.yaml during preview import", async () => {
    const portability = squadPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Slaw routine extensions", async () => {
    const portability = squadPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "slaw-demo", files },
      include: { squad: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_squad", newSquadName: "Imported Slaw" },
      agents: "all",
      collisionStrategy: "rename",
    });
    ...
    expect(routineSvc.create).toHaveBeenCalledWith("squad-imported", expect.objectContaining({
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "SQUAD.md": ['---', 'schema: "agentsquads/v1"', 'name: "Imported Slaw"', "---", ""].join("\n"),
  ...
  it("flags recurring task imports that are missing routine-required fields", async () => {
    ...
    const preview = await portability.previewImport({
      ...
      include: { squad: true, agents: false, projects: false, issues: true, skills: false },
      target: { mode: "new_squad", newSquadName: "Imported Slaw" },
      collisionStrategy: "rename",
    });
  ...
  it("imports a vendor-neutral package without .slaw.yaml", async () => {
    const portability = squadPortabilityService({} as any);
      ...
      source: {
        ...
        files: {
          "SQUAD.md": [
            ...
            'schema: "agentsquads/v1"',
            'name: "Imported Slaw"',
            'description: "Portable squad package"',
            "---",
            "",
            "# Imported Slaw",
            "",
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.squad?.name).toBe("Imported Slaw");
    ...
    expect(agentSvc.create).toHaveBeenCalledWith("squad-imported", expect.objectContaining({
      name: "ClaudeCoder",
      adapterType: "process",
    }));
  });
  ...
  it("preserves agent role from frontmatter when extension block omits it", async () => {
    ...
    const preview = await portability.previewImport({
      source: {
        type: "inline",
        rootPath: "squad_lead-package",
        files: {
          "SQUAD.md": [
            "---",
            'schema: "agentsquads/v1"',
            'name: "Squad Lead Role Test"',
            "---",
            "",
          ].join("\n"),
      ...
      target: { mode: "new_squad", newSquadName: "Squad Lead Role Test" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("imports packaged skills and restores desired skill refs on agents", async () => {
    ...
    expect(squadSkillSvc.importPackageFiles).toHaveBeenCalledWith("squad-imported", textOnlyFiles, {
      onConflict: "replace",
    });
    expect(agentSvc.create).toHaveBeenCalledWith("squad-imported", expect.objectContaining({
  ...
  it("imports a packaged squad logo and attaches it to the target squad", async () => {
    const storage = {
      putFile: vi.fn().mockResolvedValue({
        provider: "local_disk",
        objectKey: "assets/squads/imported-logo",
        contentType: "image/png",
        byteSize: 9,
        sha256: "logo-sha",
        originalFilename: "squad-logo.png",
      }),
    };
    squadSvc.create.mockResolvedValue({
      id: "squad-imported",
    ...
    squadSvc.update.mockResolvedValue({
      id: "squad-imported",
      name: "Imported Slaw",
    ...
    exported.files["images/squad-logo.png"] = {
      encoding: "base64",
      data: Buffer.from("png-bytes").toString("base64"),
      contentType: "image/png",
    };
    exported.files[".slaw.yaml"] = `${exported.files[".slaw.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
    ...
    expect(assetSvc.create).toHaveBeenCalledWith("squad-imported", expect.objectContaining({
      objectKey: "assets/squads/imported-logo",
      contentType: "image/png",
      createdByUserId: "user-1",
    }));
    expect(squadSvc.update).toHaveBeenCalledWith("squad-imported", {
      logoAssetId: "asset-created",
    });
  });

  it("copies source squad memberships for safe new-squad imports", async () => {
    const portability = squadPortabilityService({} as any);
    ...
    await portability.importBundle({
      ...
      mode: "agent_safe",
      sourceSquadId: "squad-1",
    });

    expect(accessSvc.listActiveUserMemberships).toHaveBeenCalledWith("squad-1");
    expect(accessSvc.copyActiveUserMemberships).toHaveBeenCalledWith("squad-1", "squad-imported");
    expect(accessSvc.ensureMembership).not.toHaveBeenCalledWith("squad-imported", "user", expect.anything(), "owner", "active");
    const textOnlyFiles = Object.fromEntries(Object.entries(exported.files).filter(([, v]) => typeof v === "string"));
    expect(squadSkillSvc.importPackageFiles).toHaveBeenCalledWith("squad-imported", textOnlyFiles, {
      onConflict: "rename",
    });
  ...
  it("disables timer heartbeats on imported agents", async () => {
    ...
    agentSvc.create.mockImplementation(async (_squadId: string, input: Record<string, unknown>) => ({
  ...
  it("imports only selected files and leaves unchecked squad metadata alone", async () => {
    const portability = squadPortabilityService({} as any);
    ...
    expect(squadSvc.update).not.toHaveBeenCalled();
    expect(squadSkillSvc.importPackageFiles).toHaveBeenCalledWith(
      "squad-1",
      expect.objectContaining({
        "SQUAD.md": expect.any(String),
    ...
    expect(agentSvc.create).toHaveBeenCalledWith("squad-1", expect.objectContaining({
      name: "CMO",
      runtimeConfig: {
        heartbeat: {
          enabled: false,
    ...
    expect(result.squad.action).toBe("unchanged");
  ...
  it("does not implicitly add local adapter permission bypass defaults on import", async () => {
    ...
    expect(agentSvc.create).toHaveBeenLastCalledWith("squad-imported", expect.objectContaining({
      adapterType: "codex_local",
      adapterConfig: expect.objectContaining({
  ...
  it("preserves issue labelIds through export and import round-trip", async () => {
    ...
    const exported = await portability.exportBundle("squad-1", {
      include: { squad: true, agents: false, projects: true, issues: true },
    });
    ...
    squadSvc.create.mockResolvedValue({ id: "squad-imported", name: "Imported" });
    accessSvc.ensureMembership.mockResolvedValue(undefined);
    agentSvc.list.mockResolvedValue([]);
    projectSvc.list.mockResolvedValue([]);
    ...
    await portability.importBundle({
      ...
      include: { squad: true, agents: false, projects: true, issues: true },
      target: { mode: "new_squad", newSquadName: "Imported" },
      agents: "all",
      collisionStrategy: "rename",
    }, "user-1");
  ...
  it("preserves issue comment presentation fields through export and import", async () => {
    ...
    const exported = await portability.exportBundle("squad-1", {
      include: { squad: true, agents: false, projects: false, issues: true },
    });
  ...
  it("nameOverrides applied after collision detection do not re-validate uniqueness", async () => {
    ...
    const exported = await portability.exportBundle("squad-1", {
      include: { squad: false, agents: true, projects: false, issues: false },
    });
  ...
  it("resolves issue assignee to existing agent when agent is skipped", async () => {
    ...
    projectSvc.list.mockResolvedValue([{
      id: "project-1", squadId: "squad-1", name: "TestProject", urlKey: "testproject",
    ...
    issueSvc.list.mockResolvedValue([{
      id: "issue-1", squadId: "squad-1", title: "Test task", identifier: "PAP-1",
    ...
    const exported = await portability.exportBundle("squad-1", {
      include: { squad: false, agents: true, projects: true, issues: true },
    });

    // Re-import into same squad with skip collision strategy
    ...
    projectSvc.list.mockResolvedValue([]);
    issueSvc.list.mockResolvedValue([]);
    projectSvc.create.mockResolvedValue({ id: "project-new", squadId: "squad-1", urlKey: "testproject" });
  ...
  it("handles a package with only skills (no agents or projects)", async () => {
    ...
    const exported = await portability.exportBundle("squad-1", {
      include: { squad: false, agents: false, projects: false, issues: false, skills: true },
      expandReferencedSkills: true,
    });
  ...
  it("preview import detects no agents to import when agents are excluded", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: exported.rootPath, files: exported.files },
      include: { squad: false, agents: false, projects: false, issues: false },
      target: { mode: "existing_squad", squadId: "squad-1" },
      agents: "all",
      collisionStrategy: "rename",
    });
