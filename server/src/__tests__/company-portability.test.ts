...
describe("company portability", () => {
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/slaw/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      hostname: "github.com",
      owner: "slaw",
      repo: "companies",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/slaw/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      hostname: "github.com",
      owner: "slaw",
  ...
  it("exports referenced skills as stubs by default with sanitized Slaw extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Slaw"');
  ...
  it("exports the company logo into images/ and references it from .slaw.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".slaw.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      ...
      target: { mode: "new_company", newCompanyName: "Imported Slaw" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Slaw"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .slaw.yaml", async () => {
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Slaw");
