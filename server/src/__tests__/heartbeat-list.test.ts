...
import { agents, squads, createDb, heartbeatRuns } from "@slaw-ai/db";
import {
...
describeEmbeddedPostgres("heartbeat list", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("slaw-heartbeat-list-");
  ...
  it("returns runs even when the linked db schema lacks processGroupId", async () => {
    ...
    try {
      const runs = await heartbeatService(db).list(squadId, agentId, 5);
