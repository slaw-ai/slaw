...
import { agents, squads, squadSkills, createDb } from "@slaw/db";
import {
  getEmbeddedPostgresTestSupport,
  startEmbeddedPostgresTestDatabase,
} from "./helpers/embedded-postgres.js";
import { squadSkillService } from "../services/squad-skills.ts";
...
if (!embeddedPostgresSupport.supported) {
  console.warn(
    `Skipping embedded Postgres squad skill service tests on this host: ${embeddedPostgresSupport.reason ?? "unsupported environment"}`,
  );
}

describeEmbeddedPostgres("squadSkillService.list", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("slaw-squad-skills-service-");
    oldSlawHome = process.env.SLAW_HOME;
    slawHome = await fs.mkdtemp(path.join(os.tmpdir(), "slaw-squad-skills-home-"));
  ...
  it("rejects skill inventory refresh for a missing squad", async () => {
  ...
  it("does not persist audit failures for remote-source skills", async () => {
    ...
    await expect(svc.auditSkill(squadId, skillId)).rejects.toMatchObject({
      status: 422,
      message: "Only local-path and catalog-managed squad skills support audit.",
    });
    await expect(svc.getById(squadId, skillId)).resolves.toMatchObject({
      metadata: { sourceKind: "github", owner: "acme", repo: "remote-skill" },
    });
  });
  ...
  it("preserves missing local-path skills that active agents still desire", async () => {
    ...
    const detail = await svc.detail(squadId, skillId);
    const stored = await svc.getById(squadId, skillId);
  ...
  it("continues pruning missing local-path skills that no active agent desires", async () => {
    ...
    await db.insert(squadSkills).values({
      id: skillId,
      squadId,
      key: `squad/${squadId}/unused-skill`,
      slug: "unused-skill",
      name: "Unused Skill",
      description: null,
    ...
    await expect(svc.getById(squadId, skillId)).resolves.toBeNull();
  });
  ...
  it("clears the missing-source marker when a local-path skill source returns", async () => {
    ...
    await db.insert(squadSkills).values({
      id: skillId,
      squadId,
      key: `squad/${squadId}/restored-skill`,
      slug: "restored-skill",
      name: "Restored Skill",
      description: null,
    ...
    await svc.list(squadId);
    const stored = await svc.getById(squadId, skillId);

    expect(stored?.metadata).toEqual({ sourceKind: "local_path" });
  });

  it("marks source-missing squad skills as unavailable during read-only runtime listing", async () => {
    const squadId = randomUUID();
    const skillId = randomUUID();
    const skillKey = `squad/${squadId}/reflection-coach`;
    ...
    const entries = await svc.listRuntimeSkillEntries(squadId, { materializeMissing: false });
  ...
  it("materializes source-missing squad skills from the stored markdown during runtime listing", async () => {
    const squadId = randomUUID();
    const skillId = randomUUID();
    const skillKey = `squad/${squadId}/runtime-coach`;
    ...
    const entries = await svc.listRuntimeSkillEntries(squadId);
    const entry = entries.find((candidate) => candidate.key === skillKey);
