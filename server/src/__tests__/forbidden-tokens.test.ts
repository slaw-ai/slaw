...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "slaw", LOGNAME: "slaw", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "slaw" }),
      },
    );

    expect(tokens).toEqual(["slaw", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "slaw" }, {
        userInfo: () => ({ username: "slaw" }),
      });

      expect(tokens).toEqual(["slaw", "custom-token"]);
    } finally {
