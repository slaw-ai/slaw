...
- **Issue search in skills** — The Slaw skill reference now documents the `q=` search parameter for finding issues by keyword.

## Fixes

- **Markdown list rendering** — Fixed list markers (`-`, `*`) not rendering correctly in the editor and comment views.
- **Archived squads hidden** — The new issue dialog no longer shows archived squads in the squad selector.
