...
Maintainer runbook for shipping Slaw across npm, GitHub, and the website-facing changelog surface.
...
Slaw uses calendar versions that still fit semver syntax:
...
2. **npm** — `slaw` and public workspace packages are published
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Slaw. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
SLAW_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

For the current stable:

```bash
SLAW_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
```bash
gh workflow run release-smoke.yml -f slaw_version=canary
gh workflow run release-smoke.yml -f slaw_version=latest
```
