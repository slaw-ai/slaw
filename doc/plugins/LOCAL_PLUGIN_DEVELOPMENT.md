...
This is the short happy-path guide for developing a Slaw plugin from a folder on your machine. You will scaffold a plugin, run it in watch mode, install it into a running Slaw instance from an absolute local path, and edit code with the plugin worker reloading after each rebuild.
...
- Node.js 22+ and `pnpm`.
- A local Slaw checkout you can run from source. Local plugin installs read source from disk, so the running server must be able to see the path you give it.

## The five steps

```bash
# 1. Start Slaw locally
pnpm slaw run

# 2. Scaffold a plugin outside the Slaw repo
...
slaw plugin install ~/dev/slaw-plugins/hello-plugin

# 5. Confirm it loaded
slaw plugin list
slaw plugin inspect acme.hello-plugin
```
...
Slaw listens on `http://127.0.0.1:3100` by default. The CLI talks to that server, so leave it running.
...
If your plugin has UI and you want a browser-side dev server with hot module replacement during local UI iteration, run `pnpm dev:ui` in a second terminal. It serves `dist/ui/` on `http://127.0.0.1:4177`. This is optional; Slaw can load the built UI directly from `dist/ui/` without it.
...
```
Installing plugin from local path: /Users/you/dev/slaw-plugins/hello-plugin
✓ Installed acme.hello-plugin v0.1.0 (ready)
Local plugin installs run trusted local code from your machine.
Keep `pnpm dev` running in /Users/you/dev/slaw-plugins/hello-plugin;
Slaw watches rebuilt dist output and reloads the plugin worker.
```

Relative paths are resolved against the current working directory, so `slaw plugin install .` from inside the plugin folder works too.
...
Slaw watches the on-disk plugin package after a local install. The watcher targets the runtime entrypoints declared in the package's `slawPlugin` field (`dist/manifest.js`, `dist/worker.js`, `dist/ui/`).

What that means in practice:

- **Worker code:** save a `.ts` file → esbuild rewrites `dist/worker.js` → Slaw debounces ~500ms and restarts the plugin worker. The next worker call uses the new code. There is no in-process hot module replacement for worker code; it is a worker restart.
- **Manifest:** save `src/manifest.ts` → `dist/manifest.js` rewrites → the worker restarts and the host re-reads the manifest.
- **Plugin UI:** save a `.tsx` file → esbuild rewrites `dist/ui/` → Slaw reloads the UI bundle on its next mount. To get HMR during UI iteration, run `pnpm dev:ui` and point at the dev server with `devUiUrl` in your manifest while developing.
- **Without `pnpm dev`:** the watcher only fires on `dist/*` changes. If you stop the watch build, source edits do not reach Slaw. Restart `pnpm dev` (or run `pnpm build` once) before expecting changes.
- **`node_modules`, `.git`, `.slaw-sdk`, and other dotfolders are ignored.** Adding a dependency requires the new code to actually be imported and rebuilt before the worker sees it.
...
- **Local path plugins are trusted local code.** Slaw executes worker code from disk under the same trust boundary as the rest of the running instance. This is meant for developing or operating a plugin against a checkout you control. There is no signature check, no sandboxing of worker code, and no provenance metadata beyond the path. Do not install local-path plugins you did not write.
- **npm packages are the deployable artifact.** `slaw plugin install @acme/plugin-foo` (optionally `--version 1.2.3`) installs from your configured npm registry, version-pins, and produces an install record that other operators can reproduce. Ship plugins this way.
...
- **`Plugin install returned no plugin record` or `error` status.** Run `slaw plugin inspect <key>` for the last error. The most common causes are (1) the plugin has not built yet — run `pnpm dev` or `pnpm build` first, (2) the `slawPlugin` entries in `package.json` point at files that do not exist on disk, or (3) the manifest failed validation. The Slaw server log has the full validation error.
- **Edits do not seem to reload.** Confirm `pnpm dev` is still running and writing to `dist/`. If you renamed entry files, update the `slawPlugin.manifest` / `slawPlugin.worker` / `slawPlugin.ui` fields in `package.json` so the watcher targets them.
