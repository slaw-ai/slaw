...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@slaw/plugin-sdk/ui`) with design tokens and common components.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@slaw/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@slaw/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Slaw instance.
...
- **Single SDK package**: `@slaw/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@slaw/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
```ts
import { definePlugin, z } from "@slaw/plugin-sdk";

export default definePlugin({
  id: "@slaw/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ...
    ctx.events.on("plugin.@slaw/plugin-git.push-detected", async (event) => {
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@slaw/plugin-linear`)
...
Package idea: `@slaw/plugin-workspace-files`
...
Package idea: `@slaw/plugin-terminal`
...
- optional `events.emit` (e.g. `plugin.@slaw/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@slaw/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@slaw/plugin-linear`
...
Package idea: `@slaw/plugin-github-issues`
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@slaw/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@slaw/plugin-grafana`
...
Package idea: `@slaw/plugin-runtime-processes`
...
Package idea: `@slaw/plugin-stripe`
...
- plugin UI bundle loading, host bridge, `@slaw/plugin-sdk/ui`
...
- `@slaw/plugin-test-harness` and `create-slaw-plugin` starter template
...
8. Ship `@slaw/plugin-test-harness` and `create-slaw-plugin` starter template.
