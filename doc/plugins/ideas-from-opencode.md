...
That model works well for a local coding tool. It should not be copied literally into Slaw.

The main conclusion is:

- Slaw should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Slaw should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Slaw should use multiple extension classes instead of one generic plugin bag:
...
If Slaw does this well, the examples you listed become straightforward:
...
Relevant Slaw files reviewed for current extension seams:
...
It is too dangerous for Slaw core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Slaw — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Slaw should absolutely do this.
...
Slaw should copy this discipline.
...
Slaw should adopt this style for plugin actions, automations, and UI schemas.
...
Slaw can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Slaw too.

## What Slaw Should Not Copy Directly
...
Slaw is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Slaw should not auto-load arbitrary code from a workspace repo like `.slaw/plugins` or project directories.
...
For Slaw, equivalent hooks into:
...
Slaw should not allow plugins to silently replace:
...
For Slaw it would be risky because it combines:
...
Slaw should require an explicit operator install step.

## Why Slaw Needs A Different Shape
...
That means Slaw should borrow the good ideas from `opencode` but use a stricter architecture.

## Slaw Already Has Useful Pre-Plugin Seams

Slaw has several extension-like seams already:
...
This is good news.
Slaw does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Slaw Plugin Model
...
Slaw already has this pattern implicitly:
...
- `plugin` means integration code that talks to Slaw through a typed plugin protocol and capability model
...
- invoke explicit Slaw actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@slaw-ai/plugin-sdk/ui`) with design tokens and common components.
...
Slaw should treat plugin installation as a global instance-level action.

Examples:

- install `@slaw-ai/plugin-linear` once
...
Slaw already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Slaw too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@slaw-ai/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@slaw-ai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Slaw instance.

A `create-slaw-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Slaw achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Slaw server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Slaw's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@slaw-ai/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@slaw-ai/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Slaw

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@slaw-ai/plugin-sdk";

export default definePlugin({
  id: "@slaw-ai/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Slaw entities
    });
    ...
    ctx.events.on("plugin.@slaw-ai/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@slaw-ai/plugin-sdk/ui";
...
Any Slaw plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Slaw objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@slaw-ai/plugin-linear`)
...
- reuse core tables when the data is clearly part of Slaw itself
...
If a concept is becoming part of Slaw's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Slaw itself
...
### 3. Scope plugin data to Slaw objects before adding custom schemas

A lot of plugin data naturally hangs off existing Slaw objects:
...
If Slaw eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@slaw-ai/plugin-workspace-files`
...
Package idea: `@slaw-ai/plugin-terminal`
...
Package idea: `@slaw-ai/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Slaw.
- Board reviews the diff after a run without leaving Slaw.
...
- optional `events.emit` (e.g. `plugin.@slaw-ai/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@slaw-ai/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@slaw-ai/plugin-linear`

This plugin syncs Slaw work with Linear. It is useful for:
...
- Plugin settings:
  ...
  - status mapping between Slaw and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Slaw.
- Slaw issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@slaw-ai/plugin-github-issues`

This plugin syncs Slaw issues with GitHub Issues and optionally links PRs. It is useful for:
...
- Plugin settings:
  ...
  - whether new Slaw issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Slaw issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Slaw.
- GitHub webhooks update status/comment state in Slaw.
- A PR is linked back to the Slaw issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@slaw-ai/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@slaw-ai/plugin-grafana`
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Slaw plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Slaw dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Slaw issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Slaw core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@slaw-ai/plugin-runtime-processes`
...
Package idea: `@slaw-ai/plugin-stripe`
...
- Stripe data should stay additive to Slaw core
...
- plugin UI bundle loading, host bridge, `@slaw-ai/plugin-sdk/ui`
...
- `@slaw-ai/plugin-test-harness` and `create-slaw-plugin` starter template
...
Slaw should not implement "an OpenCode-style generic in-process hook system."
Slaw should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Slaw
...
8. Ship `@slaw-ai/plugin-test-harness` and `create-slaw-plugin` starter template.
