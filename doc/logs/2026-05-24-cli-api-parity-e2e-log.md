...
Full Slaw CLI/API parity smoke pass against a disposable local source-tree instance.

## Isolation Contract

- Repo: `/Users/aronprins/Documents/Slaw/slaw`
- Scratch root: `/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity`
- `SLAW_HOME`: `/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/home`
- `SLAW_INSTANCE_ID`: `cli-api-parity`
- `SLAW_CONFIG`: `/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`
- `SLAW_CONTEXT`: `/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/home/context.json`
- `SLAW_AUTH_STORE`: `/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/home/auth.json`
- `SLAW_API_URL`: `http://127.0.0.1:3197`
- `SLAW_SERVER_PORT`: `3197`
- `PORT`: `3197`
- `CODEX_HOME`: `/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/codex-home`
- `CLAUDE_HOME`: `/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/claude-home`
- `DATABASE_URL`: unset
...
- Command: `sed -n ... slaw-localdev-runbook.md`, `doc/DEVELOPING.md`, `doc/CLI.md`, `doc/DATABASE.md`, `doc/GOAL.md`, `doc/PRODUCT.md`, `doc/SPEC-implementation.md`, `doc/DEPLOYMENT-MODES.md`, `doc/plans/2026-05-23-cli-api-parity-openapi-reference.ts`
...
- Actual result: Runbook requires explicit scratch paths, port `3197`, unset database env vars, `pnpm slaw onboard --yes --run --bind loopback`, and pre-test isolation checks.
- Status: PASS.
...
- Prerequisites/IDs used: none.
- Expected result: All Slaw/Codex/Claude paths point under `/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity`; `DATABASE_URL` and `DATABASE_MIGRATION_URL` are unset; port `3197` has no listener.
...
- Status: PASS.
- Output summary: No references to `~/.slaw`, `~/.codex`, `~/.claude`, or `localhost:3100`.
- Follow-up: Start Slaw with the runbook command.

### 2026-05-24T11:06:45+02:00 - Start isolated instance

- Command: `env -u DATABASE_URL -u DATABASE_MIGRATION_URL ... pnpm slaw onboard --yes --run --bind loopback`
- Purpose: Create and start the disposable source-tree Slaw instance.
...
- Actual result: Server started in `local_trusted/private` mode on `127.0.0.1:3197`, with embedded PostgreSQL data at `/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/home/instances/cli-api-parity/db`.
- Status: PASS.
...
- Command: `pnpm slaw env`; `pnpm slaw context show --json`; `curl -sS http://127.0.0.1:3197/api/health`; `find tmp/cli-api-parity/home -maxdepth 4 -type d`
...
- Output summary: The assigned `process` adapter agent ran automatically and generated heartbeat runs. The issue later moved to `blocked` via recovery handling because the smoke process printed output without a concrete Slaw disposition.
...
- Command: `kill <slaw pid on 3197>`; `pnpm slaw onboard --yes --run --bind loopback`; `curl http://127.0.0.1:3197/api/health`; `issue tree-hold:get <issue-id> null --json`
...
- Status: PASS.
- Output summary: No real `~/.slaw`, `~/.codex`, or `~/.claude` paths were used. The server session is currently running under the isolated environment.
...
- Expected result: `slaw health` exists; `slaw access whoami` works; `invite test-resolution` has a URL option; `join list --status pending` maps to `pending_approval`; issue help text no longer overstates valid cancel/recovery inputs.
...
- Prerequisites/IDs used: Isolated `CLAUDE_HOME=/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/claude-home`.
...
- Status: PASS.
- Output summary: Mounted `llmRoutes` under `/api` in addition to the existing root mount. Updated available-skill discovery to read `CLAUDE_HOME/skills` when configured, include built-in Slaw repo skills in `available-skill list`, and allow `available-skill get` for safe listed/built-in skill names.
...
- Command: `pnpm slaw llm agent-configuration --json`; `pnpm slaw llm agent-icons --json`; `pnpm slaw llm agent-configuration:adapter process --json`; `pnpm slaw available-skill list --json`; `pnpm slaw available-skill get slaw --json`; `pnpm slaw available-skill get cmux --json`; `pnpm slaw openapi --json`
...
- Expected result: LLM docs commands pass; built-in Slaw skills are listed and fetchable; real-user `~/.claude` skills are not listed; `openapi` still documents the unresolved gap if no route exists.
- Actual result: LLM docs commands passed. `available-skill list` returned Slaw repo skills such as `diagnose-why-work-stopped`; `available-skill get slaw` returned markdown. `available-skill get cmux` now returns 404 because `cmux` is no longer listed from the real Claude home. `openapi` still returns `404: API route not found`.
- Status: MIXED.
...
- Follow-up: Stop the fake upstream server. Keep the real scratch Slaw server running on `127.0.0.1:3197`.
...
- Command: Edited `cli/src/commands/client/agent.ts`; ran `pnpm exec vitest run cli/src/__tests__/agent-lifecycle.test.ts`; `pnpm --dir cli typecheck`; `pnpm --silent slaw agent instructions-path:update --help`.
...
- Command: Edited `server/src/services/plugin-tool-dispatcher.ts`, `server/src/services/plugin-loader.ts`, and `server/src/__tests__/plugin-database.test.ts`; ran `pnpm exec vitest run server/src/__tests__/plugin-database.test.ts`; `pnpm --dir server typecheck`; restarted the isolated server; installed kitchen-sink plugin; `plugin tools`; `plugin tool:execute --payload-json '{"tool":"slaw-kitchen-sink-example:echo",...}'`; cleanup `plugin uninstall --force`.
...
- Prerequisites/IDs used: Same scratch server and kitchen-sink plugin; tool `slaw-kitchen-sink-example:echo`.
- Expected result: The listed echo tool dispatches to the running kitchen-sink worker and returns the echo result.
- Actual result: Focused test and server typecheck passed. After restart, `plugin tools` listed `slaw-kitchen-sink-example:echo`; `plugin tool:execute` returned `content: "CLI parity tool after fix"` and the expected run context. The plugin was uninstalled and `plugin list` returned `[]`.
...
- Final paths:
  - Scratch home: `/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/home`
  - Config: `/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`
  - Context: `/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/home/context.json`
  - Auth store: `/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/home/auth.json`
...
```sh
export SLAW_HOME=/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/home
export SLAW_INSTANCE_ID=cli-api-parity
export SLAW_CONFIG=/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/home/instances/cli-api-parity/config.json
export SLAW_CONTEXT=/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/home/context.json
export SLAW_AUTH_STORE=/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/home/auth.json
export SLAW_API_URL=http://127.0.0.1:3197
export SLAW_SERVER_PORT=3197
export PORT=3197
export CODEX_HOME=/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/codex-home
export CLAUDE_HOME=/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/claude-home
unset DATABASE_URL DATABASE_MIGRATION_URL
pnpm slaw health --json
```
...
- Command: `pnpm slaw health --json`; `pnpm slaw openapi --json`; `lsof -nP -iTCP:3197 -sTCP:LISTEN`; `git status --short --branch`, all with the scratch `SLAW_*`, `CODEX_HOME`, and `CLAUDE_HOME` environment and with `DATABASE_URL`/`DATABASE_MIGRATION_URL` unset.
...
- Command: Stopped the foreground scratch server process; started the same runbook command in detached screen session `slaw-cli-parity`; verified `pnpm slaw health --json`; checked `lsof -nP -iTCP:3197 -sTCP:LISTEN`; checked `screen -ls`.
- Purpose: Leave the disposable instance running without tying it to the active tool session.
- Prerequisites/IDs used: Same scratch env and unset database variables; detached screen session `91568.slaw-cli-parity`.
- Expected result: Server continues running on non-default port `3197` with the same isolated home/config/context/auth paths.
- Actual result: Health returned `status:"ok"` with deployment mode `local_trusted`; `node` PID `91583` is listening on `127.0.0.1:3197`; `screen -ls` shows detached session `91568.slaw-cli-parity`.
- Status: PASS.
- Output summary: Detached server log is `tmp/cli-api-parity/artifacts/caveat-followup/server-screen.log`.
- Follow-up: Manual continuation can use the same env block and `screen -r slaw-cli-parity` to inspect the server session.
...
- Reproduction command: `pnpm slaw configure --config /Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/home/instances/cli-api-parity/config.json --section invalid-section`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/context.test.ts`; `pnpm --dir cli typecheck`; `pnpm slaw context show --json`.
...
- Reproduction command: `pnpm slaw access whoami --json`.
...
- Fix summary: Added `slaw access whoami` as an alias for the existing top-level `whoami` command.
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm slaw access whoami --json`.
- Remaining risk: Low.
...
- Reproduction command: `pnpm slaw issue interaction:accept <issue-id> <request-confirmation-interaction-id> --json`.
...
- Reproduction command: `pnpm slaw issue tree-hold:get <issue-id> null --json` or `pnpm slaw issue tree-hold:release <issue-id> null --json`.
...
- Reproduction command: `pnpm slaw issue interaction:cancel <issue-id> <request-confirmation-interaction-id> --reason "..." --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/issue-subresources.test.ts`; `pnpm --dir cli typecheck`; `pnpm slaw issue interaction:cancel --help`.
...
- Status: Fixed help text.
- Severity: Low command UX drift.
- Reproduction command: `pnpm slaw issue recovery:resolve <issue-id> --action-id <action-id> --outcome restored --source-issue-status blocked --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/issue-subresources.test.ts`; `pnpm --dir cli typecheck`; `pnpm slaw issue recovery:resolve --help`.
...
- Severity: Low command UX drift.
- Reproduction command: `pnpm slaw agent instructions-path:update <process-agent-id> --payload-json '{"path":"docs/cli-parity.md"}' --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/agent-lifecycle.test.ts`; `pnpm --dir cli typecheck`; `pnpm slaw agent instructions-path:update --help`.
...
- Reproduction command: `pnpm slaw invite test-resolution <invite-token> --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm slaw invite test-resolution <token> --url https://example.com/invite/<token> --json`.
...
- Reproduction command: `pnpm slaw openapi --json`; `pnpm slaw available-skill get cmux --json`; `pnpm slaw llm agent-configuration --json`; `pnpm slaw llm agent-icons --json`; `pnpm slaw llm agent-configuration:adapter process --json`.
...
- Fix summary: Mounted LLM docs routes under `/api`; made available-skill discovery honor `CLAUDE_HOME`, include built-in Slaw repo skills, and fetch safe skill markdown consistently; added `/api/openapi.json`, then upgraded it from the initial path inventory to the schema-backed `OpenAPIRegistry`/`OpenApiGeneratorV3` implementation from the parity reference.
- Verification command: `pnpm exec vitest run server/src/__tests__/llms-routes.test.ts cli/src/__tests__/access-parity.test.ts`; `pnpm --dir server typecheck`; `pnpm --dir cli typecheck`; live `llm` and `available-skill` commands after restart; `pnpm exec vitest run server/src/__tests__/openapi-routes.test.ts`; live `curl http://127.0.0.1:3197/api/openapi.json`; live `pnpm slaw openapi --json`; follow-up live schema-backed `slaw openapi --json`.
...
- Reproduction command: `CLAUDE_HOME=/Users/aronprins/Documents/Slaw/slaw/tmp/cli-api-parity/claude-home pnpm slaw available-skill list --json`.
- Expected result: Skill discovery uses the isolated Claude home or built-in repo skills only.
- Actual result: Before the fix, the list included `cmux` from the real user Claude skills home, and `available-skill get cmux` failed because only a hardcoded Slaw subset was fetchable.
- Suspected cause: Server code read `HOME/.claude/skills` directly and did not add built-in Slaw skills unless they were present in Claude's skills directory.
- Files changed: `server/src/routes/access.ts`, `doc/bugs/2026-05-24-cli-api-parity-e2e-log.md`.
- Fix summary: Use `CLAUDE_HOME/skills` when `CLAUDE_HOME` is set, include built-in Slaw skills in catalog output, and resolve safe skill markdown from both Claude and Slaw skills directories.
- Verification command: live `available-skill list`, `available-skill get slaw`, and `available-skill get cmux` after restarting the isolated server.
- Remaining risk: Low; this is runtime environment-sensitive and covered by live isolated verification.

### MISMATCH-008 - `slaw health` is not registered

- Status: Fixed and live-verified.
- Severity: Low command/API parity gap.
- Reproduction command: `pnpm slaw health --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm slaw health --json`.
- Remaining risk: Low.
...
- Status: Fixed and live-verified.
- Severity: Medium CLI/API parity gap.
- Reproduction command: `pnpm slaw secrets --help` did not expose commands for `PATCH /api/secrets/:id`, `POST /api/secrets/:id/rotate`, `GET /api/secrets/:id/usage`, `GET /api/secrets/:id/access-events`, or `DELETE /api/secrets/:id`.
