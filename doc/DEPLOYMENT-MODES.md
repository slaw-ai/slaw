...
Slaw supports two runtime modes:
...
Slaw now treats **bind** as a separate concern from auth:
...
pnpm slaw onboard --yes --bind lan
pnpm slaw run --bind tailnet
```
...
When running `authenticated` mode, if the only instance admin is `local-board`, Slaw emits a startup warning with a one-time high-entropy claim URL.
...
For `authenticated/private`, Slaw supports a browser-first setup path:

1. open the Slaw URL from the private network or appliance UI
2. sign in or create a Slaw account
