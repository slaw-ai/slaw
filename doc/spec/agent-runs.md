...
This spec defines how Slaw actually runs agents while staying runtime-agnostic.
...
1. Slaw is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Slaw manages lifecycle, and on exit Slaw parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `slaw.skill` (shared Slaw skill text block)
...
2. Preferred transport is env vars (`SLAW_*`) injected at runtime.
