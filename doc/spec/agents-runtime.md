...
For `claude_local` and `codex_local`, Slaw assumes the CLI is already installed and authenticated on the host machine.
...
Slaw stores resumable session state per `(agent, taskKey, adapterType)`.
