...
Target specification for the Slaw control plane. Living document — updated incrementally during spec interviews.
...
#### Slaw Protocol (What Slaw Knows)

At the protocol level, Slaw tracks:
...
The minimum requirement to be a Slaw agent: **be callable.** That's it. Slaw can invoke you via command or webhook. No requirement to report back — Slaw infers basic status from process liveness when it can.

#### Integration Levels

Beyond the minimum, Slaw provides progressively richer integration:

1. **Callable** (minimum) — Slaw can start you. That's the only contract.
...
Slaw ships **default agents** that demonstrate full integration: progress tracking, cost instrumentation, and a **Slaw skill** (a Claude Code skill for interacting with the Slaw API) for task management. These serve as both useful defaults and reference implementations for adapter authors.
...
Agents can create tasks and assign them to agents outside their reporting line. This is the mechanism for cross-team collaboration. These rules are primarily encoded in the Slaw SKILL.md which is recommended for all agents. Slaw the app enforces the tooling and some light governance, but the cross-team rules below are mainly implemented by agent decisions.
...
The heartbeat is a protocol, not a runtime. Slaw defines how to initiate an agent's cycle. What the agent does with that cycle — how long it runs, whether it's task-scoped or continuous — is entirely up to the agent.

### Execution Adapters

Agent configuration includes an **adapter** that defines how Slaw invokes the agent. Built-in adapters include:
...
This is the full adapter contract. `invoke` starts the agent, `status` lets Slaw check on it, `cancel` enables the board's pause functionality. Everything else (cost reporting, task updates) is optional and flows through the Slaw REST API.
...
- Health monitoring — how does Slaw distinguish "stuck" from "working on a long task"?
...
Token/LLM cost budgeting is a core part of Slaw. External revenue and expense tracking is a future plugin.

### Cost Reporting

Fully-instrumented Agents report token/API usage back to Slaw. Costs are tracked at every level:
...
A skill definition that teaches agents how to interact with Slaw. Provides:
...
When a user creates an Agent, Slaw generates a **connection string** containing: the server URL, an API key, and instructions for how to authenticate. The Agent is assumed to be capable of figuring out how to call the API with its token/key from there.

Flow:

1. Human creates an Agent in the UI
2. Slaw generates a connection string (URL + key + instructions)
...
Slaw manages task-linked work artifacts: issue documents (rich-text plans, specs, notes attached to issues) and file attachments. Agents read and write these through the API as part of normal task execution. Full delivery infrastructure (code repos, deployments, production runtime) remains the agent's domain — Slaw orchestrates the work, not the build pipeline.
...
When an agent crashes or disappears mid-task, Slaw does **not** auto-reassign or auto-release the task. Instead:

- Slaw surfaces stale tasks (tasks in `in_progress` with no recent activity) through dashboards and reporting
- Slaw does not fail silently — the auditing and visibility tools make problems obvious
...
**Principle: Slaw reports problems, it doesn't silently fix them.** Automatic recovery hides failures. Good visibility lets the right entity (human or agent) decide what to do.

### Plugin / Extension Architecture

The core Slaw system must be extensible. Features like knowledge bases, external revenue tracking, and new Agent Adapters should be addable as **plugins** without modifying core. This means:
...
The plugin framework has shipped. Plugins can register new adapter types, hook into lifecycle events, and contribute UI components (e.g. global toolbar buttons). A plugin SDK and CLI commands (`slaw plugin`) are available for authoring and installing plugins.
...
**Full loop with one adapter.** V1 must demonstrate the complete Slaw cycle end-to-end, even if narrow.
...
**Anti-goal for core.** The knowledge base is not part of the Slaw core — it will be a plugin. The task system + comments + agent descriptions provide sufficient shared context.
...
Things Slaw explicitly does **not** do:

- **Not an Agent runtime** — Slaw orchestrates, Agents run elsewhere
