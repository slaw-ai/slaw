...
Agents need API keys to authenticate with Slaw. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Slaw
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Slaw generates a token and passes it directly to the agent
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Slaw CLI
...
```
slaw auth login
# Opens browser -> user confirms -> token stored at ~/.slaw/credentials
```
...
managed by the Slaw server itself.
...
5. Slaw stores the pending registration.
...
7. On approval, Slaw provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Slaw issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.slaw.ing/skills/slaw/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Slaw should send you heartbeats",
