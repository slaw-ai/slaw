# Squad Lead Agent Creation and Hiring Governance Plan (V1.1)
...
Enable a Squad Lead agent to create new agents directly, with lightweight but explicit governance:

- Squad-level toggle: new hires require board approval (default ON).
- Agent-level permission: `can_create_agents` (default ON for Squad Lead, OFF for everyone else).
...
- Agent creation is board-only at `POST /api/squads/:squadId/agents` (`server/src/routes/agents.ts`).
- Approvals support `pending/approved/rejected/cancelled` and `hire_agent` + `approve_squad_lead_strategy` (`packages/shared/src/constants.ts`, `server/src/services/approvals.ts`).
...
- There is no squad setting for "new hires require board approval".
...
- Default: `true`
- Editable only in squad advanced settings (not onboarding/squad creation flow UI)
...
- Squad Lead can edit permissions for agents in same squad.
...
- `packages/db/src/schema/squads.ts`
- `packages/shared/src/types/squad.ts`
- `packages/shared/src/validators/squad.ts`
- UI squad API type usage and squad advanced settings form
...
- `id`, `squad_id`, `approval_id`, `author_agent_id`, `author_user_id`, `body`, timestamps
...
- `assertCanCreateAgents(req, squadId)`
- `assertCanManageAgentPermissions(req, squadId)`

Rules:

- Board always passes.
- Agent passes `can_create_agents` check if self permission true and same squad.
- Permission management by Squad Lead or board.
...
- If squad setting requires approval:
...
Board may continue using direct create route, but this route becomes canonical for Squad Lead/agent-led hiring.
...
- installed adapter list for this Slaw instance
...
- same gate as config-read endpoints (board/Squad Lead/`can_create_agents`).
...
## 7.1 Squad advanced settings

In Squads UI, add advanced settings panel/modal with:
...
In Agent Detail (board/Squad Lead context):
...
Add "Hire Agent" flow (for Squad Lead/authorized agents):
...
## 8. New Skill: `slaw-create-agent`

Create new skill directory:

- `skills/slaw-create-agent/SKILL.md`
- `skills/slaw-create-agent/references/api-reference.md`
...
Also update `skills/slaw/SKILL.md` to reference this skill for hiring workflows.
...
- DB schema updates (`squads`, `agents`, approvals status expansion, `approval_comments`)
...
- hire creation behavior with squad setting on/off
