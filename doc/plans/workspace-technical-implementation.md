...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/slaw-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `SLAW_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `SLAW_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Slaw may create host-local worktrees, processes, and paths
- `cloud`
  - Slaw should assume no durable host-local execution workspace management
...
- use `SLAW_EXECUTION_TOPOLOGY` as a defaulting guardrail
