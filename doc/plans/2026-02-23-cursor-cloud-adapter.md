...
This document defines the V1 design for a Slaw adapter that integrates with
...
4. synthesized stdout events for Slaw UI/CLI
...
3. **Agent callback auth to Slaw** uses a bootstrap exchange flow (no long-lived Slaw key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Slaw endpoints, not full SKILL.md prompt injection.
...
- `slawPublicUrl` (optional override; else `SLAW_PUBLIC_URL` env)
...
## Slaw Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `SLAW_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.slawPublicUrl`
2. `process.env.SLAW_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Slaw credentials in prompt text.
...
4. Slaw validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Slaw API calls.
...
1. Prompt includes a compact instruction to fetch skills from Slaw.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/slaw`
   - `GET /api/skills/slaw-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <slawPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Slaw run context.
...
5. `slawPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Slaw skill endpoints |
| Slaw auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Slaw context.
3. Add richer PR metadata surfacing in Slaw UI.
