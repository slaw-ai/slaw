...
Yes, there is a likely root cause in Slaw's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, Slaw can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: Slaw already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- Slaw may think it cancelled the run because the parent process is gone
...
This work is successful when Slaw can:
...
- require every skill author to add bespoke browser teardown logic before Slaw can clean up correctly
...
- make the leak measurable from Slaw's side before changing execution semantics
...
Treat this as a Slaw executor ownership bug, not an `agent-browser` bug.
