# Agent OS Technical Report for Slaw
...
1. Do not merge agent-os concepts into the Slaw core product model.
...
4. Do not import its workflow/cron/queue abstractions into Slaw core until they are reconciled with Slaw's issue/comment/governance model.
...
That gives `agent-os` a much stronger isolation story than Slaw's current "launch a host CLI in a workspace" local adapter approach.
...
This is directly relevant to Slaw because our adapter layer still normalizes each CLI agent in a fairly bespoke way.

## Slaw anchor points

The most relevant current Slaw surfaces for any future `agent-os` integration are:
...
## What Slaw can learn from it

### 1. A safer local execution substrate

Slaw's local adapters currently run host CLIs in managed workspaces and rely on adapter-specific behavior plus process-level controls. That is pragmatic, but weakly isolated.
...
This fits Slaw because it improves execution safety without changing the control-plane model.

### 2. Snapshotted execution roots instead of only mutable workspaces

Slaw already has strong execution-workspace concepts, but they are repo/worktree-centric. `agent-os` adds a stronger "start from known lower layers, write into a disposable upper layer" model.
...
Slaw's plugin system and adapters already have the beginnings of a controlled extension surface. `agent-os` reinforces the value of exposing capabilities as typed tools rather than raw shell access.
...
This aligns well with Slaw's governance story.

### 5. Better adapter normalization around sessions and capabilities

Slaw's adapter contract already supports execution results, session params, environment tests, skill syncing, quota windows, and optional `detectModel()`. But much of the per-agent behavior is still adapter-specific.
...
Slaw does not need ACP everywhere, but it would benefit from a more formal internal session capability model inspired by this.
...
Slaw can adopt that philosophy directly:
...
## What does not fit Slaw well

### 1. Its built-in orchestration primitives overlap the wrong layer

`agent-os` includes cron/session/workflow style primitives inside the runtime package. Slaw already has higher-level orchestration concepts:
...
If Slaw copied `agent-os` cron/workflow/queue ideas directly into core, we would likely duplicate orchestration across two layers. That would blur ownership and make debugging harder.
...
Those are Slaw's differentiators. They should not be displaced by runtime abstractions.
...
The permission model is good, but it is low-level. Slaw would still need to answer:
...
- keep Slaw heartbeat/task/workspace/budget logic unchanged
...
- Slaw's Codex adapter is already important and carries repo-specific behavior
...
- Slaw still records logs, summaries, cost metadata, and issue comments normally
...
Even without using `agent-os`, Slaw should consider standardizing adapter/runtime permissions around a vocabulary like:
...
Slaw should evaluate whether some execution workspaces can be backed by:
...
Slaw plugins and adapters should continue moving toward explicit typed tools over ad hoc shell access. `agent-os` confirms that this is the right direction.
...
### Recommendation E: do not import runtime-level workflows into Slaw core

Slaw should not copy `agent-os` cron/workflow/queue concepts into core orchestration yet.
...
`agent-os` is useful to Slaw as an execution technology reference, not as a product model.

Slaw should treat it the same way it treats sandboxes or agent CLIs:
