...
`PAP-447` asks how Slaw should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Slaw instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Slaw:

- some operators use Slaw to manage Slaw and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Slaw
- Claude and Codex expose different built-in affordances, so Slaw should not overfit to one tool

## Core Product Decision

Slaw should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Slaw
...
- Slaw should not assume that every adapter can see or use a host filesystem path directly
...
If Slaw makes worktrees agent-first, it will blur:
...
- Slaw core resolves and records execution workspace state
...
That gives Slaw room to support:
...
- Slaw handles workspace runtime services under the hood using project/workspace policy
...
Once Slaw is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Slaw cannot infer
...
- when issue moves to done, should Slaw auto-commit?
- should it auto-push?
...
Slaw already has the right foundation for a project-first model.
...
For server-spawned local adapters, Slaw already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Slaw should mirror directly

Implication:

- for `codex_local`, Slaw should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Slaw should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Slaw should separate:
...
Slaw should be able to express intentions such as:
...
These are not specific to Slaw. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".slaw/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Slaw-specific `devServer` flag.
...
Slaw should distinguish between:
...
Slaw should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Slaw should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Slaw
...
For runtime services specifically, Slaw should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Slaw-only dev-server flags
...
3. Inject resolved execution workspace details into `context.slawWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Slaw instance can be reused through env/config injection when appropriate
...
- Slaw can decide whether to reuse or start a fresh service deterministically
...
3. Test against Slaw-on-Slaw workflows first.
...
7. Workspace runtime services are modeled generically, not as Slaw-specific dev-server toggles.
