...
Slaw should not replace its workspace runtime service model with VS Code tasks.
...
- Slaw owns long-running workspace services and their desired state
- Slaw shows operators exactly which named thing they are starting or stopping
- Slaw distinguishes long-running services from one-shot jobs
...
This is directionally correct for Slaw because it gives the control plane an explicit model for service lifecycle, health, reuse, and restart behavior.
...
The issue is that the configuration surface is too low-level and Slaw does not yet leverage existing workspace metadata.

## Recommendation

Keep Slaw runtime services as the source of truth for service supervision.
...
4. `Runtime service instance`
   The live process record that already exists today in Slaw.

In that model, VS Code tasks are a way to populate workspace commands.
Only commands that map cleanly to Slaw service or job semantics should become runnable in Slaw.

## Why Not Fully Adopt VS Code Tasks

VS Code tasks are broader than Slaw runtime services.
...
That creates a bad fit if Slaw tries to use `tasks.json` as its only runtime model:
...
- problem matcher readiness is useful metadata, but it is not enough to replace Slaw's persisted service lifecycle model

The right boundary is interoperability, not replacement.

## Interoperability Contract

Slaw should support a conservative subset of VS Code tasks and clearly mark unsupported entries.

### Supported in phase 1

- `shell` and `process` tasks with a concrete command Slaw can resolve
...
- selected variable substitution when Slaw can resolve it safely from workspace context
- mapping task metadata into Slaw readiness/expose hints
- limited compound-task launch flows

### Not supported initially

- extension-provided task types Slaw cannot execute directly
...
Slaw needs an explicit classification layer instead of assuming every VS Code task is a service.

Recommended classification:

- `service`
  Explicitly marked by Slaw metadata, or confidently inferred from background/watch task semantics

- `job`
  One-shot command expected to exit

- `unsupported`
  Present in `tasks.json`, but not safely runnable by Slaw
...
For a workspace with `cwd`, Slaw should look for `.vscode/tasks.json`.
...
Slaw should map imported tasks with explicit, documented rules.

Recommended rules:

1. A task becomes a `job` by default.
2. A task becomes a `service` only when:
   - Slaw metadata marks it as a service, or
...
Slaw-specific metadata can live in a namespaced field on the imported task definition, for example:
...
Success condition:
an operator can see what Slaw would import and why.
...
Goal:
run supported imported service commands through the current Slaw supervisor.
...
Success condition:
imported service commands behave like native Slaw services once adopted.
...
- removal of Slaw's own runtime supervision model
- editor-dependent execution semantics inside the control plane

## Risks

- overfitting Slaw to VS Code and making the model worse for non-VS-Code repos
...
Slaw should adopt VS Code tasks as an optional workspace command source, not as the canonical runtime model.
...
- Existing Slaw workspace plan: `doc/plans/2026-03-10-workspace-strategy-and-git-worktrees.md`
