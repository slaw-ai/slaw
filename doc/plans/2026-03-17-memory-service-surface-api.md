...
This plan is based on the external landscape summarized in `doc/memory-landscape.md`, the AWS AgentCore comparison captured in [PAP-1274](/PAP/issues/PAP-1274), and the current Slaw architecture in:
...
Built-ins are useful for a zero-config local path, but most providers should arrive through the existing Slaw plugin runtime.

That keeps the core small and matches the broader Slaw direction that specialized knowledge systems live at the edges.

### 4. Slaw owns routing, provenance, and policy

Providers should not decide how Slaw entities map to governance.

Slaw core should own:
...
### 5. Slaw exposes shared hooks, providers own extraction

Slaw should emit a common set of memory hooks that built-ins, third-party adapters, and plugins can all use.

Those hooks should pass structured Slaw source objects plus normalized metadata. The provider then decides how to extract from those objects.

Slaw should not force one extraction pipeline or one canonical "memory text" transform before the provider sees the input.
...
Slaw should still log those changes in activity and preserve full auditability. Approval-gated memory governance can remain an enterprise or future policy layer.
...
A mapping from a Slaw target to a binding.
...
The normalized Slaw scope passed into a provider request.
...
A normalized trigger emitted by Slaw when something memory-relevant happens.
...
A normalized capture, record-write, query, browse, get, correction, or delete action performed through Slaw.

Slaw should log every memory operation whether the provider is local, plugin-backed, or external.

## Required Adapter Contract

The required core should be small enough to fit `memsearch`, `mem0`, `Memori`, `MemOS`, or `OpenViking`, but strong enough to satisfy Slaw's attribution and inspectability requirements.
...
This contract intentionally does not force a provider to expose its internal graph, file tree, or ontology. It does require enough structure for Slaw to browse, attribute, and audit what happened.
...
That lines up with the Slaw plan at a high level: provider configuration, scoped writes, scoped retrieval, provider-managed extraction as a capability, and a browse and inspect surface.

The concrete changes Slaw should take from AWS are:
...
AWS exposes list and retrieve surfaces directly. Slaw should not make browse optional at the portable layer.
...
Slaw should add cursor-based pagination to:
...
Slaw should keep its own control-plane-centric model, but the adapter contract needs obvious places to map those concepts:
...
AWS exposes extraction jobs explicitly. Slaw should too.
...
### 7. Keep Slaw provenance primary
...
## What Slaw Should Persist

Slaw should not mirror the full provider memory corpus into Postgres unless the provider is a Slaw-managed local provider.
...
Slaw should expose one shared hook system for memory.
...
1. `pre_run_hydrate`
   Before an agent run starts, Slaw may call `query(... intent = "agent_preamble")` using the active binding.

2. `post_run_capture`
   After a run finishes, Slaw may call `capture(...)` with structured run output, excerpts, and provenance.

3. `issue_comment_capture`
   When enabled on the binding, Slaw may call `capture(...)` for selected issue comments.

4. `issue_document_capture`
   When enabled on the binding, Slaw may call `capture(...)` for selected issue documents.
...
Slaw should give agents both automatic recall and explicit tools, with simple guidance:
...
Slaw needs a first-class UI for memory, otherwise providers become black boxes.
...
Slaw should treat memory accounting as two related but distinct concerns:
...
Slaw should record evaluation-oriented metrics where possible:
...
This is important because a memory system that "works" but silently burns budget or silently fails extraction is not acceptable in Slaw.
...
- it matches Slaw's local-first posture
- it is inspectable
...
- How much source payload should Slaw snapshot inside `memory_operations` for debugging without duplicating large transcripts?
...
- Slaw owns bindings, resolution, hooks, provenance, policy, and attribution.
- Providers own extraction, ranking, storage, and provider-native memory semantics.

That gives Slaw a stable memory service without locking the product to one memory philosophy or one vendor, and it integrates the AWS lessons without importing AWS's model into core.
