...
Turn the `agent-os` research into a low-risk Slaw execution plan that preserves Slaw's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

Slaw should not absorb `agent-os` as a product model or orchestration layer.

Slaw should evaluate `agent-os` in three narrow areas:
...
This work is successful when Slaw has:
...
- a proposed runtime capability model that fits current Slaw adapters
...
- replace Slaw heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to Slaw core

## Existing Slaw integration points
...
- prove that Slaw can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep Slaw's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a Slaw-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some Slaw workloads
...
- identify where Slaw should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in Slaw
...
- runtime experimentation could blur the boundary between Slaw as control plane and Slaw as execution platform
