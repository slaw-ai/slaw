...
Add a single storage subsystem for Slaw that supports:
...
- Default local path is under instance root: `~/.slaw/instances/<instanceId>/data/storage`.
...
- `slaw onboard` writes a valid `storage` config block by default.
- `slaw configure --section storage` can switch between local and s3 modes.
...
- `slaw doctor` reports actionable storage status.
