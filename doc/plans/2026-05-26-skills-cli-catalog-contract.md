...
Date: 2026-05-26
Source plan: approved Slaw skills CLI and catalog plan

This document freezes the first implementation contract for the `slaw skills`
...
- Agent desired skills continue to live in
  `adapterConfig.slawSkillSync.desiredSkills` for the first release. Do not
...
- Catalog skill: an app-shipped skill entry in `@slaw-ai/skills-catalog`.
...
| `skills agent sync <agent-ref> --skill <skill-ref>...` | Replaces the agent's non-required desired skill set with the supplied refs and triggers adapter sync. Required Slaw skills remain enforced by the server. | `AgentSkillSnapshot` |
...
Package name: `@slaw-ai/skills-catalog`.
