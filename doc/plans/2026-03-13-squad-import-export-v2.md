# 2026-03-13 Squad Import / Export V2 Plan

Status: Proposed implementation plan
Date: 2026-03-13
Audience: Product and engineering
Supersedes for package-format direction:
- `doc/plans/2026-02-16-module-system.md` sections that describe squad templates as JSON-only
...
This document defines the next-stage plan for Slaw squad import/export.

The core shift is:

- move from a Slaw-specific JSON-first portability package toward a markdown-first package format
- make GitHub repositories first-class package sources
- treat the squad package model as an extension of the existing Agent Skills ecosystem instead of inventing a separate skill format
- support squad, team, agent, and skill reuse without requiring a central registry

The normative package format draft lives in:

- `docs/squads/squads-spec.md`

This plan is about implementation and rollout inside Slaw.
...
Slaw already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-squad runtimes, not just Slaw
4. the squad package model is explicitly an extension of Agent Skills
5. no future dependency on `slaw.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.slaw.yaml` sidecar for high-fidelity Slaw-specific details
...
11. adapter-aware skill sync surfaces so Slaw can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Slaw at a local folder or GitHub repo and import a squad package without any registry.
...
- Export produces a clean vendor-neutral package plus a Slaw sidecar.
- `squads.sh` can later act as a discovery/index layer over repos implementing this format.
...
- shared types: `packages/shared/src/types/squad-portability.ts`
- shared validators: `packages/shared/src/validators/squad-portability.ts`
- server routes: `server/src/routes/squads.ts`
- server service: `server/src/services/squad-portability.ts`
- CLI commands: `cli/src/commands/client/squad.ts`
...
Slaw must not redefine `SKILL.md`.

Rules:

- `SKILL.md` stays Agent Skills compatible
- the squad package model is an extension of Agent Skills
- the base package is vendor-neutral and intended for any agent-squad runtime
- Slaw-specific fidelity lives in `.slaw.yaml`
- Slaw may resolve and install `SKILL.md` packages, but it must not require a Slaw-only skill format
...
- importers should resolve the shortname against local package skills first, then referenced or installed squad skills
### 5.4 Base Package Vs Slaw Extension
...
- base package:
  ...
  - no Slaw-only runtime fields by default
- Slaw extension:
  - `.slaw.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Slaw tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`slaw.manifest.json` is not part of the future package direction.
...
Slaw import/export should support these entity kinds:
...
In Slaw V2 portability:

- a team is an importable org subtree
- it is rooted at a manager agent
- it can be attached under a target manager in an existing squad
...
- Slaw can treat “this set of agents came from team package X” as the imported-team model
...
- selecting a squad auto-selects all included entities by default
...
Slaw should support source references in package metadata with:
...
For existing squad imports, the preview must support:
...
Slaw should define a new adapter surface area around skills:
...
Planned Slaw behavior:

- if an adapter supports read, Slaw should show current skills in the UI
- if an adapter supports write, Slaw should let the user enable/disable imported skills
- if an adapter supports sync, Slaw should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.slaw.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.slaw.yaml` alongside the base package
...
This supports “clean public squad package” workflows where a maintainer exports a follower-facing squad package without bundling active work items every time.
...
- skill -> squad-scoped reusable package metadata plus agent-scoped desired-skill attachment state where supported

### 10.2 Medium-Term

Slaw should add managed package/provenance records so imports are not anonymous one-off copies.
...
- support both squad-scoped reusable skills and agent-scoped skill attachments
...
- `slaw squad export <squad-id> --out <path>`
- `slaw squad import <path-or-url> --dry-run`
- `slaw squad import <path-or-url> --target existing -C <squad-id>`

Planned additions:

- `--package-kind squad|team|agent`
...
### 13.1 Squad Settings Import / Export

Add a real import/export section to Squad Settings.
...
If importing a team into an existing squad:
...
- add Squad Settings UI for current import/export capabilities
- start cutover work toward the markdown-first package reader

### Phase 2: Markdown-First Package Reader

- support `SQUAD.md` / `TEAM.md` / `AGENTS.md` root detection
...
- squad skill library groundwork
...
- `docs/squads/squads-spec.md` as the package-format draft
...
- board operator docs for Squad Settings import/export

## 16. Open Questions

1. Should imported skill packages be stored as managed package files in Slaw storage, or only referenced at import time?
   Decision: managed package files should support both squad-scoped reuse and agent-scoped attachment.
...
Engineering should treat this as the current plan of record for squad import/export beyond the existing V1 portability feature.

Immediate next steps:

1. accept `docs/squads/squads-spec.md` as the package-format draft
2. implement phase 1 stabilization work
3. build phase 2 markdown-first package reader before expanding ClipHub or `squads.sh`
4. treat the old manifest-based format as deprecated and not part of the future surface

This keeps Slaw aligned with:
