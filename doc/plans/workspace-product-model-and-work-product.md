...
Slaw needs to support two very different but equally valid ways of working:
...
Today, Slaw already has the beginnings of this model:
...
- making Slaw feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Slaw users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Slaw deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Slaw into a full code review product
...
This object must be recorded explicitly so that Slaw can:
...
Slaw should treat PRs as a type of work product linked back to:
...
Slaw already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Slaw must be a first-class deployment mode

Slaw cannot assume that it is running on the same machine as the code.

In cloud deployments, Slaw may:
...
- `Runtime service`: a process or service Slaw owns or tracks for a workspace
...
It also lets cloud-hosted Slaw deployments point at codebases and remotes without pretending the Slaw host has direct filesystem access.
...
Without an explicit `execution workspace` record, Slaw has nowhere stable to attach:
...
Slaw needs a single place to show:
...
- Slaw-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Slaw owning the execution host
...
## 1. Slaw host and execution host must be decoupled
...
without the Slaw host itself running local git or local preview processes.
...
Slaw should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Slaw.
...
Slaw should keep workspace automation centered on a small set of hooks:
...
Slaw should provide the equivalent visibility in the project `Code` surface:
...
Slaw should treat execution workspaces as the natural home for:
...
Slaw should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Slaw should continue treating cleanup policy as part of the core workspace model:
...
Slaw should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Slaw host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Slaw should borrow Worktrunk's good execution patterns while keeping the broader Slaw model:
...
That keeps Slaw focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Slaw deployments remain first-class

That is a better fit for Slaw than either extreme:
