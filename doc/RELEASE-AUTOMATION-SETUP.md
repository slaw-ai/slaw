...
This document covers the GitHub and npm setup required for the current Slaw release model:
...
Do this for every public package that Slaw publishes.
...
3. add a trusted publisher for the GitHub repository `slaw/slaw`
...
2. open that package on npm and add the `slaw/slaw` trusted publisher for `.github/workflows/release.yml`
...
2. revoke any personal automation token that used to publish Slaw
