...
All persisted under your bind mount (`./data/docker-slaw` in the example above).
...
- host port: `3100`
- persistent data dir: `./data/docker-slaw`
...
If you change host port or use a non-local domain, set `SLAW_PUBLIC_URL` to the external URL you will use in browser/auth flows.

Pass `OPENAI_API_KEY` and/or `ANTHROPIC_API_KEY` to enable local adapter runs.

### Full stack (with PostgreSQL)

Slaw server + PostgreSQL 17. The database is health-checked before the server starts.
...
PostgreSQL data persists in a named Docker volume (`pgdata`). Slaw data persists in `slaw-data`.
...
For authenticated deployments, set one canonical public URL and let Slaw derive auth/callback defaults:
...
services:
  slaw:
    environment:
      ...
      SLAW_PUBLIC_URL: https://desk.koker.net
```

`SLAW_PUBLIC_URL` is used as the primary source for:
...
Slaw URL, sign in or create an account, then choose `Claim this instance`
...
Set `SLAW_ALLOWED_HOSTNAMES` explicitly only when you need additional hostnames beyond the public URL host (for example Tailscale/LAN aliases or multiple private hostnames).
...
- Adapter environment checks in Slaw will surface missing auth/CLI prerequisites.

## Podman Quadlet (systemd)

The `docker/quadlet/` directory contains unit files to run Slaw + PostgreSQL as systemd services via Podman Quadlet.

| File | Purpose |
|------|---------|
| `docker/quadlet/slaw.pod` | Pod definition — groups containers into a shared network namespace |
| `docker/quadlet/slaw.container` | Slaw server — joins the pod, connects to Postgres at `127.0.0.1` |
| `docker/quadlet/slaw-db.container` | PostgreSQL 17 — joins the pod, health-checked |
...
3. Create a secrets env file (keep out of version control):
   ...
   ```sh
   cat > ~/.config/containers/systemd/slaw.env <<EOL
   BETTER_AUTH_SECRET=$(openssl rand -hex 32)
   POSTGRES_USER=slaw
   POSTGRES_PASSWORD=slaw
   POSTGRES_DB=slaw
   DATABASE_URL=postgres://slaw:slaw@127.0.0.1:5432/slaw
   # OPENAI_API_KEY=sk-...
...
4. Create the data directory and start:
   ...
   ```sh
   mkdir -p ~/.local/share/slaw
   systemctl --user daemon-reload
   systemctl --user start slaw-pod
   ```

### Quadlet management

```sh
journalctl --user -u slaw -f        # App logs
journalctl --user -u slaw-db -f     # DB logs
systemctl --user status slaw-pod    # Pod status
systemctl --user restart slaw-pod   # Restart all
systemctl --user stop slaw-pod      # Stop all
```

### Quadlet notes

- **First boot**: Unlike Docker Compose's `condition: service_healthy`, Quadlet's `After=` only waits for the DB unit to *start*, not for PostgreSQL to be ready. On a cold first boot you may see one or two restart attempts in `journalctl --user -u slaw` while PostgreSQL initialises — this is expected and resolves automatically via `Restart=on-failure`.
- Containers in a pod share `localhost`, so Slaw reaches Postgres at `127.0.0.1:5432`.
- PostgreSQL data persists in the `slaw-pgdata` named volume.
- Slaw data persists at `~/.local/share/slaw`.
...
- `npx @slaw-ai/slaw onboard --yes` completes
...
```sh
HOST_PORT=3200 SLAW_VERSION=latest ./scripts/docker-onboard-smoke.sh
SLAW_DEPLOYMENT_MODE=authenticated SLAW_DEPLOYMENT_EXPOSURE=private ./scripts/docker-onboard-smoke.sh
SMOKE_DETACH=true SMOKE_METADATA_FILE=/tmp/slaw-smoke.env SLAW_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
- Slaw data persists via Docker volumes/bind mounts (compose) or at `~/.local/share/slaw` (quadlet).
