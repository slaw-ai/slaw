...
- In authenticated mode, run with board auth via `SLAW_AUTH_HEADER` or `SLAW_COOKIE`.

Optional auth flags (for authenticated mode):

- `SLAW_AUTH_HEADER` (for example `Bearer ...`)
- `SLAW_COOKIE` (session cookie header value)
...
- defaults to isolated config dir `~/.openclaw-slaw-smoke`
- resets smoke agent state each run by default (`OPENCLAW_RESET_STATE=1`) to avoid stale provider/auth drift

Networking behavior for this smoke script:

- auto-detects and prints a Slaw host URL reachable from inside OpenClaw Docker
- default container-side host alias is `host.docker.internal` (override with `SLAW_HOST_FROM_CONTAINER` / `SLAW_HOST_PORT`)
- if Slaw rejects container hostnames in authenticated/private mode, allow `host.docker.internal` via `pnpm slaw allowed-hostname host.docker.internal` and restart Slaw
