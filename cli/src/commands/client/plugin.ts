...
} from "../../../../packages/plugins/create-slaw-plugin/src/index.js";
...
interface PluginSquadOptions extends PluginJsonOptions {
  squadId?: string;
}
...
export function renderLocalPluginInstallHint(packagePath: string): string {
  return [
    ...
    pc.dim(`Keep ${pc.cyan("pnpm dev")} running in ${packagePath}; Slaw watches rebuilt dist output and reloads the plugin worker.`),
  ].join("\n");
}
...
export function buildPluginInitNextCommands(outputDir: string): string[] {
  ...
  return [
    ...
    "pnpm install",
    "pnpm dev",
    `slaw plugin install ${quotedOutputDir}`,
  ];
}
...
export function registerPluginCommands(program: Command): void {
  ...
  addCommonClientOptions(
    plugin
      ...
      .description("Scaffold a local Slaw plugin project")
    ...
    plugin
      ...
      .description(
        "Install a plugin from a local path or npm package.\n" +
          "  Examples:\n" +
          "    slaw plugin install ./my-plugin              # local path\n" +
          "    slaw plugin install @acme/plugin-linear      # npm package\n" +
          "    slaw plugin install @acme/plugin-linear@1.2  # pinned version",
      )
          ...
          for (const ex of rows) {
            console.log(
              `${pc.bold(ex.displayName)}  ${pc.dim(ex.pluginKey)}\n` +
                `  ${ex.description}\n` +
                `  ${pc.cyan(`slaw plugin install ${ex.localPath}`)}`,
            );
          }
        } catch (err) {
          handleCommandError(err);
...
function addPluginLocalFolderGet(parent: Command, name: string, description: string): void {
  addCommonClientOptions(
    parent
      ...
      .action(async (pluginId: string, opts: PluginSquadOptions) => {
        try {
          const ctx = resolveCommandContext(opts, { requireSquad: true });
          printOutput(await ctx.api.get(`/api/plugins/${encodeURIComponent(pluginId)}/squads/${ctx.squadId}/local-folders`), { json: ctx.json });
        } catch (err) {
          handleCommandError(err);
        }
      }),
...
function addPluginLocalFolderKeyGet(parent: Command, name: string, description: string, suffix: string): void {
  addCommonClientOptions(
    parent
      ...
      .requiredOption("-C, --squad-id <id>", "Squad ID")
      .action(async (pluginId: string, folderKey: string, opts: PluginSquadOptions) => {
        try {
          const ctx = resolveCommandContext(opts, { requireSquad: true });
          printOutput(
            await ctx.api.get(`/api/plugins/${encodeURIComponent(pluginId)}/squads/${ctx.squadId}/local-folders/${encodeURIComponent(folderKey)}/${suffix}`),
            { json: ctx.json },
          );
        } catch (err) {
          handleCommandError(err);
    ...
    parent
      ...
      .action(async (pluginId: string, folderKey: string, opts: PluginSquadOptions) => {
        try {
          ...
          printOutput(
            await ctx.api.post(
              `/api/plugins/${encodeURIComponent(pluginId)}/squads/${ctx.squadId}/local-folders/${encodeURIComponent(folderKey)}/${suffix}`,
    ...
    parent
      ...
      .action(async (pluginId: string, folderKey: string, opts: PluginSquadOptions) => {
        try {
          ...
          printOutput(
            await ctx.api.put(
              `/api/plugins/${encodeURIComponent(pluginId)}/squads/${ctx.squadId}/local-folders/${encodeURIComponent(folderKey)}`,
              parseJson(opts.payloadJson ?? "{}"),
            ),
            { json: ctx.json },
          );
