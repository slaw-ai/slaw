...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.SLAW_CONFIG;
...
const WORKTREE_NAME_PREFIX = "slaw-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.SLAW_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.SLAW_WORKTREE_START_POINT) ?? undefined;
}

type ConfiguredStorage = {
  getObject(squadId: string, objectKey: string): Promise<Buffer>;
  putObject(squadId: string, objectKey: string, body: Buffer, contentType: string): Promise<void>;
};

function assertStorageSquadPrefix(squadId: string, objectKey: string): void {
  if (!objectKey.startsWith(`${squadId}/`) || objectKey.includes("..")) {
    throw new Error(`Invalid object key for squad ${squadId}.`);
  }
}
...
function createConfiguredStorageFromSlawConfig(config: SlawConfig): ConfiguredStorage {
  if (config.storage.provider === "local_disk") {
    const baseDir = expandHomePrefix(config.storage.localDisk.baseDir);
    return {
      async getObject(squadId: string, objectKey: string) {
        assertStorageSquadPrefix(squadId, objectKey);
        return await fsPromises.readFile(resolveLocalStoragePath(baseDir, objectKey));
      },
      async putObject(squadId: string, objectKey: string, body: Buffer) {
        assertStorageSquadPrefix(squadId, objectKey);
  ...
  return {
    ...
    async putObject(squadId: string, objectKey: string, body: Buffer, contentType: string) {
      assertStorageSquadPrefix(squadId, objectKey);
      const { sdk, client } = await getS3Client();
      await client.send(
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromSlawConfig(config);
}
...
export async function readSourceAttachmentBody(
  ...
  for (const sourceStorage of sourceStorages) {
    try {
      return await sourceStorage.getObject(squadId, objectKey);
    } catch (error) {
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.slaw${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".slaw", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".slaw", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.slaw"));
...
export function resolveWorktreeReseedTargetPaths(input: {
  ...
  const envEntries = readSlawEnvEntries(resolveSlawEnvFile(input.configPath));
  const homeDir = nonEmpty(envEntries.SLAW_HOME);
  const instanceId = nonEmpty(envEntries.SLAW_INSTANCE_ID);

  if (!homeDir || !instanceId) {
    throw new Error(
      `Target config ${input.configPath} does not look like a worktree-local Slaw instance. Expected SLAW_HOME and SLAW_INSTANCE_ID in the adjacent .env.`,
    );
  }
...
function resolveExistingGitWorktree(selector: string, cwd: string): MergeSourceChoice | null {
  ...
  if (existsSync(directPath)) {
    return {
      ...
      hasSlawConfig: existsSync(path.resolve(directPath, ".slaw", "config.json")),
...
async function ensureRepairTargetWorktree(input: {
  ...
  const currentConfigPath = path.resolve(currentRoot, ".slaw", "config.json");
  ...
  if (existing) {
    return {
      ...
      configPath: path.resolve(existing.worktree, ".slaw", "config.json"),
  ...
  return {
    rootPath: targetPath,
    configPath: path.resolve(targetPath, ".slaw", "config.json"),
    label: branchName,
    branchName,
    created: true,
  };
...
function resolveSourceConnectionString(config: SlawConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://slaw:slaw@127.0.0.1:${port}/slaw`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: SlawConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.SLAW_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.SLAW_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.SLAW_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.SLAW_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  ...
  const sourceEnvFile = resolveSlawEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readSlawEnvEntries(sourceEnvFile);
  ...
  try {
    if (input.sourceConfig.database.mode === "embedded-postgres") {
      ...
      const sourceAdminConnectionString = `postgres://slaw:slaw@127.0.0.1:${sourceHandle.port}/postgres`;
      await ensurePostgresDatabase(sourceAdminConnectionString, "slaw");
    }
    ...
    const adminConnectionString = `postgres://slaw:slaw@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "slaw");
    const targetConnectionString = `postgres://slaw:slaw@127.0.0.1:${targetHandle.port}/slaw`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  if (opts.force) {
    ...
    // <repo>/.slaw/worktrees/ holding every repo-managed worktree
  ...
  const sourceEnvEntries = readSlawEnvEntries(resolveSlawEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.SLAW_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.SLAW_AGENT_JWT_SECRET);
  mergeSlawEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { SLAW_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadSlawEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Slaw inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printSlawCliBanner();
  p.intro(pc.bgCyan(pc.black(" slaw worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printSlawCliBanner();
  p.intro(pc.bgCyan(pc.black(" slaw worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasSlawConfig: existsSync(path.resolve(worktreePath, ".slaw", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printSlawCliBanner();
  p.intro(pc.bgCyan(pc.black(" slaw worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveSlawEnvFile(configPath);
  const envEntries = readSlawEnvEntries(envPath);
  const out = {
    SLAW_CONFIG: configPath,
    ...(envEntries.SLAW_HOME ? { SLAW_HOME: envEntries.SLAW_HOME } : {}),
    ...(envEntries.SLAW_INSTANCE_ID ? { SLAW_INSTANCE_ID: envEntries.SLAW_INSTANCE_ID } : {}),
    ...(envEntries.SLAW_CONTEXT ? { SLAW_CONTEXT: envEntries.SLAW_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasSlawConfig)
      .map((choice) => path.resolve(choice.worktree, ".slaw", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readSlawEnvEntries(resolveSlawEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start Slaw once) before using worktree merge history.`,
      );
    }
...
async function resolveMergeSquad(input: {
  sourceDb: ClosableDb;
  targetDb: ClosableDb;
  selector?: string;
}): Promise<ResolvedMergeSquad> {
  ...
  const targetById = new Map(targetSquads.map((squad) => [squad.id, squad]));
  const shared = sourceSquads.filter((squad) => targetById.has(squad.id));
  const selector = nonEmpty(input.selector);
  if (selector) {
    const matched = shared.find(
      (squad) => squad.id === selector || squad.issuePrefix.toLowerCase() === selector.toLowerCase(),
    );
    if (!matched) {
      throw new Error(`Could not resolve squad "${selector}" in both source and target databases.`);
    }
    return matched;
  }
  ...
  if (shared.length === 0) {
    throw new Error("Source and target databases do not share a squad id. Pass --squad explicitly once both sides match.");
  }

  const options = shared
    .map((squad) => `${squad.issuePrefix} (${squad.name})`)
    .join(", ");
  throw new Error(`Multiple shared squads found. Re-run with --squad <id-or-prefix>. Options: ${options}`);
}
...
function renderMergePlan(plan: Awaited<ReturnType<typeof collectMergePlan>>["plan"], extras: {
  ...
  const lines = [
    ...
    `Squad: ${plan.squadName} (${plan.issuePrefix})`,
    "",
    "Projects",
...
function resolveRunningEmbeddedPostgresPid(config: SlawConfig): number | null {
  if (config.database.mode !== "embedded-postgres") {
    return null;
  }
...
async function collectMergePlan(input: {
  ...
  const [
    ...
    input.scopes.includes("comments")
      ? input.sourceDb
        .select()
        .from(issueComments)
        .where(eq(issueComments.squadId, squadId))
      : Promise.resolve([]),
    input.targetDb
      .select()
      .from(issueComments)
    ...
    input.sourceDb
      .select({
        ...
        squadId: issueDocuments.squadId,
    ...
    input.sourceDb
      .select()
      .from(projects)
      .where(eq(projects.squadId, squadId)),
    input.sourceDb
      .select()
      .from(projectWorkspaces)
      .where(eq(projectWorkspaces.squadId, squadId)),
    input.targetDb
      .select()
      .from(projects)
      .where(eq(projects.squadId, squadId)),
    ...
    input.targetDb
      .select()
      .from(goals)
      .where(eq(goals.squadId, squadId)),
    input.sourceDb
  ...
  if (!targetSquadRow) {
    throw new Error(`Target squad ${squadId} was not found.`);
  }

  const plan = buildWorktreeMergePlan({
    squadId,
    squadName: input.squad.name,
    issuePrefix: input.squad.issuePrefix,
    previewIssueCounterStart: targetSquadRow.issueCounter,
    scopes: input.scopes,
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasSlawConfig ? "slaw" : "no-slaw-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".slaw", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".slaw", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .slaw/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasSlawConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a Slaw worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasSlawConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No Slaw worktrees were found. Run `slaw worktree:list` to inspect the repo worktrees.");
  }
...
async function applyMergePlan(input: {
  ...
  return await input.targetDb.transaction(async (tx) => {
    ...
    if (issueInserts.length > 0) {
      const [squadRow] = await tx
        .update(squads)
        .set({ issueCounter: sql`${squads.issueCounter} + ${issueInserts.length}` })
        .where(eq(squads.id, squadId))
        .returning({ issueCounter: squads.issueCounter });
      nextIssueNumber = squadRow.issueCounter - issueInserts.length + 1;
    }
    ...
    for (const issue of issueInserts) {
      ...
      nextIssueNumber += 1;
      const identifier = `${input.squad.issuePrefix}-${issueNumber}`;
    ...
    for (const comment of commentCandidates) {
      ...
      const parentExists = await tx
        .select({ id: issues.id })
        .from(issues)
        .where(and(eq(issues.id, comment.source.issueId), eq(issues.squadId, squadId)))
        .then((rows) => rows[0] ?? null);
    ...
    for (const documentPlan of documentCandidates) {
      const parentExists = await tx
        .select({ id: issues.id })
        .from(issues)
        .where(and(eq(issues.id, documentPlan.source.issueId), eq(issues.squadId, squadId)))
        .then((rows) => rows[0] ?? null);
      if (!parentExists) continue;
    ...
    const existingAttachmentIds = new Set(
      (
        await tx
          ...
          .from(issueAttachments)
          .where(eq(issueAttachments.squadId, squadId))
      ).map((row) => row.id),
    );
    ...
    for (const attachment of attachmentCandidates) {
      ...
      const parentExists = await tx
        .select({ id: issues.id })
        .from(issues)
        .where(and(eq(issues.id, attachment.source.issueId), eq(issues.squadId, squadId)))
        .then((rows) => rows[0] ?? null);
      if (!parentExists) continue;
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Slaw configs are the same. Choose different --from/--to worktrees.");
  }
  ...
  try {
    const squad = await resolveMergeSquad({
...
async function runWorktreeReseed(opts: WorktreeReseedOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Slaw configs are the same. Choose different --from/--to values.");
  }
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop Slaw in ${targetEndpoint.rootPath} before reseeding, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  ...
  const confirmed = opts.yes
    ? true
    : await p.confirm({
      message: `Overwrite the isolated Slaw DB for ${targetEndpoint.label} from ${source.label} using ${seedMode} seed mode?`,
      initialValue: false,
    });
  if (p.isCancel(confirmed) || !confirmed) {
...
export async function worktreeRepairCommand(opts: WorktreeRepairOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(target.configPath)) {
    throw new Error("Source and target Slaw configs are the same. Use --from-config/--from-instance to point repair at a different source.");
  }

  const targetConfig = existsSync(target.configPath) ? readConfig(target.configPath) : null;
  const targetEnvEntries = readSlawEnvEntries(resolveSlawEnvFile(target.configPath));
  const targetHasWorktreeEnv = Boolean(
    nonEmpty(targetEnvEntries.SLAW_HOME) && nonEmpty(targetEnvEntries.SLAW_INSTANCE_ID),
  );
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop Slaw in ${target.rootPath} before repairing, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  if (runningTargetPid && opts.allowLiveTarget) {
    p.log.warning(`Proceeding even though the target embedded PostgreSQL appears to be running (pid ${runningTargetPid}).`);
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Slaw instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Slaw instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with slaw- if needed (created at ~/slaw-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: SLAW_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: SLAW_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source SLAW_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Slaw instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like Slaw worktrees")
  ...
  program
    ...
    .option("--squad <id-or-prefix>", "Shared squad id or issue prefix inside the chosen source/target instances")
  ...
  worktree
    .command("reseed")
    .description("Re-seed an existing worktree-local instance from another Slaw instance or worktree")
    .option("--from <worktree>", "Source worktree path, directory name, branch name, or current")
    .option("--to <worktree>", "Target worktree path, directory name, branch name, or current (defaults to current)")
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source SLAW_HOME used when deriving the source config")
  ...
  worktree
    .command("repair")
    .description("Create or repair a linked worktree-local Slaw instance without touching the primary checkout")
    .option("--branch <name>", "Existing branch/worktree selector to repair, or a branch name to create under .slaw/worktrees")
    .option("--home <path>", `Home root for worktree instances (env: SLAW_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source SLAW_HOME used when deriving the source config")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with slaw- if needed")
