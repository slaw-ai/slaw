...
export async function runCommand(opts: RunOptions): Promise<void> {
  const instanceId = resolveSlawInstanceId(opts.instance);
  process.env.SLAW_INSTANCE_ID = instanceId;

  const homeDir = resolveSlawHomeDir();
  ...
  p.intro(pc.bgCyan(pc.black(" slaw run ")));
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("slaw onboard")} once, then retry ${pc.cyan("slaw run")}.`);
      process.exit(1);
    }
  ...
  p.log.step("Starting Slaw server...");
...
function ensureDevWorkspaceBuildDeps(projectRoot: string): void {
  ...
  if (result.error) {
    throw new Error(
      `Failed to prepare workspace build artifacts before starting the Slaw dev server.\n${formatError(result.error)}`,
    );
  }
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  try {
    ...
    throw new Error(
      `Slaw server failed to start.\n` +
        `${formatError(err)}`,
    );
  }
}

function shouldGenerateBootstrapInviteAfterStart(config: SlawConfig): boolean {
...
async function startServerFromModule(mod: unknown, label: string): Promise<StartedServer> {
  ...
  if (typeof startServer !== "function") {
    throw new Error(`Slaw server entrypoint did not export startServer(): ${label}`);
  }
