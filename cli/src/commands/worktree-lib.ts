...
export const DEFAULT_WORKTREE_HOME = "~/.slaw-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".slaw");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "slaw",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    SLAW_HOME: paths.homeDir,
    SLAW_INSTANCE_ID: paths.instanceId,
    SLAW_CONFIG: paths.configPath,
    SLAW_CONTEXT: paths.contextPath,
    SLAW_IN_WORKTREE: "true",
    ...(branding?.name ? { SLAW_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { SLAW_WORKTREE_COLOR: branding.color } : {}),
  };
}
