...
if (!embeddedPostgresSupport.supported) {
  console.warn(
    `Skipping embedded Postgres squad import/export e2e tests on this host: ${embeddedPostgresSupport.reason ?? "unsupported environment"}`,
  );
}
...
function createBaseSlawEnv(options: TestSlawEnv) {
  const env = { ...process.env };
  for (const key of Object.keys(env)) {
    if (key.startsWith("SLAW_")) {
      delete env[key];
    }
  }

  env.SLAW_CONFIG = options.configPath;
  env.SLAW_HOME = options.slawHome;
  env.SLAW_INSTANCE_ID = options.instanceId;
  env.SLAW_CONTEXT = path.join(options.slawHome, "context.json");
  env.SLAW_AUTH_STORE = path.join(options.slawHome, "auth.json");
...
function createServerEnv(
  ...
  options: Omit<TestSlawEnv, "configPath">,
) {
  const env = createBaseSlawEnv({
    configPath,
    ...options,
  });
  ...
  env.SERVE_UI = "false";
  env.SLAW_DB_BACKUP_ENABLED = "false";
  env.HEARTBEAT_SCHEDULER_ENABLED = "false";
  env.SLAW_MIGRATION_AUTO_APPLY = "true";
  env.SLAW_UI_DEV_MIDDLEWARE = "false";

  return env;
}

function createCliEnv(options: TestSlawEnv) {
  const env = createBaseSlawEnv(options);
  delete env.DATABASE_URL;
  delete env.PORT;
  delete env.HOST;
  delete env.SERVE_UI;
  delete env.SLAW_DB_BACKUP_ENABLED;
  delete env.HEARTBEAT_SCHEDULER_ENABLED;
  delete env.SLAW_MIGRATION_AUTO_APPLY;
  delete env.SLAW_UI_DEV_MIDDLEWARE;
  return env;
}
...
async function runCliJson<T>(
  args: string[],
  opts: TestSlawEnv & { apiBase?: string; includeConfigArg?: boolean },
) {
  const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../..");
  const cliArgs = ["--silent", "slaw", ...args];
  if (opts.apiBase) {
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `slaw run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describeEmbeddedPostgres("slaw squad import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
  ...
  beforeAll(async () => {
    tempRoot = mkdtempSync(path.join(os.tmpdir(), "slaw-squad-cli-e2e-"));
    configPath = path.join(tempRoot, "config", "config.json");
    exportDir = path.join(tempRoot, "exported-squad");
    slawHome = path.join(tempRoot, "slaw-home");
    cliShellHome = path.join(tempRoot, "shell-home");
    slawInstanceId = "squad-cli-e2e";
    mkdirSync(slawHome, { recursive: true });
    mkdirSync(cliShellHome, { recursive: true });

    tempDb = await startEmbeddedPostgresTestDatabase("slaw-squad-cli-db-");
    ...
    const child = spawn(
      "pnpm",
      ["slaw", "run", "--config", configPath],
      {
        cwd: repoRoot,
        env: createServerEnv(configPath, port, tempDb.connectionString, {
          slawHome,
          instanceId: slawInstanceId,
  ...
  it("exports a squad package and imports it into new and existing squads", async () => {
    ...
    const expectedContextPath = path.join(slawHome, "context.json");
    const leakedContextPath = path.join(cliShellHome, ".slaw", "context.json");
    ...
    const sourceSquad = await api<{ id: string; name: string; issuePrefix: string }>(apiBase, "/api/squads", {
      method: "POST",
    ...
    const sourceAgent = await api<{ id: string; name: string }>(
      apiBase,
      `/api/squads/${sourceSquad.id}/agents`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          ...
          instructionsBundle: {
            files: {
              "AGENTS.md": "You verify squad portability.",
            },
          },
        }),
      },
    ...
    const sourceProject = await api<{ id: string; name: string }>(
      apiBase,
      `/api/squads/${sourceSquad.id}/projects`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
    ...
    const largeIssueDescription = `Round-trip the squad package through the CLI.\n\n${"portable-data ".repeat(12_000)}`;

    const sourceIssue = await api<{ id: string; title: string; identifier: string }>(
      apiBase,
      `/api/squads/${sourceSquad.id}/issues`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          title: "Validate squad import/export",
    ...
    expect(readFileSync(path.join(exportDir, "SQUAD.md"), "utf8")).toContain(sourceSquad.name);
    expect(readFileSync(path.join(exportDir, ".slaw.yaml"), "utf8")).toContain('schema: "slaw/v1"');

    const importedNew = await runCliJson<{
      squad: { id: string; name: string; action: string };
    ...
    expect(importedNew.squad.action).toBe("created");
    ...
    expect(previewExisting.plan.squadAction).toBe("none");
    ...
    expect(importedExisting.squad.action).toBe("unchanged");
    ...
    const zipPath = path.join(tempRoot, "exported-squad.zip");
    const portableFiles: Record<string, string> = {};
    collectTextFiles(exportDir, exportDir, portableFiles);
    writeFileSync(zipPath, createStoredZipArchive(portableFiles, "slaw-demo"));
    ...
    expect(importedFromZip.squad.action).toBe("created");
