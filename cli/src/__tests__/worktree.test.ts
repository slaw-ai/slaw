...
function buildSourceConfig(): SlawConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("slaw-pr-432")).toBe(
      path.resolve(os.homedir(), "slaw-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("slaw/pr-432")).toThrow(
  ...
  it("rewrites auth URLs only when they already include a port", () => {
    ...
    expect(rewriteLocalUrlPort("https://slaw.example", 3110)).toBe("https://slaw.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/slaw-feature",
      homeDir: "/tmp/slaw-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/slaw-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/slaw-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.SLAW_HOME).toBe(path.resolve("/tmp/slaw-worktrees"));
    expect(env.SLAW_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.SLAW_IN_WORKTREE).toBe("true");
    expect(env.SLAW_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.SLAW_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export SLAW_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "slaw-worktree-secrets-"));
    const originalInlineMasterKey = process.env.SLAW_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.SLAW_SECRETS_MASTER_KEY_FILE;
    try {
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.SLAW_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.SLAW_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.SLAW_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          SLAW_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "slaw-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.SLAW_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.SLAW_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".slaw-worktrees"),
      });

      const envPath = path.join(repoRoot, ".slaw", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("SLAW_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("SLAW_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/SLAW_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.SLAW_AGENT_JWT_SECRET;
      } else {
        process.env.SLAW_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves repo-managed worktree checkouts when --force re-runs from the source repo", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "slaw-worktree-force-preserve-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      const repoConfigDir = path.join(repoRoot, ".slaw");
  ...
  itEmbeddedPostgres(
    ...
    async () => {
      const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "slaw-worktree-auth-seed-"));
      ...
      const worktreeHome = path.join(tempRoot, ".slaw-worktrees");
      const originalCwd = process.cwd();
      const sourceDb = await startEmbeddedPostgresTestDatabase("slaw-worktree-auth-source-");
      ...
      try {
        ...
        const targetConfig = JSON.parse(
          fs.readFileSync(path.join(worktreeRoot, ".slaw", "config.json"), "utf8"),
        ) as SlawConfig;
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "slaw-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".slaw-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".slaw", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local Slaw config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "slaw-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".slaw", "config.json");
    const originalCwd = process.cwd();
    const originalSlawConfig = process.env.SLAW_CONFIG;
    ...
    try {
      ...
      process.chdir(originalCwd);
      if (originalSlawConfig === undefined) {
        delete process.env.SLAW_CONFIG;
      } else {
        process.env.SLAW_CONFIG = originalSlawConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "slaw-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("derives worktree reseed target paths from the adjacent env file", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "slaw-worktree-reseed-target-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".slaw", "config.json");
    const envPath = path.join(worktreeRoot, ".slaw", ".env");
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          ...
          "SLAW_INSTANCE_ID=pap-1132-chat",
        ].join("\n"),
        "utf8",
      );
      expect(
  ...
  it("rejects reseed targets without worktree env metadata", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "slaw-worktree-reseed-target-missing-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".slaw", "config.json");
    ...
    try {
      fs.mkdirSync(path.dirname(configPath), { recursive: true });
      fs.writeFileSync(configPath, JSON.stringify(buildSourceConfig()), "utf8");
      fs.writeFileSync(path.join(worktreeRoot, ".slaw", ".env"), "", "utf8");

      expect(() =>
        resolveWorktreeReseedTargetPaths({
          configPath,
          rootPath: worktreeRoot,
        })).toThrow("does not look like a worktree-local Slaw instance");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("reseed preserves the current worktree ports, instance id, and branding", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "slaw-worktree-reseed-"));
    const repoRoot = path.join(tempRoot, "repo");
    ...
    const sourcePaths = resolveWorktreeLocalPaths({
      cwd: sourceRoot,
      homeDir: path.join(tempRoot, ".slaw-source"),
      instanceId: "default",
    });
    const originalCwd = process.cwd();
    const originalSlawConfig = process.env.SLAW_CONFIG;
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          `SLAW_HOME=${homeDir}`,
          `SLAW_INSTANCE_ID=${currentInstanceId}`,
          "SLAW_WORKTREE_NAME=existing-name",
          "SLAW_WORKTREE_COLOR=\"#112233\"",
        ].join("\n"),
        "utf8",
      );
      ...
      expect(rewrittenEnv).toContain(`SLAW_INSTANCE_ID=${currentInstanceId}`);
      expect(rewrittenEnv).toContain("SLAW_WORKTREE_NAME=existing-name");
      expect(rewrittenEnv).toContain("SLAW_WORKTREE_COLOR=\"#112233\"");
    } finally {
      process.chdir(originalCwd);
      if (originalSlawConfig === undefined) {
        delete process.env.SLAW_CONFIG;
  ...
  it("restores the current worktree config and instance data if reseed fails", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "slaw-worktree-reseed-rollback-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceRoot = path.join(tempRoot, "source");
    const homeDir = path.join(tempRoot, ".slaw-worktrees");
    ...
    try {
      ...
      fs.writeFileSync(currentPaths.configPath, JSON.stringify(currentConfig, null, 2), "utf8");
      fs.writeFileSync(currentPaths.envPath, `SLAW_HOME=${homeDir}\nSLAW_INSTANCE_ID=${currentInstanceId}\n`, "utf8");
      ...
      expect(restoredEnv).toContain(`SLAW_INSTANCE_ID=${currentInstanceId}`);
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/slaw",
        targetRepoRoot: "/Users/example/slaw-pr-432",
        workspaceCwd: "/Users/example/slaw",
      }),
    ).toBe("/Users/example/slaw-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/slaw",
        targetRepoRoot: "/Users/example/slaw-pr-432",
        workspaceCwd: "/Users/example/slaw/packages/db",
      }),
    ).toBe("/Users/example/slaw-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "slaw-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "slaw-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "slaw-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("slaw-make-test", {
        seed: false,
        home: path.join(tempRoot, ".slaw-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".slaw", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".slaw", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
  ...
  it("no-ops on the primary checkout unless --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "slaw-worktree-repair-primary-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      expect(fs.existsSync(path.join(repoRoot, ".slaw", "config.json"))).toBe(false);
      expect(fs.existsSync(path.join(repoRoot, ".slaw", "worktrees"))).toBe(false);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  ...
  it("repairs the current linked worktree when Slaw metadata is missing", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "slaw-worktree-repair-current-"));
    const repoRoot = path.join(tempRoot, "repo");
    const worktreePath = path.join(repoRoot, ".slaw", "worktrees", "repair-me");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".slaw-worktrees");
  ...
  it("creates and repairs a missing branch worktree when --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "slaw-worktree-repair-branch-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".slaw-worktrees");
    const originalCwd = process.cwd();
    const expectedWorktreePath = path.join(repoRoot, ".slaw", "worktrees", "feature-repair-me");
    ...
    try {
      ...
      expect(fs.existsSync(path.join(expectedWorktreePath, ".slaw", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(expectedWorktreePath, ".slaw", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
