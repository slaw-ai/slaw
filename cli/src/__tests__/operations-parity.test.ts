...
describe("operations parity commands", () => {
  ...
  it("wraps cost, finance, and budget endpoints", async () => {
    ...
    await run(["cost", "summary", "--squad-id", SQUAD_ID]);
    await run(["cost", "by-agent", "--squad-id", SQUAD_ID]);
    await run(["cost", "by-project", "--squad-id", SQUAD_ID]);
    await run(["cost", "issue", ISSUE_ID]);
    await run(["cost", "event:create", "--squad-id", SQUAD_ID, "--payload-json", "{}"]);
    await run(["finance", "event:create", "--squad-id", SQUAD_ID, "--payload-json", "{}"]);
    await run(["finance", "summary", "--squad-id", SQUAD_ID]);
    await run(["budget", "overview", "--squad-id", SQUAD_ID]);
    await run(["budget", "policy:upsert", "--squad-id", SQUAD_ID, "--payload-json", "{}"]);
    await run(["budget", "squad:update", "--squad-id", SQUAD_ID, "--payload-json", "{}"]);
    await run(["budget", "agent:update", AGENT_ID, "--payload-json", "{}"]);
    await run(["budget", "incident:resolve", INCIDENT_ID, "--squad-id", SQUAD_ID]);

    expect(fetchMock.mock.calls.map((call) => [call[1]?.method ?? "GET", call[0]])).toEqual([
      ["GET", `http://localhost:3100/api/squads/${SQUAD_ID}/costs/summary`],
      ["GET", `http://localhost:3100/api/squads/${SQUAD_ID}/costs/by-agent`],
      ["GET", `http://localhost:3100/api/squads/${SQUAD_ID}/costs/by-project`],
      ["GET", `http://localhost:3100/api/issues/${ISSUE_ID}/cost-summary`],
      ["POST", `http://localhost:3100/api/squads/${SQUAD_ID}/cost-events`],
      ["POST", `http://localhost:3100/api/squads/${SQUAD_ID}/finance-events`],
      ["GET", `http://localhost:3100/api/squads/${SQUAD_ID}/costs/finance-summary`],
      ["GET", `http://localhost:3100/api/squads/${SQUAD_ID}/budgets/overview`],
      ["POST", `http://localhost:3100/api/squads/${SQUAD_ID}/budgets/policies`],
      ["PATCH", `http://localhost:3100/api/squads/${SQUAD_ID}/budgets`],
      ["PATCH", `http://localhost:3100/api/agents/${AGENT_ID}/budgets`],
      ["POST", `http://localhost:3100/api/squads/${SQUAD_ID}/budget-incidents/${INCIDENT_ID}/resolve`],
    ]);
  });
  ...
  it("wraps org, execution workspace, environment, and project workspace endpoints", async () => {
    ...
    await run(["org", "get", "--squad-id", SQUAD_ID]);
    await run(["org", "svg", "--squad-id", SQUAD_ID]);
    await run(["agent-config", "list", "--squad-id", SQUAD_ID]);
    await run(["workspace", "list", "--squad-id", SQUAD_ID]);
    ...
    await run(["environment", "list", "--squad-id", SQUAD_ID]);
    await run(["environment", "capabilities", "--squad-id", SQUAD_ID]);
    await run(["environment", "create", "--squad-id", SQUAD_ID, "--payload-json", "{}"]);
    ...
    await run(["environment", "probe-config", "--squad-id", SQUAD_ID, "--payload-json", "{}"]);
    ...
    expect(fetchMock.mock.calls.map((call) => [call[1]?.method ?? "GET", call[0]])).toEqual([
      ["GET", `http://localhost:3100/api/squads/${SQUAD_ID}/org`],
      ["GET", `http://localhost:3100/api/squads/${SQUAD_ID}/org.svg`],
      ["GET", `http://localhost:3100/api/squads/${SQUAD_ID}/agent-configurations`],
      ["GET", `http://localhost:3100/api/squads/${SQUAD_ID}/execution-workspaces`],
      ...
      ["GET", `http://localhost:3100/api/squads/${SQUAD_ID}/environments`],
      ["GET", `http://localhost:3100/api/squads/${SQUAD_ID}/environments/capabilities`],
      ["POST", `http://localhost:3100/api/squads/${SQUAD_ID}/environments`],
      ...
      ["POST", `http://localhost:3100/api/squads/${SQUAD_ID}/environments/probe-config`],
