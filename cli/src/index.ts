...
import { bootstrapSquadLeadInvite } from "./commands/auth-bootstrap-squad-lead.js";
import { dbBackupCommand } from "./commands/db-backup.js";
import { registerEnvLabCommands } from "./commands/env-lab.js";
import { registerContextCommands } from "./commands/client/context.js";
import { registerSquadCommands } from "./commands/client/squad.js";
...
import { registerBotfatherCommands } from "./commands/botfather.js";
...
const DATA_DIR_OPTION_HELP =
  "Slaw data directory root (isolates state from ~/.slaw)";

program
  .name("slaw")
  .description("Slaw CLI — setup, diagnose, and configure your instance")
  .version(cliVersion);
...
program
  ...
  .option("--run", "Start Slaw immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Slaw setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "slaw")
...
const run = program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Slaw")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Slaw server API")
...
registerBotfatherCommands(program);
...
auth
  ...
  .action(bootstrapSquadLeadInvite);
