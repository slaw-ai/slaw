...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Slaw API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "slawApiUrl",
                  String(config.slawApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "slawApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://slaw.example"
            />
          </Field>
          ...
          <Field label="Claimed API key path">
            <DraftInput
              ...
              immediate
              className={inputClass}
              placeholder="~/.openclaw/workspace/slaw-claimed-api-key.json"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "slaw"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Slaw persists a device key during onboarding so pairing approvals
