...
export function SquadInvites() {
  ...
  const inviteHistoryQueryKey = queryKeys.access.invites(selectedSquadId ?? "", "all", INVITE_HISTORY_PAGE_SIZE);
  const invitesQuery = useInfiniteQuery({
    queryKey: inviteHistoryQueryKey,
    queryFn: ({ pageParam }) =>
      accessApi.listInvites(selectedSquadId!, {
  ...
  if (!selectedSquadId) {
    return <div className="text-sm text-muted-foreground">Select a squad to manage invites.</div>;
  }
    ...
    <div className="max-w-5xl space-y-8">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">Squad Invites</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">
          Invite people to request access to this squad. New invite links are copied to your clipboard when they are generated.
        </p>
      </div>
