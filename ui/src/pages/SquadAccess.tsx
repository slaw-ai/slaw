...
export function SquadAccess() {
  ...
  const [draftRole, setDraftRole] = useState<SquadMember["membershipRole"]>(null);
  ...
  const membersQuery = useQuery({
    queryKey: queryKeys.access.squadMembers(selectedSquadId ?? ""),
    queryFn: () => accessApi.listMembers(selectedSquadId!),
  ...
  const joinRequestsQuery = useQuery({
    queryKey: queryKeys.access.joinRequests(selectedSquadId ?? "", "pending_approval"),
    queryFn: () => accessApi.listJoinRequests(selectedSquadId!, "pending_approval"),
    enabled: !!selectedSquadId && !!membersQuery.data?.access.canApproveJoinRequests,
  });
  ...
  const refreshAccessData = async () => {
    ...
    await queryClient.invalidateQueries({ queryKey: queryKeys.access.joinRequests(selectedSquadId, "pending_approval") });
  };

  const updateMemberMutation = useMutation({
    mutationFn: async (input: { memberId: string; membershipRole: SquadMember["membershipRole"]; status: EditableMemberStatus }) => {
      return accessApi.updateMember(selectedSquadId!, input.memberId, {
  ...
  const assignedIssuesQuery = useQuery({
    queryKey: ["access", "member-assigned-issues", selectedSquadId ?? "", removingMember?.principalId ?? ""],
    queryFn: () =>
      issuesApi.list(selectedSquadId!, {
        assigneeUserId: removingMember!.principalId,
        status: reassignmentIssueStatuses,
      }),
    enabled: !!selectedSquadId && !!removingMember,
  });
  ...
  const archiveMemberMutation = useMutation({
      ...
      const reassignment =
        input.target.startsWith("agent:")
          ...
          : input.target.startsWith("user:")
            ...
            : null;
      return accessApi.archiveMember(selectedSquadId!, input.memberId, { reassignment });
    },
    onSuccess: async (result) => {
      ...
      await refreshAccessData();
      if (selectedSquadId) {
        await queryClient.invalidateQueries({ queryKey: queryKeys.issues.list(selectedSquadId) });
        await queryClient.invalidateQueries({ queryKey: queryKeys.issues.listAssignedToMe(selectedSquadId) });
        await queryClient.invalidateQueries({ queryKey: queryKeys.issues.listTouchedByMe(selectedSquadId) });
      }
      pushToast({
  ...
  if (!selectedSquadId) {
    return <div className="text-sm text-muted-foreground">Select a squad to manage access.</div>;
  }

  if (membersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">Loading squad access…</div>;
  }

  if (membersQuery.error) {
    const message =
      membersQuery.error instanceof ApiError && membersQuery.error.status === 403
        ? "You do not have permission to manage squad members."
        : membersQuery.error instanceof Error
          ? membersQuery.error.message
          : "Failed to load squad members.";
    ...
    <div className="max-w-6xl space-y-8">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">Squad Members</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">
          Manage the people who can work in {selectedSquad?.name}. Members can collaborate across the squad by default.
        </p>
      ...
      {access && !access.currentUserRole && (
        <div className="rounded-xl border border-amber-500/40 px-4 py-3 text-sm text-amber-200">
          This account can manage access here through instance-admin privileges, but it does not currently hold an active squad membership.
        </div>
      )}
      ...
      <section className="space-y-4">
        <div className="space-y-1">
          ...
          <p className="max-w-3xl text-sm text-muted-foreground">
            Manage human squad memberships and status here.
          </p>
        </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Review pending join requests before they become active squad members.
                </p>
              </div>
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          ...
          {members.length === 0 ? (
            <div className="px-4 py-8 text-sm text-muted-foreground">No user memberships found for this squad yet.</div>
          ) : (
            members.map((member) => {
                ...
                >
                  ...
                  <div className="text-sm">
                    {member.membershipRole
                      ? HUMAN_SQUAD_MEMBERSHIP_ROLE_LABELS[member.membershipRole]
                      : "Unset"}
                  </div>
                  <div>
      ...
      <Dialog open={!!editingMember} onOpenChange={(open) => !open && setEditingMemberId(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Update squad role and membership status for {editingMember?.user?.name || editingMember?.user?.email || editingMember?.principalId}.
            </DialogDescription>
          </DialogHeader>
          {editingMember && (
            <div className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                <label className="space-y-2 text-sm">
                  <span className="font-medium">Squad role</span>
                  <select
                    className="w-full rounded-md border border-border bg-background px-3 py-2"
                    value={draftRole ?? ""}
                    onChange={(event) =>
                      setDraftRole((event.target.value || null) as SquadMember["membershipRole"])
                    }
                  >
                    <option value="">Unset</option>
                    {Object.entries(HUMAN_SQUAD_MEMBERSHIP_ROLE_LABELS).map(([value, label]) => (
...
export function SquadAccessLegacyRoute() {
  ...
  if (permissionsSlot) {
    return <Navigate to="/squad/settings/permissions" replace />;
  }
  ...
  return (
    <div className="max-w-2xl space-y-5">
      <div className="space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          Advanced access, scoped assignment, and explicit grant controls are provided by installed squad settings extensions.
        </p>
      </div>
      ...
      <div className="space-y-4 rounded-xl border border-border px-5 py-5">
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Core Slaw keeps enforcing squad boundaries and any existing restrictive policy data, but editing advanced permissions requires an installed extension.
          </p>
          {errorMessage ? (
        ...
        <div className="flex flex-wrap gap-2">
          <Button asChild>
            <Link to="/squad/settings/members">Open Members</Link>
          </Button>
          <Button asChild variant="outline">
            <Link to="/squad/settings/invites">Open Invites</Link>
          </Button>
        </div>
      </div>
    </div>
...
function memberDisplayName(member: SquadMember | null) {
...
function isEditableMemberStatus(status: SquadMember["status"]): status is EditableMemberStatus {
