...
describe("Secrets page layout", () => {
  ...
  it("warns that removing a provider vault only removes Slaw config", async () => {
    ...
    expect(document.body.textContent).toContain("from Slaw only");
    ...
    const confirmButton = [...document.querySelectorAll("button")].find(
      (button) => button.textContent?.includes("Remove from Slaw"),
    ) as HTMLButtonElement | undefined;
    await act(async () => {
      confirmButton?.click();
    });
  ...
  it("discovers AWS provider vault candidates and applies selected values as prefill", async () => {
    ...
    await act(async () => {
      ...
      setInputValue(prefixInput!, "slaw");
    });
    await flushReact();
    ...
    expect((document.getElementById("provider-vault-secret-name-prefix") as HTMLInputElement).value).toBe("slaw");
    expect((document.getElementById("provider-vault-kms-key-id") as HTMLInputElement).value).toBe("alias/slaw-secrets");
