...
import { SquadAccess, SquadAccessLegacyRoute } from "./SquadAccess";
...
vi.mock("@/api/access", () => ({
  accessApi: {
    listMembers: (squadId: string) => listMembersMock(squadId),
    listJoinRequests: (squadId: string, status: string) => listJoinRequestsMock(squadId, status),
    updateMember: (squadId: string, memberId: string, input: unknown) =>
      updateMemberMock(squadId, memberId, input),
    updateMemberPermissions: vi.fn(),
    updateMemberAccess: vi.fn(),
    archiveMember: (squadId: string, memberId: string, input: unknown) =>
      archiveMemberMock(squadId, memberId, input),
...
vi.mock("@/api/issues", () => ({
  issuesApi: {
    list: (squadId: string, filters: unknown) => listIssuesMock(squadId, filters),
  },
}));
...
vi.mock("@/context/SquadContext", () => ({
  useSquad: () => ({
    ...
    selectedSquad: { id: "squad-1", name: "Slaw" },
  }),
}));
...
describe("SquadAccess", () => {
  ...
  it("keeps the page human-focused and hides advanced permission controls", async () => {
    ...
    expect(container.textContent).toContain("Manage the people who can work in Slaw");
    expect(container.textContent).toContain("Members can collaborate across the squad by default");
    ...
    expect(document.body.textContent).toContain("Update squad role and membership status");
  ...
  it("saves member role and status without touching grants", async () => {
    ...
    expect(updateMemberMock).toHaveBeenCalledWith("squad-1", "member-1", {
      membershipRole: "owner",
      status: "active",
    });
  ...
  it("removes a member with an issue reassignment target", async () => {
    ...
    expect(archiveMemberMock).toHaveBeenCalledWith("squad-1", "member-1", {
  ...
  it("shows protected member removal reasons from the API", async () => {
      ...
      members: [
        {
          ...
          removal: {
            canArchive: false,
            reason: "Squad admins cannot be removed from squad access.",
          },
        },
      ],
      access: {
    ...
    expect(container.textContent).toContain("Squad admins cannot be removed from squad access.");
  ...
  it("redirects legacy access deep links to the permissions extension route when installed", async () => {
    ...
    await act(async () => {
      root.render(
        <QueryClientProvider client={queryClient}>
          <SquadAccessLegacyRoute />
        </QueryClientProvider>,
      );
    });
    await flushReact();

    expect(mockNavigate).toHaveBeenCalledWith("/squad/settings/permissions", true);
    expect(container.textContent).toContain("/squad/settings/permissions");
