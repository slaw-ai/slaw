...
vi.mock("@/api/access", () => ({
  accessApi: {
    listInvites: (squadId: string, options?: unknown) => listInvitesMock(squadId, options),
...
describe("SquadInvites", () => {
  ...
  beforeEach(() => {
    ...
    listInvitesMock.mockImplementation((_squadId: string, options?: { limit?: number; offset?: number }) => {
    ...
    createSquadInviteMock.mockImplementation(() => {
  ...
  it("renders a human-only invite flow and keeps invite history in a table", async () => {
    ...
    expect(container.textContent).toContain("Squad Invites");
  ...
  it("ignores legacy cached invite arrays and refetches paginated history", async () => {
    ...
    queryClient.setQueryData(["access", "invites", "squad-1", "all"], inviteHistory.slice(0, 2));
