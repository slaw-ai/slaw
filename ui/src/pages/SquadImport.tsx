...
function buildActionMap(preview: SquadPortabilityPreviewResult): Map<string, string> {
  ...
  // Squad file
  if (manifest.squad) {
    const path = ensureMarkdownPath(manifest.squad.path);
    map.set(path, preview.plan.squadAction === "none" ? "skip" : preview.plan.squadAction);
  }
...
function ImportPreviewPane({
  ...
  content: SquadPortabilityFileEntry | null;
  allFiles: Record<string, SquadPortabilityFileEntry>;
  action: string | null;
...
/** Generate a prefix-based rename: e.g. "gstack" + "Squad Lead" → "gstack-Squad Lead" */
...
async function applyImportedSidebarOrder(
  preview: SquadPortabilityPreviewResult | null,
  result: {
    squad: { id: string };
  ...
  if (orderedAgentIds.length > 0) {
    writeAgentOrder(getAgentOrderStorageKey(result.squad.id, userId), orderedAgentIds);
  }
  if (orderedProjectIds.length > 0) {
    await sidebarPreferencesApi.updateProjectOrder(result.squad.id, { orderedIds: orderedProjectIds });
  }
}
...
async function readLocalPackageZip(file: File): Promise<{
  ...
  if (!/\.zip$/i.test(file.name)) {
    throw new Error("Select a .zip squad package.");
  }
...
export function SquadImport() {
  ...
  const [importPreview, setImportPreview] =
    useState<SquadPortabilityPreviewResult | null>(null);
  ...
  const [collisionStrategy, setCollisionStrategy] = useState<SquadPortabilityCollisionStrategy>("rename");
  ...
  // Fetch current squad agents to find Squad Lead adapter type
  const { data: squadAgents } = useQuery({
    queryKey: selectedSquadId ? queryKeys.agents.list(selectedSquadId) : ["agents", "none"],
    queryFn: () => agentsApi.list(selectedSquadId!),
    enabled: Boolean(selectedSquadId),
  });
  const ceoAdapterType = useMemo(() => {
    if (!squadAgents) return "claude_local";
    const squad_lead = squadAgents.find((a) => a.role === "squad_lead");
    return squad_lead?.adapterType ?? "claude_local";
  }, [squadAgents]);

  const localZipHelpText =
    "Upload a .zip exported directly from Slaw. Re-zipped archives created by Finder, Explorer, or other zip tools may not import correctly.";
  ...
  function buildSource(): SquadPortabilitySource | null {
    if (sourceMode === "local") {
  ...
  const previewMutation = useMutation({
    mutationFn: () => {
      ...
      return squadsApi.importPreview({
        source,
        include: { squad: true, agents: true, projects: true, issues: true },
        target:
          targetMode === "new"
            ? { mode: "new_squad", newSquadName: newSquadName || null }
            : { mode: "existing_squad", squadId: selectedSquadId! },
        collisionStrategy,
      });
    },
    onSuccess: (result) => {
      ...
      // Initialize adapter overrides — default all agents to the Squad Lead's adapter type
      ...
      // Check all files by default, then uncheck SQUAD.md for existing squad
      const allFiles = new Set(Object.keys(result.files));
      if (targetMode === "existing" && result.manifest.squad && result.plan.squadAction === "update") {
        const squadPath = ensureMarkdownPath(result.manifest.squad.path);
        allFiles.delete(squadPath);
      }
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      const importedSquad = await squadsApi.get(result.squad.id);
      ...
      setSelectedSquadId(importedSquad.id);
      pushToast({
        tone: "success",
        title: "Import complete",
        body: `${result.squad.name}: ${result.agents.length} agent${result.agents.length === 1 ? "" : "s"} processed.`,
      });
      // Force a fresh dashboard load so newly imported agents are immediately visible.
      window.location.assign(`/${importedSquad.issuePrefix}/dashboard`);
    },
    onError: (err) => {
      pushToast({
        tone: "error",
  ...
  const renameMap = useMemo(() => {
    ...
    for (const c of conflicts) {
      ...
      // Map the parent directory (e.g. agents/squad_lead → gstack-squad_lead) for the file tree
  ...
  function buildFinalAdapterOverrides(): Record<string, SquadPortabilityAdapterOverride> | undefined {
    if (adapterAgents.length === 0) return undefined;
    const overrides: Record<string, SquadPortabilityAdapterOverride> = {};
    for (const agent of adapterAgents) {
      const selectedType = adapterOverrides[agent.slug] ?? agent.adapterType;
      const configVals = adapterConfigValues[agent.slug];
      const override: SquadPortabilityAdapterOverride = { adapterType: selectedType };
      if (configVals) {
  ...
  if (!selectedSquadId) {
    return <EmptyState icon={Download} message="Select a squad to import into." />;
  }
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Choose a GitHub repo or upload a local Slaw zip package.
          </p>
        </div>
        ...
        {sourceMode === "local" ? (
          ...
          <Field
            label="GitHub URL"
            hint="Repo tree path or blob URL to SQUAD.md (e.g. github.com/owner/repo/tree/main/squad)."
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              value={importUrl}
              placeholder="https://github.com/owner/repo/tree/main/squad"
              onChange={(e) => {
        ...
        <Field label="Target" hint="Import into this squad or create a new one.">
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
          ...
          >
            ...
            <option value="existing">
              Existing squad: {selectedSquad?.name}
            </option>
          </select>
        </Field>
        ...
        >
          <select
            ...
            onChange={(e) => {
              setCollisionStrategy(e.target.value as SquadPortabilityCollisionStrategy);
              setImportPreview(null);
            }}
          >
