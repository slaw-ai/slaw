...
import { buildInitialExportCheckedFiles } from "../lib/squad-export-selection";
import { useAgentOrder } from "../hooks/useAgentOrder";
import { useProjectOrder } from "../hooks/useProjectOrder";
import { buildPortableSidebarOrder } from "../lib/squad-portability-sidebar";
...
/**
 * Filter .slaw.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterSlawYaml(yaml: string, checkedFiles: Set<string>): string {
...
function downloadZip(
  ...
  effectiveFiles: Record<string, SquadPortabilityFileEntry>,
) {
  const filteredFiles: Record<string, SquadPortabilityFileEntry> = {};
...
const ROLE_LABELS: Record<string, string> = {
  squad_lead: "Squad Lead", cto: "CTO", cmo: "CMO", cfo: "CFO", coo: "COO",
...
/** Extract the file path from the current URL pathname (after /squad/export/files/) */
function filePathFromLocation(pathname: string): string | null {
  const marker = "/squad/export/files/";
...
export function SquadExport() {
  ...
  const [exportData, setExportData] = useState<SquadPortabilityExportPreviewResult | null>(null);
  ...
  const selectFile = useCallback(
    (filePath: string | null, replace = false) => {
      ...
      if (filePath) {
        navigate(`/squad/export/files/${encodeURI(filePath)}`, { replace });
      } else {
        navigate("/squad/export", { replace });
      }
    },
    [navigate],
  );
  ...
  const exportPreviewMutation = useMutation({
    mutationFn: () =>
      squadsApi.exportPreview(selectedSquadId!, {
  ...
  const downloadMutation = useMutation({
    mutationFn: () =>
      squadsApi.exportBundle(selectedSquadId!, {
        include: { squad: true, agents: true, projects: true, issues: true },
  ...
  useEffect(() => {
    if (!selectedSquadId || exportPreviewMutation.isPending) return;
    if (!isSessionFetched || !areAgentsFetched || !areProjectsFetched) return;
    setExportData(null);
    exportPreviewMutation.mutate();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedSquadId, isSessionFetched, areAgentsFetched, areProjectsFetched, sidebarOrderKey]);
  ...
  // Recompute .slaw.yaml and README.md content whenever checked files
  // change so the preview & download always reflect the current selection.
  const effectiveFiles = useMemo(() => {
    if (!exportData) return {} as Record<string, SquadPortabilityFileEntry>;
    const filtered = { ...exportData.files };

    // Filter .slaw.yaml
    const yamlPath = exportData.slawExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterSlawYaml(exportData.files[yamlPath], checkedFiles);
    }

    // Regenerate README.md based on checked selection
    if (typeof exportData.files["README.md"] === "string") {
      const squadName = exportData.manifest.squad?.name ?? selectedSquad?.name ?? "Squad";
      const squadDescription = exportData.manifest.squad?.description ?? null;
    ...
    return filtered;
  }, [exportData, checkedFiles, selectedSquad?.name]);
  ...
  if (!selectedSquadId) {
    return <EmptyState icon={Package} message="Select a squad to export." />;
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-4 text-sm">
            <span className="font-medium">
              {selectedSquad?.name ?? "Squad"} export
            </span>
