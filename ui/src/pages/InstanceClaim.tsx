...
export function InstanceClaimPage() {
  ...
  const currentPath = useMemo(
    () => `/instance-claim/${encodeURIComponent(token)}${code ? `?code=${encodeURIComponent(code)}` : ""}`,
    [token, code],
  );
  ...
  const statusQuery = useQuery({
    queryKey: ["instance-claim", token, code],
    queryFn: () => accessApi.getInstanceClaimStatus(token, code),
    enabled: token.length > 0 && code.length > 0,
    retry: false,
  });

  const claimMutation = useMutation({
    mutationFn: () => accessApi.claimInstance(token, code),
    onSuccess: async () => {
