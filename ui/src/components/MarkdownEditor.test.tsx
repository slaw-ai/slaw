...
import { buildProjectMentionHref, buildRoutineMentionHref, buildSkillMentionHref } from "@slaw-ai/shared";
import {
...
describe("MarkdownEditor", () => {
  ...
  it("keeps mention queries active across spaces", () => {
    expect(findMentionMatch("Ping @Slaw App", "Ping @Slaw App".length)).toEqual({
      trigger: "mention",
      marker: "@",
      query: "Slaw App",
      atPos: 5,
      endPos: "Ping @Slaw App".length,
    });
  });
  ...
  async function openMentionMenuFor(
    ...
    range.setStart(textNode!, "@Sla".length);
    range.collapse(true);
    ...
    const option = Array.from(document.body.querySelectorAll('button[type="button"]'))
      .find((node) => node.textContent?.includes("Slaw App")) as HTMLButtonElement | undefined;
  ...
  it("accepts mention selection from a touch tap", async () => {
    ...
    expect(handleChange).toHaveBeenCalledWith(
      `[@Slaw App](${buildProjectMentionHref("project-123", "#336699")}) `,
    );
  ...
  it("marks the autocomplete portal as floating UI for modal pointer handling", async () => {
    ...
    const menu = option.closest("[data-slaw-floating-ui]");
    expect(menu).toBeTruthy();
