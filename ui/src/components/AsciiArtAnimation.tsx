...
type SlawSprite = (typeof SLAW_SPRITES)[number];
...
function spriteSize(sprite: SlawSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = SLAW_SPRITES[(baseCol + baseRow) % SLAW_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = SLAW_SPRITES[Math.floor(Math.random() * SLAW_SPRITES.length)]!;
