...
interface SquadPatternIconProps {
  squadName: string;
  logoUrl?: string | null;
...
function makeSquadPatternDataUrl(seed: string, brandColor?: string | null, logicalSize = 22, cellSize = 2): string {
...
export function SquadPatternIcon({
  squadName,
  logoUrl,
  brandColor,
  className,
  logoFit = "cover",
}: SquadPatternIconProps) {
  const initial = squadName.trim().charAt(0).toUpperCase() || "?";
  ...
  const patternDataUrl = useMemo(
    () => makeSquadPatternDataUrl(squadName.trim().toLowerCase(), brandColor),
    [squadName, brandColor],
  );
