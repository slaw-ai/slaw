...
export interface SlawIssueRuntimeReassignment {
  assigneeAgentId: string | null;
  assigneeUserId: string | null;
}

export interface SlawIssueRuntimeSendOptions {
  body: string;
  reopen?: boolean;
  reassignment?: SlawIssueRuntimeReassignment;
}

interface UseSlawIssueRuntimeOptions {
  messages: readonly ThreadMessage[];
  isRunning: boolean;
  onSend: (options: SlawIssueRuntimeSendOptions) => Promise<void>;
...
export function useSlawIssueRuntime({
  messages,
  isRunning,
  onSend,
  onCancel,
}: UseSlawIssueRuntimeOptions) {
