...
summary: Build, package, and distribute adapters as plugins without modifying Slaw source
---

Slaw supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to Slaw's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `slaw-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with Slaw | Published to npm or linked via `file:` |
| Updates | Requires Slaw release | Independent versioning |
...
```json
{
  ...
  "dependencies": {
    "@slaw-ai/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
| `slaw.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
```ts
import type { ServerAdapterModule } from "@slaw-ai/adapter-utils";
...
export async function execute(
  ...
  // 2. Build environment with Slaw vars injected
  const env = buildSlawEnv(agent);
...
```sh
# Via the Slaw UI
# Settings → Adapters → Install from npm → "my-slaw-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-slaw-adapter"}'
```
...
Local adapters are symlinked into Slaw's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.slaw/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-slaw-adapter",
...
Other Slaw users can then install your adapter by package name from the UI or API.
