...
summary: What adapters are and how they connect agents to Slaw
---

Adapters are the bridge between Slaw's orchestration layer and agent runtimes. Each adapter knows how to invoke a specific type of AI agent and capture its results.

## How Adapters Work

When a heartbeat fires, Slaw:
...
| Hermes Local | `hermes_local` | Runs Hermes CLI locally (`hermes-slaw-adapter`) |
...
|---------|---------|----------|-------------|
| Droid Local | `@henkey/droid-slaw-adapter` | `droid_local` | Runs Factory Droid locally |

## External Adapters

You can build and distribute adapters as standalone packages — no changes to Slaw's source code required. External adapters are loaded at startup via the plugin system.
...
my-adapter/
  src/
    ...
    cli/
      format-event.ts   # Terminal output for `slaw run --watch`
```
...
External adapters can ship a self-contained UI parser that tells the Slaw web UI how to render their stdout. Without it, the UI uses a generic shell parser. See the [UI Parser Contract](/adapters/adapter-ui-parser) for details.
