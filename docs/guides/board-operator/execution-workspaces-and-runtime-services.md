...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in Slaw.

Slaw now presents this as a workspace-command model:
...
- Slaw does not automatically start or stop these workspace services as part of issue execution.
- Slaw also does not automatically restart workspace services on server boot.
...
2. Slaw realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. Slaw persists execution-workspace metadata such as paths, refs, and provisioning settings.
