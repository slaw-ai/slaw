...
How to get OpenClaw running in a Docker container for local development and testing the Slaw OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Slaw includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-slaw-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a Slaw host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-slaw-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `SLAW_HOST_PORT` (default `3100`)
- `SLAW_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your Slaw deployment is `authenticated`, provide auth context:

```bash
SLAW_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
SLAW_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Slaw server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Slaw URL (typically `http://host.docker.internal:3100`).
- If Slaw rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm slaw allowed-hostname host.docker.internal
```

Then restart Slaw and rerun the smoke script.
