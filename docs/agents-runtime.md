...
Audience: Operators setting up and running agents in Slaw

## 1. What this system does

Agents in Slaw do not run continuously.  
...
- `droid_local`: runs your local Factory Droid CLI (`@henkey/droid-slaw-adapter`)

For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), Slaw assumes the CLI is already installed and authenticated on the host machine.
...
Slaw stores session IDs for resumable adapters.
...
Slaw pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Slaw surfaces this as a warning in environment tests, not a hard error.
