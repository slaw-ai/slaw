...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Slaw-Signature` + `X-Slaw-Timestamp` header pair matching the trigger's signing mode.
