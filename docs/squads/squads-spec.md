# Agent Squads Specification

Extension of the Agent Skills Specification

Version: `agentsquads/v1-draft`

## 1. Purpose

An Agent Squad package is a filesystem- and GitHub-native format for describing a squad, team, agent, project, task, and associated skills using markdown files with YAML frontmatter.

This specification is an extension of the Agent Skills specification, not a replacement for it.

It defines how squad-, team-, and agent-level package structure composes around the existing `SKILL.md` model.

This specification is vendor-neutral. It is intended to be usable by any agent-squad runtime, not only Slaw.
...
kind: squad | team | agent | project | task
slug: my-slug
...
`SQUAD.md` is the root entrypoint for a whole squad package.

### Required fields

```yaml
name: Lean Dev Shop
description: Small engineering-focused AI squad
slug: lean-dev-shop
schema: agentsquads/v1
```
...
includes:
  - https://github.com/example/shared-squad-parts/blob/0123456789abcdef0123456789abcdef01234567/teams/engineering/TEAM.md
requirements:
  secrets:
    - OPENAI_API_KEY
```
...
- `SQUAD.md` may include agents directly, teams, projects, tasks, or skills
- a squad importer may render `includes` as the tree/checkbox import UI
...
- a team package can be imported into an existing squad and attached under a target manager
...
3. a tool-managed squad skill library entry with the same shortname
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.slaw.yaml`
...
- Slaw must not require extra top-level fields for skill validity
- Slaw-specific extensions must live under `metadata.slaw` or `metadata.sources`
...
In other words, this spec extends Agent Skills upward into squad/team/agent composition. It does not redefine skill package semantics.
...
For Slaw, the preferred fidelity extension is:
...
- Slaw tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Slaw shape:
...
Additional rules for Slaw exporters:
...
- omit empty and default-valued Slaw fields when possible
...
- Slaw exporters should emit `.slaw.yaml` as a sidecar by default
...
Slaw can map this spec to its runtime model like this:
...
- Slaw extension:
  - `.slaw.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Slaw-specific fidelity

Inline Slaw-only metadata that must live inside a shared markdown file should use:

- `metadata.slaw`

That keeps the base format broader than Slaw.

This specification itself remains vendor-neutral and intended for any agent-squad runtime, not only Slaw.

## 20. Cutover

Slaw should cut over to this markdown-first package model as the primary portability format.

`slaw.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Slaw, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
...
- make `squads.sh` a discovery layer for repos implementing this spec, not a publishing authority
