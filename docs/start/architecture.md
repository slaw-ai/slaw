...
Slaw is a monorepo with four main layers.
...
│
├── skills/                      # Agent skills
│   └── slaw/               # Core Slaw skill (heartbeat protocol)
│
├── cli/                         # CLI client
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with Slaw env vars and a prompt
4. **Agent work** — The agent calls Slaw's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between Slaw and agent runtimes. Each adapter is a package with three modules:
...
- **CLI module** — terminal formatter for `slaw run --watch`
