...
Slaw supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
```sh
pnpm slaw allowed-hostname my-machine
```
...
```sh
SLAW_DEPLOYMENT_MODE=authenticated SLAW_BIND=lan pnpm slaw run
```
